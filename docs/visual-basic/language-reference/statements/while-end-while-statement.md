...
|Termin|Definicja|  
|---|---|  
|`condition`|Wymagany. `Boolean`wyrażenie. Jeśli `condition` jest `Nothing`, Visual Basic traktuje ją jako `False`.|  
|`statements`|Opcjonalny. Jeden lub więcej następujących instrukcji `While`, uruchamiania zawsze `condition` jest `True`.|  
|`Continue While`|Opcjonalny. Przekazuje sterowanie do następnej iteracji `While` bloku.|  
|`Exit While`|Opcjonalny. Przekazuje sterowanie poza `While` bloku.|  
