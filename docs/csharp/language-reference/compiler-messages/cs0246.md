...
- Czy poznasz nazwę typu lub przestrzeni nazw? Bez poprawnej nazwy kompilator nie może odnaleźć definicji typu lub przestrzeni nazw. Często dzieje się tak, ponieważ wielkość liter użyta w nazwie typu jest niepoprawna. Na przykład generuje `Dataset ds;` CS0246, ponieważ należy wykonać wielką literę s w zestawie danych.  
...
- Jeśli błąd dotyczy nazwy typu, podano odpowiednią [dyrektywę using](../keywords/using-directive.md)lub, Alternatywnie, w pełni zakwalifikowanie nazwy typu? Rozważmy następującą deklarację `DataSet ds`:. Aby użyć tego `DataSet` typu, potrzebne są dwie rzeczy. Najpierw należy odwołać się do zestawu, który zawiera definicję dla tego `DataSet` typu. Po drugie, potrzebna `using` jest dyrektywa dla przestrzeni nazw, w której `DataSet` znajduje się lokalizacja. Na przykład, ponieważ `DataSet` znajduje się w przestrzeni nazw **System. Data** , na początku kodu potrzebna jest następująca dyrektywa: `using System.Data`.  
...
- Czy używasz *dyrektywy aliasu using* bez w pełni kwalifikowanej nazwy typu? Dyrektywa aliasu nie używa dyrektyw w pliku kodu źródłowego do rozpoznawania typów. `using` `using` Poniższy przykład generuje CS0246, ponieważ typ `List<int>` nie jest w pełni kwalifikowany. `using` Dyrektywa dla`System.Collections.Generic` nie uniemożliwia błędu.  
