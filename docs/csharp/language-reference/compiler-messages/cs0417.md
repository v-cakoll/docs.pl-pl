...
'Identyfikator': nie można używać argumentów podczas tworzenia wystąpienia typu zmiennej  
  
 Ten błąd występuje, gdy wywołanie operatora `new` na parametrze typu ma argumenty. Jedynym Konstruktorem, którego można wywołać za pomocą operatora `new` dla typu nieznanego parametru jest konstruktor, który nie ma argumentów. Jeśli potrzebujesz wywołania innego konstruktora, rozważ użycie ograniczenia typu klasy lub ograniczenia interfejsu.  
