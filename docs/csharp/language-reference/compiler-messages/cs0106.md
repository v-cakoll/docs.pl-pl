...
- Modyfikatory dostępu są niedozwolone w [funkcja lokalna](../../programming-guide/classes-and-structs/local-functions.md). Funkcje lokalne są zawsze prywatne.
 ...
 W poprzednich wersjach programu Visual Studio `static` modyfikator było niedozwolone w klasie, ale `static` klasy są dozwolone, począwszy od [!INCLUDE[vsprvsext](~/includes/vsprvsext-md.md)].  
