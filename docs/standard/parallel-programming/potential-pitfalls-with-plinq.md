...
Ten sam problem odnosi się do <xref:System.Threading.Tasks.Parallel.ForEach%2A?displayProperty=nameWithType>. Innymi słowy, `source.AsParallel().Where().ForAll(...)` powinien być silnie preferowany, aby `Parallel.ForEach(source.AsParallel().Where(), ...)`.
