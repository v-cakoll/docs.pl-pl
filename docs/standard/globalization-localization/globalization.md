...
Domyślnie platforma .NET używa ciągów Unicode. Ciąg Unicode składa się z zero, jeden lub więcej obiektów <xref:System.Char>, z których każdy reprezentuje jednostkę kodu UTF-16. Istnieje reprezentacja Unicode dla niemal każdego znaku w każdym zestawie znaków używanym na całym świecie.
...
- Zapisz liczbę jako ciąg przy użyciu Konwencji formatowania kultury niezmiennej, która jest zwracana przez właściwość <xref:System.Globalization.CultureInfo.InvariantCulture%2A?displayProperty=nameWithType>.

- Serializacja liczby w postaci binarnej, a nie w formacie ciągu.

Poniższy przykład ilustruje ostatnią metodę. Deserializować tablicę wartości <xref:System.Double>, a następnie deserializacji i wyświetla je przy użyciu Konwencji formatowania kultur angielskiej (Stany Zjednoczone) i francuski (Francja).
...
W programie .NET Klasa <xref:System.Globalization.CultureInfo> reprezentuje określoną kulturę lub region. Niektóre z jej właściwości zwracają obiekty, które zawierają określone informacje dotyczące pewnego aspektu kultury:
