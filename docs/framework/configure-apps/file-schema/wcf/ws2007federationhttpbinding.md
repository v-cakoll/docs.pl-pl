...
|`useDefaultWebProxy`|Wartość wskazująca, czy jest używany konfigurowany przez system serwer proxy HTTP. Jeśli ten atrybut jest `true`, adres serwera proxy musi być `null` (to nie jest ustawione). Wartość domyślna to `true`.|
