...
Rozmiar globalnej pamięci współdzielonej można ustawić tylko przy użyciu pliku konfiguracji.  Domyślny rozmiar to 524 288 bTak, maksymalny rozmiar to 33 554 432 bajtów, a minimalny rozmiar to 32 768 bajtów.  Ponieważ globalna pamięć udostępniona jest współdzielona przez wszystkie procesy i kategorie, pierwszy twórca określa rozmiar.  W przypadku zdefiniowania rozmiaru w pliku konfiguracyjnym aplikacji ten rozmiar jest używany tylko wtedy, gdy aplikacja jest pierwszą aplikacją, która powoduje wykonanie liczników wydajności.  W związku z tym poprawną lokalizacją do określenia `filemappingsize` wartości jest plik Machine. config.  Pamięć w globalnej pamięci współdzielonej nie może zostać wydana przez poszczególne liczniki wydajności, więc ostatecznie globalna pamięć udostępniona jest wyczerpana, jeśli utworzono dużą liczbę wystąpień liczników wydajności o różnych nazwach.
