...
.NET Framework ma już klasy ogólne `Tuple` . Jednak te klasy miały dwa zasadnicze ograniczenia. Dla jednego, `Tuple` klasy o nazwie ich właściwości `Item1` , itd `Item2` . Te nazwy nie zawierają informacji semantycznych. Korzystanie `Tuple` z tych typów nie pozwala na komunikowanie znaczenia każdej z właściwości. Nowe funkcje języka umożliwiają deklarowanie i używanie semantycznie zrozumiałych nazw dla elementów w spójnej kolekcji.

`Tuple`Klasy powodują zwiększenie wydajności, ponieważ są to typy odwołań. Użycie jednego z `Tuple` typów oznacza alokowanie obiektów. W przypadku ścieżek aktywnych alokowanie wielu małych obiektów może mieć wymierny wpływ na wydajność aplikacji. W związku z tym obsługa wielu krotek wykorzystuje nowe `ValueTuple` struktury.

Aby uniknąć tych uchybień, można utworzyć `class` lub, `struct` Aby przenieść wiele elementów. Niestety, to jeszcze więcej pracy i zasłania zamiar projektowania. Tworzenie `struct` lub `class` oznacza, że definiujesz typ z danymi i zachowaniem. Wiele razy, Wystarczy przechowywać wiele wartości w pojedynczym obiekcie.
...
Wszystkie `ValueTuple` typy są *niemodyfikowalnymi strukturami*. Każde pole elementu członkowskiego jest polem publicznym. Dzięki temu są one bardzo lekkie. Jednak oznacza to, że nie należy używać krotek, gdzie niezmienności jest ważne.
...
`ValueTuple`Struktura ma pola o nazwach `Item1` , `Item2` , `Item3` i tak dalej, podobnie jak właściwości zdefiniowane w istniejących `Tuple` typach.
...
Ważne jest, aby zrozumieć te podstawowe podstawy nowych krotek i `ValueTuple` Typ, aby zrozumieć reguły przypisywania nazwanych spójnych krotek.
...
Jeśli podano jawną nazwę, która ma pierwszeństwo przed nazwą przewidywaną. Na przykład w poniższym inicjatorze elementy to `explicitFieldOne` i `explicitFieldTwo` , nie `localVariableOne` i `localVariableTwo` :
...
Te sytuacje nie powodują błędów kompilatora, ponieważ mogą to być istotne zmiany w kodzie zapisanym w języku C# 7,0, gdy projekcje nazw pól krotek nie są dostępne.

## <a name="equality-and-tuples"></a>Równość i krotki

Począwszy od języka C# 7,3, typy krotek obsługują `==` `!=` Operatory i. Te operatory działają przez porównanie poszczególnych elementów członkowskich argumentu po lewej stronie z każdym elementem członkowskim argumentu w odpowiedniej kolejności. Te porównania są skracane. Przestaną oceniać członków, gdy tylko jedna para nie jest równa. W poniższym przykładzie kodu użyto `==` , ale wszystkie reguły porównania mają zastosowanie do `!=` . Poniższy przykład kodu pokazuje porównanie równości dla dwóch par liczb całkowitych:
...
Obsługa refaktoryzacji programu Visual Studio ułatwia wyodrębnienie funkcji podstawowych statystyk do metody prywatnej. Zapewnia `private static` metodę, która zwraca typ krotki z trzema wartościami `Sum` , `SumOfSquares` i `Count` :
...
Język umożliwia korzystanie z kilku opcji, których można użyć, jeśli chcesz zrobić kilka szybkiej edycji ręcznie. Najpierw można użyć `var` deklaracji do zainicjowania wyniku krotki z `ComputeSumAndSumOfSquares` wywołania metody. Możesz również utworzyć trzy zmienne dyskretne wewnątrz `ComputeSumAndSumOfSquares` metody. Wersja końcowa jest pokazana w poniższym kodzie:
...
> [!NOTE]
> W języku C# 7,1, projekcje krotek umożliwiają tworzenie nazwanych krotek przy użyciu elementów w sposób podobny do nazw właściwości w typach anonimowych. W powyższym kodzie, `select` instrukcja w projekcji zapytania tworzy krotkę z elementami `ID` i `Title` .
...
Autor typu może zdefiniować jedną lub więcej `Deconstruct` metod, które przypisują wartości do dowolnej liczby [ `out` zmiennych](language-reference/keywords/out-parameter-modifier.md) reprezentujących elementy danych, które składają się na typ. Na przykład następujący `Person` Typ definiuje `Deconstruct` metodę, która dekonstruuje obiekt osoby do elementów reprezentujących imię i nazwisko:

[!code-csharp[TypeWithDeconstructMethod](../../samples/snippets/csharp/tuples/person.cs#12_TypeWithDeconstructMethod "Type with a deconstruct method")]

Metoda dekonstrukcja umożliwia przypisanie od a `Person` do dwóch ciągów reprezentujących `FirstName` `LastName` właściwości i:
...
`Deconstruct`Metoda może być metodą rozszerzenia, która rozpakuje dostępne elementy członkowskie danych obiektu. W poniższym przykładzie przedstawiono `Student` Typ, pochodny od `Person` typu, a metoda rozszerzająca, która dekonstrukcjauje `Student` do trzech zmiennych reprezentujących,, `FirstName` `LastName` i `GPA` :
...
`Student`Obiekt ma teraz dwie dostępne `Deconstruct` metody: Metoda rozszerzająca zadeklarowana dla `Student` typów i składowej `Person` typu. Oba znajdują się w zakresie, co umożliwia `Student` rozbudowanie na dwie zmienne lub trzy.
...
Należy ostrożnie definiować wiele `Deconstruct` metod w klasie lub hierarchii klas. Wiele `Deconstruct` metod, które mają taką samą liczbę `out` parametrów, może szybko spowodować niejasności. Obiekty wywołujące mogą nie być w stanie łatwo wywołać żądanej `Deconstruct` metody.

W tym przykładzie istnieje minimalna szansa na niejednoznaczne wywołanie, ponieważ `Deconstruct` Metoda `Person` ma dwa parametry wyjściowe, a `Deconstruct` Metoda dla `Student` ma wartość trzy.
...
`Deconstruct`Metoda może przekonwertować `Person` obiekt `p` na krotkę zawierającą dwa ciągi, ale nie ma zastosowania w kontekście testów równości.

## <a name="tuples-as-out-parameters"></a>Krotki jako parametry out

Krotki mogą służyć jako *same* [ `out` Parametry](language-reference/keywords/out-parameter-modifier.md) . Nie należy mylić z żadną niejednoznaczność wymienioną wcześniej w sekcji [dekonstrukcja](#deconstruction) . W wywołaniu metody jest potrzebne tylko opisywanie kształtu krotki:

[!code-csharp[TuplesAsOutParameters](~/samples/snippets/csharp/tuples/program.cs#01_TupleAsOutVariable "Tuples as out parameters")]

Alternatywnie możesz użyć [_nienazwanej_](#named-and-unnamed-tuples) krotki i odwołać się do jego pól w `Item1` i `Item2` :
