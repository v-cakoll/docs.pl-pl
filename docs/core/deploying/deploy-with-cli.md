...
\*W przypadku korzystania z zestawu SDK w wersji 3,0, zależny od platformy plik wykonywalny jest domyślnym trybem publikowania podczas uruchamiania polecenia Basic `dotnet publish` . Ma to zastosowanie tylko wtedy, gdy projekt jest przeznaczony dla **platformy .net core 2,1** lub **.NET Core 3,0**.

## <a name="publishing-basics"></a>Podstawowe informacje o publikowaniu

`<TargetFramework>` Ustawienie pliku projektu określa domyślną platformę docelową podczas publikowania aplikacji. Można zmienić platformę docelową na dowolną prawidłową [moniker platformy docelowej (TFM)](../../standard/frameworks.md). Na przykład, jeśli używany `<TargetFramework>netcoreapp2.2</TargetFramework>`jest projekt, tworzony jest plik binarny, który jest przeznaczony dla programu .NET Core 2,2. TFM określony w tym ustawieniu jest domyślnym obiektem docelowym używanym przez [`dotnet publish`](../tools/dotnet-publish.md) polecenie.
...
O ile nie określono inaczej, katalog [`dotnet publish`](../tools/dotnet-publish.md) wyjściowy polecenia jest. `./bin/<BUILD-CONFIGURATION>/<TFM>/publish/` Domyślny tryb **kompilacji — konfiguracja** jest **debugowany** , chyba że zostanie `-c` zmieniony przy użyciu parametru. Na przykład program `dotnet publish -c Release -f netcoreapp2.1` publikuje w `myfolder/bin/Release/netcoreapp2.1/publish/`usłudze.
...
Plik `Program.cs` lub`Program.vb` generowany przez szablon konsoli należy zmienić na następujący:
...
W przypadku interfejsu wiersza polecenia zestaw .NET Core SDK 2. x wdrożenie zależne od platformy (FDD) jest domyślnym trybem dla `dotnet publish` podstawowe polecenie.

Po opublikowaniu aplikacji jako FDD `<PROJECT-NAME>.dll` plik zostanie utworzony `./bin/<BUILD-CONFIGURATION>/<TFM>/publish/` w folderze. Aby uruchomić aplikację, przejdź do folderu danych wyjściowych i Użyj `dotnet <PROJECT-NAME>.dll` polecenia.
...
W przypadku interfejsu wiersza polecenia zestaw .NET Core SDK 3. x zależny od platformy plik wykonywalny (całego) jest domyślnym trybem `dotnet publish` dla podstawowych poleceń. Nie trzeba określać żadnych innych parametrów, o ile chcesz ustawić jako docelowy bieżący system operacyjny.

W tym trybie jest tworzony Host wykonywalny specyficzny dla platformy, który umożliwia hostowanie aplikacji dla wielu platform. Ten tryb jest podobny do FDD, ponieważ FDD wymaga hosta w postaci `dotnet` polecenia. Nazwa pliku wykonywalnego hosta różni się w zależności od platformy i nosi nazwę podobną do `<PROJECT-FILE>.exe`. Można uruchomić ten plik wykonywalny bezpośrednio zamiast wywoływania `dotnet <PROJECT-FILE>.dll` , który jest nadal akceptowalnym sposobem uruchomienia aplikacji.
...
Musisz (z wyjątkiem programu .NET Core 3. x, gdy jest przeznaczony dla bieżącej platformy), użyj następujących przełączników `dotnet publish` z poleceniem, aby opublikować całego:
...
Za każdym razem, `-r` gdy używasz przełącznika, ścieżka folderu wyjściowego zmieni się na:`./bin/<BUILD-CONFIGURATION>/<TFM>/<RID>/publish/`

Jeśli używasz [przykładowej aplikacji](#sample-app), uruchom `dotnet publish -f netcoreapp2.2 -r win10-x64 --self-contained false`polecenie. To polecenie tworzy następujący plik wykonywalny:`./bin/Debug/netcoreapp2.2/win10-x64/publish/apptest1.exe`
