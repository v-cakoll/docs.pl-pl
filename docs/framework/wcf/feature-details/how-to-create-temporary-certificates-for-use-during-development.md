...
1.  Za pomocą narzędzia MakeCert.exe przy użyciu następujących parametrów:  
  
    1.  `-n` `subjectName`. Określa nazwę podmiotu. Konwencja jest jako prefiks nazwy podmiotu, za pomocą "CN =" do "Common Name".  
    ...
    ```  
     ...
     Zostanie wyświetlony monit podanie hasła w celu ochrony klucza prywatnego. To hasło będzie wymagane podczas tworzenia certyfikatu podpisanego przez ten certyfikat główny.  
...
1.  Za pomocą narzędzia MakeCert.exe przy użyciu następujących parametrów:  
    ...
    1.  `-sk` `subjectKey`. Lokalizacja kontenera klucza podmiotu, który zawiera klucz prywatny. Jeśli kontener kluczy nie istnieje, utworzony. Jeśli żadna z opcji -sk lub - sv jest używany, kontener kluczy o nazwie JoeSoft jest domyślnie tworzone.  
  
    2.  `-n` `subjectName`. Określa nazwę podmiotu. Konwencja jest jako prefiks nazwy podmiotu, za pomocą "CN =" do "Common Name".  
  
    3.  `-iv` `issuerKeyFile`. Określa plik klucza prywatnego przez wystawcę.  
  
    4.  `-ic` `issuerCertFile`. Określa lokalizację certyfikatu wystawcy.  
