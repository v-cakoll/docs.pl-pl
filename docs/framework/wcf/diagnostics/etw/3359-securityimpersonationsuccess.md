---
title: 3359 — SecurityImpersonationSuccess
ms.date: 03/30/2017
ms.assetid: 80bcea7c-ae57-4b89-8032-5f9787545b07
ms.openlocfilehash: 270387b3ab65c7485ab27e9617040c84999b8a40
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
ms.locfileid: "33464251"
---
# <a name="3359---securityimpersonationsuccess"></a>3359 — SecurityImpersonationSuccess
## <a name="properties"></a>Właściwości  
  
|||  
|-|-|  
|ID|3359|  
|Słowa kluczowe|Zabezpieczenia|  
|Poziom|Pełny|  
|Kanał|Microsoft-Windows aplikacji debugowania serwera — aplikacje|  
  
## <a name="description"></a>Opis  
 To zdarzenie jest emitowany podczas personifikacji zabezpieczeń zakończyła się pomyślnie.  
  
## <a name="message"></a>Komunikat  
 Personifikacja zabezpieczeń zakończyło się pomyślnie.  
  
## <a name="details"></a>Szczegóły
