...
- Typy zasad odbicia określają, które metadane są udostępniane w czasie wykonywania dla odbicia:
  ...
  |Element|Zdezaktywować|Przycisku|dynamicznych|
  |-------------|--------------|------------|-------------|
...
|`Auto`|Określa, że zasady domyślne mają być stosowane dla typu zasad dla tego elementu programu. Jest to identyczne z pominięciem zasad dla tego typu zasad. `Auto` jest zwykle używany do wskazania, że zasady są dziedziczone z elementu nadrzędnego.|✓|✓|
