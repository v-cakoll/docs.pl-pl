---
title: ImageList — Składnik (Formularze systemu Windows)
ms.date: 03/30/2017
helpviewer_keywords:
- ImageList component [Windows Forms]
- image controls
ms.assetid: 83b48634-782b-464d-9b7d-568dc6e0bef2
ms.openlocfilehash: 68296328a574997d5211bbd26ea4ca40be926e75
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
---
# <a name="imagelist-component-windows-forms"></a>ImageList — Składnik (Formularze systemu Windows)
Formularze systemu Windows `ImageList` składnik jest używany do przechowywania obrazów, które mogą być następnie wyświetlane przez formanty. Listy obrazów umożliwia pisanie kodu dla wykazu powinny być zawsze obrazów.  
  
## <a name="in-this-section"></a>W tej sekcji  
 [ImageList, składnik — omówienie](../../../../docs/framework/winforms/controls/imagelist-component-overview-windows-forms.md)  
 Wyjaśniono, co to jest ten składnik i jego najważniejszych funkcjach i właściwości  
  
 [Instrukcje: dodawanie lub usuwanie obrazów za pomocą składnika ImageList formularzy Windows Forms](../../../../docs/framework/winforms/controls/how-to-add-or-remove-images-with-the-windows-forms-imagelist-component.md)  
 Zapewnia instrukcje do dodawania i usuwania obrazów z listy obrazów.  
  
 Zobacz też [porady: Dodawanie lub usuwanie obrazów ImageList przy użyciu projektanta](http://msdn.microsoft.com/library/ms233674\(v=vs.110\)).  
  
## <a name="reference"></a>Tematy pomocy  
 <xref:System.Windows.Forms.ImageList>  
 Ta klasa opisuje i zawiera łącza do wszystkich jej członków.  
  
## <a name="related-sections"></a>Sekcje pokrewne  
 [Kontrolki do użycia w formularzach Windows Forms](../../../../docs/framework/winforms/controls/controls-to-use-on-windows-forms.md)  
 Zawiera listę wszystkich formanty formularzy systemu Windows, linki do informacji na temat ich użycia.
