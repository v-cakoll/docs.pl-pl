...
## <a name="parts"></a>Części  
 ...
 `accessmodifier`  
 Opcjonalne dla co najwyżej jednej instrukcji `Get` i `Set` w tej właściwości. Może być jedną z następujących czynności:  
...
- `Protected Friend`  
 ...
 `statements`  
 Opcjonalny. Jedna lub więcej instrukcji, które są uruchamiane po wywołaniu procedury `Set` właściwości.  
  
 `End Set`  
 Wymagany. Kończy definicję procedury `Set` właściwości.  
