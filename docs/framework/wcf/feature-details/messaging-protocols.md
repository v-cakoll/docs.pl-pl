...
<span data-ttu-id="e5e3d-111">Specyfikacja/dokumentu:</span><span class="sxs-lookup"><span data-stu-id="e5e3d-111">Specification/Document:</span></span>
...
- <span data-ttu-id="e5e3d-152">B2121: WCF zwraca następujące kody błędów protokołu SOAP 1.1: `s11:mustUnderstand`, `s11:Client`, i `s11:Server`.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-152">B2121: WCF returns the following SOAP 1.1 Fault Codes: `s11:mustUnderstand`, `s11:Client`, and `s11:Server`.</span></span>
...
- <span data-ttu-id="e5e3d-164">R2222: `application/soap+xml` parametr akcji, jeśli jest obecny na wiadomości SOAP 1.2, muszą być zgodne `wsa:Action` gdy są używane usługi WS-Addressing 2004/08 lub 1.0 WS-Addressing.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-164">R2222: The `application/soap+xml` action parameter, when present on a SOAP 1.2 message, must match `wsa:Action` when WS-Addressing 2004/08 or WS-Addressing 1.0 are used.</span></span>
...
#### <a name="request-reply"></a><span data-ttu-id="e5e3d-210">Żądanie i odpowiedź</span><span class="sxs-lookup"><span data-stu-id="e5e3d-210">Request-Reply</span></span>
...
<span data-ttu-id="e5e3d-223">R3412: WCF tworzy następujące błędy, zdefiniowane przez WS-Addressing 1.0.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-223">R3412: WCF produces the following faults defined by WS-Addressing 1.0.</span></span>
...
<span data-ttu-id="e5e3d-240">Następujących asercji zasad ma punkt końcowy tematu zasad [WS-PA] i wskazuje, że komunikaty wysyłane i odbierane z punktu końcowego musi używać protokołu WS-Addressing 2004/08.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-240">The following policy assertion has an Endpoint Policy Subject [WS-PA] and indicates that messages sent and received from the endpoint must use WS-Addressing 2004/08.</span></span>
...
<span data-ttu-id="e5e3d-250">W przypadku pojawieniu się odpowiedzi wysyłane za pośrednictwem żądań http w oddzielnych jest potwierdzenie ws am</span><span class="sxs-lookup"><span data-stu-id="e5e3d-250">For replies sent over separate http requests, the ws-am assertion is</span></span>
...
<span data-ttu-id="e5e3d-266">Aby rozwiązać ten niejasności, WCF obsługuje jednej wersji `Action` atrybutu.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-266">To resolve this controversy, WCF supports a single version of the `Action` attribute.</span></span>
...
### <a name="composition-with-ws-security"></a><span data-ttu-id="e5e3d-275">Kompozycja za pomocą usługi WS-Security</span><span class="sxs-lookup"><span data-stu-id="e5e3d-275">Composition with WS-Security</span></span>
...
- <span data-ttu-id="e5e3d-296">MIME XOP kodowania, stosowane do protokołu SOAP 1.x koperty.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-296">A MIME XOP encoding applied to SOAP 1.x Envelope.</span></span>
...
5. <span data-ttu-id="e5e3d-321">Utwórz głównej części MIME z XOP koperty protokołu SOAP z kroku 4.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-321">Create a root MIME part with the XOP SOAP Envelope from step 4.</span></span>
...
#### <a name="http-content-type-header"></a><span data-ttu-id="e5e3d-334">Nagłówek Content-Type protokołu HTTP</span><span class="sxs-lookup"><span data-stu-id="e5e3d-334">HTTP Content-Type Header</span></span>
...
<span data-ttu-id="e5e3d-352">SOAP 1.x koperty jest hermetyzowany jako część pakietu XOP MIME głównym i jest często określany mianem `infoset` części.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-352">The SOAP 1.x Envelope is encapsulated as a root part of the XOP MIME package and is often called the `infoset` part.</span></span>

- <span data-ttu-id="e5e3d-353">R4141: SOAP 1.x koperty musi być zhermetyzowany w ramach głównym pakietu XOP MIME, o nazwie `infoset` wchodzi w skład i odwołania z HTTP Content-Type.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-353">R4141: The SOAP 1.x Envelope must be encapsulated as a root part of the XOP MIME package, called the `infoset` part and referenced from the HTTP Content-Type.</span></span>

- <span data-ttu-id="e5e3d-354">R4142: SOAP `Infoset` części musi zawierać następujące nagłówki MIME: `Content-ID`, `Content-Transfer-Encoding`, i `Content-Type`.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-354">R4142: The SOAP `Infoset` part must include the following MIME headers: `Content-ID`, `Content-Transfer-Encoding`, and `Content-Type`.</span></span>

<span data-ttu-id="e5e3d-355">Format nagłówka Content-ID jest definiowany przez 2045 RFC jako</span><span class="sxs-lookup"><span data-stu-id="e5e3d-355">The format of the Content-ID header is defined by RFC 2045 as</span></span>

```
"Content-ID" ":" msg-id
```

<span data-ttu-id="e5e3d-356">gdzie `msg-id` jest zdefiniowany w specyfikacji RFC 2822, (zastępująca RFC 822, do którego odwołuje się RFC 2045) jako:</span><span class="sxs-lookup"><span data-stu-id="e5e3d-356">where `msg-id` is defined in RFC 2822 (that supersedes RFC 822, referenced in RFC 2045) as:</span></span>

```
msg-id    =       [CFWS] "<" id-left "@" id-right ">" [CFWS]
```

<span data-ttu-id="e5e3d-357">i znajduje się skutecznie adres e-mail w ciągu "\<" i ">".</span><span class="sxs-lookup"><span data-stu-id="e5e3d-357">and is effectively an email address enclosed within "\<" and  ">".</span></span> <span data-ttu-id="e5e3d-358">`[CFWS]` Prefiksem i sufiksem zostały dodane w RFC 2822 do przenoszenia komentarze i nie należy używać, aby zachować współdziałania.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-358">The `[CFWS]` prefix and suffix were added in RFC 2822 to carry comments and should not be used to preserve interoperability.</span></span>

<span data-ttu-id="e5e3d-359">R4143: Wartość nagłówka Content-ID dla części MIME zestaw informacji należy wykonać `msg-id` produkcji z RFC 2822 z `[CFWS]` prefiksem i sufiksem części pominięty.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-359">R4143: The value of the Content-ID header for the Infoset MIME part must follow `msg-id` production from RFC 2822 with the `[CFWS]` prefix and suffix parts omitted.</span></span>

<span data-ttu-id="e5e3d-360">Wiele implementacji MIME złagodzone wymagania dla wartości ujęte w "\<" i ">" jako adresu e-mail i użyć `absoluteURI` ujęty w znaki "\<", ">" oprócz adres e-mail.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-360">A number of MIME implementations relaxed requirements for the value enclosed within "\<" and ">" to be an email address and used `absoluteURI` enclosed in "\<" , ">" in addition to the email address.</span></span> <span data-ttu-id="e5e3d-361">Ta wersja programu WCF używa wartości nagłówek MIME Content-ID w postaci:</span><span class="sxs-lookup"><span data-stu-id="e5e3d-361">This version of WCF uses values of the Content-ID MIME header of the form:</span></span>

```
Content-ID: <http://tempuri.org/0> 
```

<span data-ttu-id="e5e3d-362">R4144: Procesory MTOM powinna obsługiwać nagłówek Content-ID wartości, które pasują do następujących złagodzone `msg-id`.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-362">R4144: MTOM processors should accept Content-ID header values that match the following relaxed `msg-id`.</span></span>
...
<span data-ttu-id="e5e3d-363">MIME (RFC 2045) zawiera nagłówek kodowanie transferu zawartości do komunikowania się kodowania zawartości części MIME.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-363">MIME (RFC 2045) provides the Content-Transfer-Encoding header to communicate encoding of the content of the MIME part.</span></span> <span data-ttu-id="e5e3d-364">Domyślnie zdefiniowane dla kodowanie transferu zawartości jest 7-bitowym, który nie jest odpowiedni dla większości komunikaty protokołu SOAP, więc kodowanie transferu zawartości nagłówka wymagane jest współdziałanie większa:</span><span class="sxs-lookup"><span data-stu-id="e5e3d-364">The default defined for Content-Transfer-Encoding is 7-bit, which is not suitable for most SOAP messages, so the Content-Transfer-Encoding header is needed for greater interoperability:</span></span>

- <span data-ttu-id="e5e3d-365">R4145: Część zestaw informacji protokołu SOAP musi zawierać nagłówek kodowanie transferu zawartości.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-365">R4145: The SOAP Infoset part must contain the Content-Transfer-Encoding header.</span></span>

- <span data-ttu-id="e5e3d-366">R4146: W przypadku koperty protokołu SOAP kodowania znaków UTF-8, wartość nagłówka kodowanie transferu zawartości musi być 8-bitowych.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-366">R4146: If the SOAP Envelope character encoding is UTF-8, the value of the Content-Transfer-Encoding header must be 8-bit.</span></span>

- <span data-ttu-id="e5e3d-367">R4147: W przypadku koperty protokołu SOAP kodowania znaków UTF-16, wartość nagłówka kodowanie transferu zawartości musi być binarny.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-367">R4147: If the SOAP Envelope character encoding is UTF-16, the value of the Content-Transfer-Encoding header must be binary.</span></span>

- <span data-ttu-id="e5e3d-368">Zgodnie z [XOP] części 5,</span><span class="sxs-lookup"><span data-stu-id="e5e3d-368">According to [XOP] section 5,</span></span>

- <span data-ttu-id="e5e3d-369">R4148: Zestaw informacji SOAP1.1 część może zawierać nagłówek Content-Type, typ nośnika application/xop + xml i parametry typu = "text/xml" i zestaw znaków</span><span class="sxs-lookup"><span data-stu-id="e5e3d-369">R4148: SOAP1.1 Infoset part must contain Content-Type header with media type application/xop+xml and parameters type="text/xml" and charset</span></span>
...
- <span data-ttu-id="e5e3d-370">R4149: Część protokołu SOAP 1.2 w jedno odniesienie musi zawierać nagłówek Content-Type z typem nośnika `application/xop+xml` i parametry type = "`application/soap+xml`" i `charset`.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-370">R4149: The SOAP 1.2 Infoset part must contain the Content-Type header with media type `application/xop+xml` and parameters type="`application/soap+xml`" and `charset`.</span></span>
    ...
    ```
     ...
     <span data-ttu-id="e5e3d-371">Gdy definiuje XOP `charset` parametr `application/xop+xml` na opcjonalny wymagana jest podobne do wymagań BP 1.1 na współdziałanie `charset` parametr `text/xml` typu nośnika.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-371">While XOP defines the `charset` parameter for `application/xop+xml` to be optional, it is needed for interoperability similar to the BP 1.1 requirement on the `charset` parameter for the `text/xml` media type.</span></span>

- <span data-ttu-id="e5e3d-372">R41410: `type` i `charset` parametry muszą być obecne w nagłówku Content-Type części zestaw informacji 1.x protokołu SOAP.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-372">R41410: The `type` and `charset` parameters must be present on the Content-Type header of the SOAP 1.x Infoset part.</span></span>

#### <a name="wcf-endpoint-support-for-mtom"></a><span data-ttu-id="e5e3d-373">Obsługa punktu końcowego WCF MTOM</span><span class="sxs-lookup"><span data-stu-id="e5e3d-373">WCF Endpoint Support for MTOM</span></span>
<span data-ttu-id="e5e3d-374">MTOM ma na celu zakodowania komunikatu protokołu SOAP, aby zoptymalizować dane zakodowane w formacie base64.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-374">The purpose of MTOM is to encode a SOAP message to optimize base64-encoded data.</span></span> <span data-ttu-id="e5e3d-375">Oto lista ograniczeń:</span><span class="sxs-lookup"><span data-stu-id="e5e3d-375">The following is a list of constraints:</span></span>

- <span data-ttu-id="e5e3d-376">R4151: Może być zoptymalizowany dowolny element informacji elementu, który zawiera dane zakodowane w formacie base64.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-376">R4151: Any element information item that contains base64-encoded data may be optimized.</span></span>

- <span data-ttu-id="e5e3d-377">B4152: WCF optymalizuje pozycje informacji elementów, które zawierają dane zakodowane w formacie base64 i przekracza 1024 bajty długości.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-377">B4152: WCF optimizes element information items that contain base64-encoded data and exceed 1024 bytes in length.</span></span>

<span data-ttu-id="e5e3d-378">Punkt końcowy usługi WCF skonfigurowany do używania MTOM będą zawsze wysyłały zakodowane w formacie MTOM wiadomości.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-378">A WCF endpoint configured to use MTOM will always send MTOM-encoded messages.</span></span> <span data-ttu-id="e5e3d-379">Nawet jeśli nie części spełnia wymagane kryteria, komunikat jest nadal MTOM zakodowane w formacie (zserializowanym w formacie MIME pakietu przy użyciu jednej części MIME zawierający koperty protokołu SOAP).</span><span class="sxs-lookup"><span data-stu-id="e5e3d-379">Even if no parts meet the required criteria, the message is still MTOM-encoded (serialized as a MIME package with a single MIME part containing the SOAP envelope).</span></span>

### <a name="ws-policy-assertion-for-mtom"></a><span data-ttu-id="e5e3d-380">Potwierdzenie WS-Policy dla MTOM</span><span class="sxs-lookup"><span data-stu-id="e5e3d-380">WS-Policy Assertion for MTOM</span></span>
<span data-ttu-id="e5e3d-381">Usługi WCF używa następujących asercji zasad, aby wskazać MTOM użycia przez punkt końcowy:</span><span class="sxs-lookup"><span data-stu-id="e5e3d-381">WCF uses the following policy assertion to indicate MTOM usage by endpoint:</span></span>

```xml
<wsoma:OptimizedMimeSerialization ... />
```

- <span data-ttu-id="e5e3d-382">R4211: Poprzedniego asercję zasad ma podmiotu zasad punktu końcowego i określa, że wszystkie komunikaty wysyłane do i odbierane z punktu końcowego musi być zoptymalizowany przy użyciu MTOM.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-382">R4211: The preceding policy assertion has an Endpoint Policy Subject and specifies that all messages sent to and received from the endpoint must be optimized using MTOM.</span></span>

- <span data-ttu-id="e5e3d-383">B4212: W przypadku skonfigurowania, aby używać optymalizacji MTOM, punkt końcowy usługi WCF dodaje potwierdzenie zasad MTOM do zasady dołączone do odpowiednich `wsdl:binding`.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-383">B4212: When configured to use MTOM optimization, an WCF endpoint adds an MTOM Policy assertion to the policy attached to the corresponding `wsdl:binding`.</span></span>

### <a name="composition-with-ws-security"></a><span data-ttu-id="e5e3d-384">Kompozycja za pomocą usługi WS-Security</span><span class="sxs-lookup"><span data-stu-id="e5e3d-384">Composition with WS-Security</span></span>
<span data-ttu-id="e5e3d-385">MTOM jest mechanizm kodowania, która jest podobna do `text/xml` i WCF binarny kod XML.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-385">MTOM is an encoding mechanism that is similar to `text/xml` and WCF Binary XML.</span></span> <span data-ttu-id="e5e3d-386">MTOM oferuje kompozycji fizyczną za pomocą usługi WS-Security i inne usługi WS-\* protokoły: komunikat zabezpieczone przy użyciu usługi WS-Security można zoptymalizować za pomocą MTOM.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-386">MTOM offers natural composition with WS-Security and other WS-\* protocols: a message secured using WS-Security can be optimized using MTOM.</span></span>

### <a name="examples"></a><span data-ttu-id="e5e3d-387">Przykłady</span><span class="sxs-lookup"><span data-stu-id="e5e3d-387">Examples</span></span>

#### <a name="wcf-soap-11-message-encoded-using-mtom"></a><span data-ttu-id="e5e3d-388">Usługi WCF SOAP 1.1 komunikat zakodowane przy użyciu MTOM</span><span class="sxs-lookup"><span data-stu-id="e5e3d-388">WCF SOAP 1.1 Message Encoded Using MTOM</span></span>
...
#### <a name="wcf-secure-soap-12-message-encoded-using-mtom"></a><span data-ttu-id="e5e3d-389">Usługi WCF bezpiecznego protokołu SOAP 1.2 komunikat zakodowane przy użyciu MTOM</span><span class="sxs-lookup"><span data-stu-id="e5e3d-389">WCF Secure SOAP 1.2 Message Encoded Using MTOM</span></span>
<span data-ttu-id="e5e3d-390">W tym przykładzie wiadomość jest zakodowany przy użyciu MTOM i SOAP 1.2, który jest chroniony przy użyciu usługi WS-Security.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-390">In this example, a message is encoded using MTOM and SOAP 1.2 that is protected using WS-Security.</span></span> <span data-ttu-id="e5e3d-391">Binarny części zidentyfikowane dla kodowania zawartości `BinarySecurityToken`, `CipherValue` z `EncryptedData` odpowiadający zaszyfrowanych podpisu i treść zaszyfrowanego.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-391">The binary parts identified for encoding are the contents of the `BinarySecurityToken`, `CipherValue` of the `EncryptedData` corresponding to the encrypted signature and encrypted body.</span></span> <span data-ttu-id="e5e3d-392">Należy pamiętać, że `CipherValue` z `EncryptedKey` nie został zidentyfikowany na optymalizację przez architekturę WCF, ponieważ jego długość jest mniejsza następnie 1024 bajty.</span><span class="sxs-lookup"><span data-stu-id="e5e3d-392">Note that the `CipherValue` of the `EncryptedKey` was not identified for optimization by WCF, because its length is less then 1024 bytes.</span></span>
