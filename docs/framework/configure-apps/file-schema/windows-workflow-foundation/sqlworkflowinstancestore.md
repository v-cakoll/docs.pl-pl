...
|instanceEncodingOption|Opcjonalna wartość, która określa, czy informacje o stanie wystąpienie jest skompresowany za pomocą algorytmu GZip, zanim informacje są zapisywane w magazynie w trwałości... Ta wartość jest typu <xref:System.Activities.DurableInstancing.InstanceEncodingOption>. Możliwe wartości dla tej właściwości to <xref:System.Activities.DurableInstancing.InstanceEncodingOption.None>, który określa brak kompresji i <xref:System.Activities.DurableInstancing.InstanceEncodingOption.GZip>, która określa, czy dane wystąpienia jest skompresowany i za pomocą algorytmu gzip.|  
