---
title: Błąd kompilatora CS1726
ms.date: 07/20/2015
f1_keywords:
- CS1726
helpviewer_keywords:
- CS1726
ms.assetid: 02b71f22-72f5-42b5-bc9e-1d5dc480cce0
ms.openlocfilehash: ae743593939bae038747b884aeb9fe6360a4b3cd
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
ms.locfileid: "33213275"
---
# <a name="compiler-error-cs1726"></a>Błąd kompilatora CS1726
Odwołanie do przyjaznego zestawu "reference" jest nieprawidłowe. Zestawy podpisane silnymi nazwami muszą określać klucz publiczny w swoich deklaracjach InternalsVisibleTo.  
  
 Podpisany silnie zestaw możne udzielać dostępu przyjaźnie zestawom dotępnym za pomocą atrybutu <xref:System.Runtime.CompilerServices.InternalsVisibleToAttribute>.  
  
 Aby rozwiązać CS1726, możesz nadać silną nazwę zestawowi, do którego chcesz udzielić dostępu przyjaznego lub nie udzielić dostępu przyjaznego.  
  
 Aby uzyskać więcej informacji, zobacz [przyjazne zestawy](../../programming-guide/concepts/assemblies-gac/friend-assemblies.md).  
  
## <a name="example"></a>Przykład  
 Poniższy przykład generuje CS1726.  
  
```csharp  
// Save this code as CS1726.cs  
  
// Run the following command to create CS1726.key:  
//      sn -k CS1726.key  
  
// Then compile by using the following command:   
//      csc /keyfile:CS1726.key /target:library CS1726.cs  
  
using System.Runtime.CompilerServices;  
  
// The following line causes compiler error CS1726.  
[assembly: InternalsVisibleTo("UnsignedAssembly")]     
  
// To get rid of the error, try the following line instead.  
//[assembly: InternalsVisibleTo("SignedAssembly, PublicKey=")]  
  
class A { }  
```  
  
## <a name="see-also"></a>Zobacz też  
 [Instrukcje: tworzenie oznaczonych przyjaznych zestawów](../../programming-guide/concepts/assemblies-gac/how-to-create-signed-friend-assemblies.md)
