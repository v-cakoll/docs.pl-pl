...
|[Asercje](assertions.md)|Opisuje wyrażenie `assert`, które jest funkcją debugowania, której można użyć do przetestowania wyrażenia. W przypadku niepowodzenia w trybie debugowania potwierdzenie generuje okno dialogowe błędu systemu.|
...
|[Deklaracje importu: słowo kluczowe `open`](import-declarations-the-open-keyword.md)|Opisuje sposób działania `open`. Deklaracja importu określa moduł lub przestrzeń nazw, których elementy można odwoływać bez używania w pełni kwalifikowanej nazwy.|
|[Podpisy](signature-files.md)|Opisuje sygnatury i pliki sygnatur. Plik podpisu zawiera informacje o podpisach publicznych zestawu elementów F# programu, takich jak typy, przestrzenie nazw i moduły. Może służyć do określania dostępności tych elementów programu.|
...
|[Pełna składnia](verbose-syntax.md)|Opisuje składnię dla F# konstrukcji, gdy nie jest włączona składnia uproszczona. Składnia verbose jest wskazywana przez dyrektywę `#light "off"` w górnej części pliku kodu.|
...
|[Typ jednostki](unit-type.md)|Opisuje typ `unit`, który jest typem, który wskazuje brak określonej wartości; Typ `unit` ma tylko jedną wartość, która działa jako symbol zastępczy, gdy inna wartość nie istnieje lub nie jest wymagana.|
|[Ciągi](strings.md)|Opisuje ciągi w F#. Typ `string` reprezentuje niezmienny tekst, jako sekwencję znaków Unicode. `string` jest aliasem `System.String` w .NET Framework.|
...
|Tytuł|Opis|
|-----|-----------|
|[Wyrażenia warunkowe: `if...then...else`](conditional-expressions-if-then-else.md)|Opisuje wyrażenie `if...then...else`, które uruchamia różne gałęzie kodu, a także oblicza inną wartość w zależności od danego wyrażenia logicznego.|
|[Wyrażenia dopasowania](match-expressions.md)|Opisuje wyrażenie `match`, które zapewnia sterowanie rozgałęzieniem oparte na porównaniu z zestawem wzorców.|
|[Pętle: wyrażenie `for...to`](loops-for-to-expression.md)|Opisuje wyrażenie `for...to`, które jest używane do iteracji w pętli względem zakresu wartości zmiennej pętli.|
|[Pętle: wyrażenie `for...in`](loops-for-in-expression.md)|Opisuje wyrażenie `for...in`, konstrukcja pętli, która jest używana do iteracji na dopasowaniach wzorca w wyliczalnej kolekcji, takiej jak wyrażenie zakresu, sekwencja, lista, tablica lub inna konstrukcja, która obsługuje Wyliczenie.|
|[Pętle: wyrażenie `while...do`](loops-while-do-expression.md)|Opisuje wyrażenie `while...do`, które jest używane do wykonywania iteracji wykonywania (zapętlanie), podczas gdy określony warunek testu ma wartość true.|
