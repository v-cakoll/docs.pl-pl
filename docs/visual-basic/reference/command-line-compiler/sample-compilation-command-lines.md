...
Kompilator wiersza polecenia Visual Basic obsługuje kompletny zestaw opcji kontrolujących pliki wejściowe i wyjściowe, zestawy oraz opcje debugowania i preprocesora. Każda opcja jest dostępna w dwóch niezmienionych formularzach: `-option` i `/option`. Ta dokumentacja zawiera tylko formularz `-option`.
...
|Kompiluj wszystkie pliki Visual Basic w bieżącym katalogu z optymalizacjami na i zdefiniowanym symbolem `DEBUG`, wytwarzając plik2. exe|`vbc -define:DEBUG=1 -optimize -out:File2.exe *.vb`|
