---
title: 5603 — FederationMessageCreationSuccess
ms.date: 03/30/2017
ms.assetid: b55abdf4-cbe1-401e-b23e-f2bf1bcca719
ms.openlocfilehash: 631d79c4df182082d1b829b0708d913e0a44eaae
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
ms.locfileid: "33468963"
---
# <a name="5603---federationmessagecreationsuccess"></a>5603 — FederationMessageCreationSuccess
## <a name="properties"></a>Właściwości  
  
|||  
|-|-|  
|ID|5603|  
|Słowa kluczowe|Zabezpieczenia|  
|Poziom|Pełny|  
|Kanał|Microsoft-Windows aplikacji debugowania serwera — aplikacje|  
  
## <a name="description"></a>Opis  
 To zdarzenie jest emitowany, gdy Tworzenie komunikatu federacyjnego z post formularza zakończyło się pomyślnie.  
  
## <a name="message"></a>Komunikat  
 Tworzenie komunikatu federacyjnego na podstawie post formularza zakończyło się pomyślnie.  
  
## <a name="details"></a>Szczegóły
