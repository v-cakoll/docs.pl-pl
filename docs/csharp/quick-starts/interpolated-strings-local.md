...
Następnie tworzymy wystąpienie `Vegetable` przy użyciu [ `new` — słowo kluczowe](../language-reference/keywords/new-operator.md) i podając parametr name dla konstruktora `Vegetable`:
...
Ukończono Szybki Start interpolacji ciągu.

Możesz kontynuować [listy kolekcji](arrays-and-collections.md) Szybki Start w środowisku projektowym.
