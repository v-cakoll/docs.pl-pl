---
title: Biblioteka działań
ms.date: 03/30/2017
ms.assetid: 5323e9d4-71d6-47eb-bfa6-31feac62044d
ms.openlocfilehash: 29b3efd648ac103526f572a2cac5b8b67c4b220a
ms.sourcegitcommit: c7f3e2e9d6ead6cc3acd0d66b10a251d0c66e59d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 09/09/2018
ms.locfileid: "44252424"
---
# <a name="activity-library"></a>Biblioteka działań
Ta sekcja zawiera przykłady pokazujące, zaawansowanych niestandardowych działań w Windows Workflow Foundation (WF).  
  
## <a name="in-this-section"></a>W tej sekcji  
 [Działanie zasad w programie .NET Framework 4.5](../../../../docs/framework/windows-workflow-foundation/samples/policy-activity-in-net-framework-4-5.md)  
 Pokazuje, jak działanie Policy4 zezwala na Windows Workflow Foundation w [!INCLUDE[netfx35_long](../../../../includes/netfx35-long-md.md)] (WF 3.5) <xref:System.Workflow.Activities.Rules.RuleSet> obiektów, które ma być używany w programie Windows Workflow Foundation w [!INCLUDE[netfx_current_long](../../../../includes/netfx-current-long-md.md)] (WF 4.5) bezpośrednio przy użyciu aparatu reguł, który jest dostarczany w WF 3.5.  
  
 [Działanie niestandardowe do przełączania na zakres wartości](../../../../docs/framework/windows-workflow-foundation/samples/custom-activity-to-switch-on-a-range-of-values.md)  
 Pokazuje, jak utworzyć niestandardowe działanie, które rozszerza użytkowania <xref:System.Activities.Statements.Switch%601>.  
  
 [Działanie LINQ to Objects](../../../../docs/framework/windows-workflow-foundation/samples/linq-to-objects-activity.md)  
 Pokazuje, jak utworzyć działanie na potrzeby LINQ to Objects elementy zapytania w kolekcji.  
  
 [Przykład LINQ to SQL](../../../../docs/framework/windows-workflow-foundation/samples/linq-to-sql-sample.md)  
 Pokazuje, jak utworzyć działanie używać programu LINQ do jednostek zapytania SQL z tabel bazy danych programu SQL Server.  
  
 [Używanie działania InvokePowerShell](../../../../docs/framework/windows-workflow-foundation/samples/using-the-invokepowershell-activity.md)  
 Przedstawia sposób wywołania poleceń programu Windows PowerShell przy użyciu działania InvokePowerShell.  
  
 [Działanie RangeEnumeration](../../../../docs/framework/windows-workflow-foundation/samples/rangeenumeration-activity.md)  
 Pokazuje, jak utworzyć niestandardowe działanie, który iteruje po kolekcji liczb.  
  
 [Regularne wyrażenia działań](../../../../docs/framework/windows-workflow-foundation/samples/regular-expression-activities.md)  
 Pokazuje, jak utworzyć zestaw działań, które udostępniają funkcje wyrażenia regularnego <xref:System.Text.RegularExpressions> przestrzeni nazw.  
  
 [Niestandardowe działanie SendMail](../../../../docs/framework/windows-workflow-foundation/samples/sendmail-custom-activity.md)  
 Pokazuje, jak utworzyć niestandardowe działanie, która pochodzi od klasy <xref:System.Activities.AsyncCodeActivity> do wysyłania wiadomości e-mail przy użyciu protokołu SMTP do użycia w aplikacji przepływu pracy.  
  
 [Działanie For](../../../../docs/framework/windows-workflow-foundation/samples/for-activity.md)  
 Pokazuje, jak utworzyć niestandardowe działanie, która dziedziczy <xref:System.Activities.NativeActivity> i korzystać z niego w przepływie pracy Iterowanie zakresu wartości.  
  
 [Działanie WaitForInput](../../../../docs/framework/windows-workflow-foundation/samples/wait-for-input-activity.md)  
 Pokazuje, jak utworzyć nazwany zakładki w przepływie pracy.  
  
 [Działanie ThrottledParallelForEach](../../../../docs/framework/windows-workflow-foundation/samples/throttled-parallel-foreach.md)  
 Pokazuje, jak `ThrottleParallelForEach` działanie jest podobne do <xref:System.Activities.Statements.ParallelForEach%601> działania z jednym wyjątkiem że umożliwia ustawienie współczynnika współbieżności Aby ograniczyć liczbę jednoczesnych gałęzi do wykonania.  
  
 [Działania jednostki](../../../../docs/framework/windows-workflow-foundation/samples/entity-activities.md)  
 Pokazuje sposób użycia ADO.NET Entity Framework za pomocą programu Windows Workflow Foundation, aby uprościć dostęp do danych.  
  
 [Działania dostępu do bazy danych](../../../../docs/framework/windows-workflow-foundation/samples/database-access-activities.md)  
 Pokazuje, jak utworzyć działania, które umożliwiają uzyskiwanie dostępu do bazy danych do pobierania lub modyfikowanie informacji i użycia [ADO.NET](https://go.microsoft.com/fwlink/?LinkId=166081) dostęp do bazy danych.  
  
 [Działanie CommentOut](../../../../docs/framework/windows-workflow-foundation/samples/commentout-activity.md)  
 Pokazuje, jak pisać niestandardowe działanie, które powoduje usunięcie innych działań ze ścieżki wykonywania i efektywne Dodawanie komentarza do ich.  
  
 [Działanie zasad zewnętrznych w programie .NET Framework 4.5](../../../../docs/framework/windows-workflow-foundation/samples/externalized-policy-activity-in-net-framework-4-5.md)  
 Pokazuje, jak działanie ExternalizedPolicy4 zezwala na wykonywanie istniejącej Windows Workflow Foundation w [!INCLUDE[netfx35_long](../../../../includes/netfx35-long-md.md)] (WF 3.5) <xref:System.Workflow.Activities.Rules.RuleSet> obiektów w programie Windows Workflow Foundation w [!INCLUDE[netfx_current_long](../../../../includes/netfx-current-long-md.md)] (WF 4.5) bezpośrednio za pomocą aparatu reguł to znaczy wydane w wersji 3.5 WF.  
  
 [Działanie NoPersistScope](../../../../docs/framework/windows-workflow-foundation/samples/nopersistscope-activity.md)  
 Pokazuje, jak do manipulowania nieprzeznaczone i możliwe do rozporządzania stan w przepływie pracy.  
  
 [Nieogólne działanie ForEach](../../../../docs/framework/windows-workflow-foundation/samples/non-generic-foreach.md)  
 Pokazuje, jak utworzyć wersję inną niż ogólna <xref:System.Activities.Statements.ForEach%601> działania.  
  
 [Nieogólne działanie ParallelForEach](../../../../docs/framework/windows-workflow-foundation/samples/non-generic-parallelforeach.md)  
 Pokazuje, jak utworzyć wersję inną niż ogólna <xref:System.Activities.Statements.ParallelForEach%601> działania.  
  
 [Działanie GetWorkflowInstanceId](../../../../docs/framework/windows-workflow-foundation/samples/get-workflowinstanceid.md)  
 Pokazuje, jak użyć niestandardowego działania, `GetWorkflowInstanceId`, aby zwrócić identyfikator wystąpienia przepływu pracy
