...
##### <a name="test-setup"></a>Testuj ustawienia  
 W tym scenariuszu komputer kliencki kontaktuje się z usług przepływu pracy WCF dzięki wykorzystaniu korelacja oparta na kontekście.  Kontekst korelacji wymaga specjalnych kontekst powiązania i używa nagłówka kontekstu lub plik cookie do powiązania komunikatów dla wystąpienia przepływu pracy poprawne.  Ma ona wydajności korzyści w tym korelacji, której identyfikator znajduje się w nagłówku komunikatu, więc treść komunikatu musi zostać przeanalizowany. Aby uzyskać więcej informacji na temat kontekstu korelacji zobacz [korelacja wymiany kontekstu](../../../docs/framework/wcf/feature-details/context-exchange-correlation.md)  
