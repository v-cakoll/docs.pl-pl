...
## <a name="xml-namespaces"></a>Przestrzenie nazw XML  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]Umożliwia określenie aliasu globalnej przestrzeni nazw XML przy użyciu `Imports` instrukcji. Poniższy przykład przedstawia użycie `Imports` instrukcji do zaimportowania do przestrzeni nazw XML:  
...
### <a name="using-xml-namespaces-in-xml-literals"></a>Używanie przestrzeni nazw XML w literałach XML  
 ...
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] Kompilatora tłumaczy literałów XML, zawierające aliasów przestrzeni nazw XML na równoważne kod, który używa notacji XML przy użyciu przestrzeni nazw XML, z `xmlns` atrybutu. Podczas kompilacji, kod w przykładzie w poprzedniej sekcji tworzy zasadniczo tego samego kodu wykonywalnego jak w poniższym przykładzie:  
