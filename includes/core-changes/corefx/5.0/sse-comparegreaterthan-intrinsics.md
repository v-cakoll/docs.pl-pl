...
#### <a name="change-description"></a><span data-ttu-id="1c3e6-103">Zmień opis</span><span class="sxs-lookup"><span data-stu-id="1c3e6-103">Change description</span></span>

<span data-ttu-id="1c3e6-104">Wcześniej `NaN` dane wejściowe na liście <xref:System.Runtime.Intrinsics.X86.Sse> i <xref:System.Runtime.Intrinsics.X86.Sse2> metody zwróciły niepoprawny wynik.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-104">Previously, `NaN` inputs to the listed <xref:System.Runtime.Intrinsics.X86.Sse> and <xref:System.Runtime.Intrinsics.X86.Sse2> methods returned an incorrect result.</span></span> <span data-ttu-id="1c3e6-105">Wynik również różni się od wyniku wygenerowanego przez odpowiadającą metodę w <xref:System.Runtime.Intrinsics.X86.Avx> klasie.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-105">The result also differed from the result generated by the corresponding method in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="1c3e6-106">Począwszy od platformy .NET 5,0, te metody poprawnie `NaN` obsługują dane wejściowe i zwracają te same wyniki, co odpowiednie metody <xref:System.Runtime.Intrinsics.X86.Avx> w klasie.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-106">Starting in .NET 5.0, these methods correctly handle `NaN` inputs and return the same results as the corresponding methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="1c3e6-107">Streaming SIMD Extensions (SSE) i Streaming SIMD Extensions 2 (SSE2) Industry Architecture (ISAs) nie zapewniają bezpośredniej obsługi sprzętu dla tych metod porównywania, dzięki czemu są one zaimplementowane w oprogramowaniu.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-107">The Streaming SIMD Extensions (SSE) and Streaming SIMD Extensions 2 (SSE2) industry standard architectures (ISAs) don't provide direct hardware support for these comparison methods, so they're implemented in software.</span></span> <span data-ttu-id="1c3e6-108">Wcześniej metody były niepoprawnie zaimplementowane i nieprawidłowo obsługiwały `NaN` dane wejściowe.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-108">Previously, the methods were improperly implemented, and they incorrectly handled `NaN` inputs.</span></span> <span data-ttu-id="1c3e6-109">W przypadku kodu portowego z macierzystego nieprawidłowe zachowanie może spowodować błędy.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-109">For code ported from native, the incorrect behavior may introduce bugs.</span></span> <span data-ttu-id="1c3e6-110">W przypadku ścieżki kodu 256-bitowego metody mogą również generować różne wyniki do równoważnych metod w <xref:System.Runtime.Intrinsics.X86.Avx> klasie.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-110">For a 256-bit code path, the methods can also produce different results to the equivalent methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="1c3e6-111">Przykład sposobu, w jaki metody były wcześniej nieprawidłowe, można zaimplementować `CompareNotGreaterThan(x,y)` jako `CompareLessThanOrEqual(x,y)` dla zwykłych liczb całkowitych.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-111">As an example of how the methods were previously incorrect, you can implement `CompareNotGreaterThan(x,y)` as `CompareLessThanOrEqual(x,y)` for regular integers.</span></span> <span data-ttu-id="1c3e6-112">Jednak w przypadku `NaN` danych wejściowych logika oblicza niewłaściwy wynik.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-112">However, for `NaN` inputs, that logic computes the wrong result.</span></span> <span data-ttu-id="1c3e6-113">Zamiast tego, `CompareNotLessThan(y,x)` używając porównuje cyfry *and* poprawnie i `NaN` pobiera dane wejściowe.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-113">Instead, using `CompareNotLessThan(y,x)` compares the numbers correctly *and* takes `NaN` inputs into consideration.</span></span>

#### <a name="version-introduced"></a><span data-ttu-id="1c3e6-114">Wprowadzona wersja</span><span class="sxs-lookup"><span data-stu-id="1c3e6-114">Version introduced</span></span>

<span data-ttu-id="1c3e6-115">5,0 wersja zapoznawcza 4</span><span class="sxs-lookup"><span data-stu-id="1c3e6-115">5.0 Preview 4</span></span>

#### <a name="recommended-action"></a><span data-ttu-id="1c3e6-116">Zalecana akcja</span><span class="sxs-lookup"><span data-stu-id="1c3e6-116">Recommended action</span></span>

- <span data-ttu-id="1c3e6-117">Jeśli poprzednie zachowanie była usterką, nie jest wymagana żadna zmiana.</span><span class="sxs-lookup"><span data-stu-id="1c3e6-117">If the previous behavior was a bug, no change is required.</span></span>

- <span data-ttu-id="1c3e6-118">Jeśli poprzednie zachowanie było wymagane, można zachować takie zachowanie, zmieniając odpowiednie wywołanie w następujący sposób:</span><span class="sxs-lookup"><span data-stu-id="1c3e6-118">If the previous behavior was desired, you can maintain that behavior by changing the relevant invocation as follows:</span></span>
...
#### <a name="category"></a><span data-ttu-id="1c3e6-119">Kategoria</span><span class="sxs-lookup"><span data-stu-id="1c3e6-119">Category</span></span>

<span data-ttu-id="1c3e6-120">Podstawowe biblioteki platformy .NET</span><span class="sxs-lookup"><span data-stu-id="1c3e6-120">Core .NET libraries</span></span>

#### <a name="affected-apis"></a><span data-ttu-id="1c3e6-121">Dotyczy interfejsów API</span><span class="sxs-lookup"><span data-stu-id="1c3e6-121">Affected APIs</span></span>
