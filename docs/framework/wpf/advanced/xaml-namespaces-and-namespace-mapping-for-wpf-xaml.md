---
title: Przestrzenie nazw XAML i mapowanie przestrzeni nazw
description: Dowiedz się więcej o obecności i przeznaczeniu dwóch mapowań przestrzeni nazw XAML często znalezionych w tagu głównym Windows Presentation Foundation pliku XAML.
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
## <a name="what-is-a-xaml-namespace"></a>Co to jest przestrzeń nazw XAML?  
 Przestrzeń nazw XAML jest w rzeczywistości rozszerzeniem koncepcji przestrzeni nazw XML. Techniki określania przestrzeni nazw XAML opierają się na składni przestrzeni nazw XML, Konwencji o użyciu identyfikatorów URI jako identyfikatorów przestrzeni nazw, przy użyciu prefiksów, aby umożliwić odwoływanie się do wielu przestrzeni nazw z tego samego źródła znaczników i tak dalej. Koncepcja podstawowa, która jest dodawana do definicji XAML przestrzeni nazw XML, polega na tym, że przestrzeń nazw XAML wymaga zarówno zakresu unikatowości użycia znaczników, jak i ma wpływ na sposób, w jaki jednostki znaczników są potencjalnie obsługiwane przez określone przestrzenie nazw CLR i przywoływane zestawy. Ten ostatni nacisk ma wpływ na koncepcję kontekstu schematu XAML. Jednak w celach, w których WPF współpracuje z przestrzeniami nazw XAML, można ogólnie traktować przestrzenie nazw XAML pod względem domyślnej przestrzeni nazw XAML, przestrzeni nazw języka XAML oraz wszelkich innych przestrzeni nazw XAML, które są mapowane przez znaczniki języka XAML bezpośrednio do określonych kopii zapasowych przestrzeni nazw środowiska CLR i przywoływanych zestawów.  
...
## <a name="the-wpf-and-xaml-namespace-declarations"></a>Deklaracje przestrzeni nazw WPF i XAML  
 ...
 Relacja między tymi deklaracjami polega na tym, że `x:` Mapowanie prefiksu obsługuje funkcje wewnętrzne, które są częścią definicji języka XAML, i [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] jest jedną implementacją, która używa języka XAML jako język i definiuje słownictwo obiektów dla języka XAML. Ze względu na to, że użycie słownictwa WPF będzie znacznie bardziej typowe niż użycie elementów wewnętrznych języka XAML, słownictwo WPF jest mapowane jako domyślne.  
  
 `x:`Konwencja prefiksu do mapowania obsługi elementów wewnętrznych języka XAML obejmuje szablony projektów, przykładowy kod i dokumentację funkcji języka w ramach tego zestawu SDK. Przestrzeń nazw XAML definiuje wiele powszechnie używanych funkcji, które są niezbędne nawet dla podstawowych aplikacji WPF. Na przykład w celu dołączenia dowolnego kodu do pliku XAML za pomocą klasy częściowej należy nazwać tę klasę jako `x:Class` atrybut w elemencie głównym odpowiedniego pliku XAML. Lub każdy element zdefiniowany na stronie XAML, do którego chcesz uzyskać dostęp jako zasób z kluczem, powinien mieć `x:Key` atrybut ustawiony dla danego elementu. Aby uzyskać więcej informacji na temat tych i innych aspektów języka XAML, zobacz [Omówienie języka XAML (WPF)](../../../desktop-wpf/fundamentals/xaml.md) lub [składni języka XAML](xaml-syntax-in-detail.md).  
...
```  
 ...
 Ta klasa niestandardowa jest następnie skompilowana do biblioteki, która na ustawienia projektu (niepokazywany) ma nazwę `SDKSampleLibrary` .  
 ...
 Aby wszystkie te elementy umieścić razem, poniżej znajduje się kod XAML, który zawiera niestandardowe mapowanie wraz z typowymi domyślnymi i x: mapowaniami w tagu głównym, a następnie używa prefiksu stałego do utworzenia wystąpienia `ExampleClass` w tym interfejsie użytkownika:  
...
### <a name="mapping-to-current-assemblies"></a>Mapowanie na bieżące zestawy  
 `assembly`można pominąć, jeśli `clr-namespace` odwołanie jest zdefiniowane w tym samym zestawie, co kod aplikacji, który odwołuje się do klas niestandardowych. Lub równoważną składnią w tym przypadku jest określenie `assembly=` , bez tokenu ciągu po znaku równości.  
...
- [Informacje o przestrzeniach nazw XML](https://docs.microsoft.com/previous-versions/aa468565(v=msdn.10))
