...
Jeśli korzystasz z platformy .NET dla Apache Spark, zapoznaj się z samouczkiem wprowadzenie do [platformy .NET dla Apache Spark](get-started.md) , aby dowiedzieć się, jak przygotować środowisko i uruchomić pierwszą aplikację .net for Apache Spark.
...
1. W wierszu polecenia Uruchom następujące polecenia, aby utworzyć nową aplikację konsolową:
   ...
   `dotnet`Polecenie tworzy `new` aplikację typu `console` . `-o`Parametr tworzy katalog o nazwie *mySparkBatchApp* , w którym jest przechowywana aplikacja i wypełnia je wymaganymi plikami. `cd mySparkBatchApp`Polecenie zmienia katalog na właśnie utworzony katalog aplikacji.
...
1. Dodaj następujące dodatkowe `using` instrukcje na początku pliku *program.cs* w *mySparkBatchApp*.
...
1. Dodaj następujący kod wewnątrz metody Main, aby określić nowy SparkSession. SparkSession jest punktem wejścia do programowania platformy Spark za pomocą zestawu danych i interfejsu API Dataframe. Wywołując `spark` obiekt, można uzyskać dostęp do funkcji Spark i Dataframe w całym programie.
...
1. Odczytaj plik wejściowy do `DataFrame` , który jest rozproszoną kolekcją danych zorganizowanych w nazwanych kolumn. Możesz ustawić kolumny dla danych za pomocą programu <xref:Microsoft.Spark.Sql.DataFrame.Schema%2A> . Użyj <xref:Microsoft.Spark.Sql.DataFrame.Show%2A> metody, aby wyświetlić dane w ramce Dataframe. Pamiętaj, aby zaktualizować ścieżkę pliku CSV do lokalizacji pobranych danych usługi GitHub.
...
1. Użyj <xref:Microsoft.Spark.Sql.DataFrame.Na%2A> metody, aby porzucić wiersze z wartościami na (null), i <xref:Microsoft.Spark.Sql.DataFrame.Drop%2A> metodę usuwania niektórych kolumn z danych. Pozwala to zapobiec błędom, jeśli próbujesz analizować dane o wartości null lub kolumny, które nie są istotne dla ostatniej analizy.
...
Można w ten sposób wywołać `spark.Sql` naśladowanie standardowych wywołań SQL widzianych w innych typach aplikacji. Można również wywołać metody, takie jak <xref:Microsoft.Spark.Sql.DataFrame.GroupBy%2A> i <xref:Microsoft.Spark.Sql.DataFrame.Agg%2A> , w celu łączenia, filtrowania i wykonywania obliczeń na danych.
...
1. Wywołanie metody `spark.Stop()` End SparkSession.
