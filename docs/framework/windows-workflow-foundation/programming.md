---
title: Windows Workflow Foundation programowania
ms.custom: 
ms.date: 03/30/2017
ms.prod: .net-framework
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- Windows Workflow Foundation [WF], programming
- programming [WF]
- WF [WF], programming
ms.assetid: f62354e2-6a2b-4e8c-a4bc-533fb96267a4
caps.latest.revision: "16"
author: Erikre
ms.author: erikre
manager: erikre
ms.openlocfilehash: 952f1f16ff610282f2f15c9d78bf3d45d5c354da
ms.sourcegitcommit: bd1ef61f4bb794b25383d3d72e71041a5ced172e
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 10/18/2017
---
# <a name="windows-workflow-foundation-programming"></a>Windows Workflow Foundation programowania
Ta sekcja zawiera zestaw tematów Elementarz, które należy zrozumieć, aby stać się zaawansowany [!INCLUDE[wf](../../../includes/wf-md.md)] programisty.  
  
## <a name="in-this-section"></a>W tej sekcji  
 [Projektowanie przepływów pracy](../../../docs/framework/windows-workflow-foundation/designing-workflows.md)  
 Tematach opisano wzorcami sterowanie przepływem używany do rozwoju przepływu pracy.  
  
 [Używanie i tworzenie działania](../../../docs/framework/windows-workflow-foundation/using-and-creating-activities.md)  
 Tematy, które opisuje działania dostarczane przez system, które muszą być dostępne w [!INCLUDE[wf](../../../includes/wf-md.md)].  
  
 [Model danych podstawy przepływu pracy systemu Windows](../../../docs/framework/windows-workflow-foundation/data-model.md)  
 Tematów opisujących zmiennych, argumenty i wyrażenia w [!INCLUDE[wf1](../../../includes/wf1-md.md)].  
  
 [Trwa oczekiwanie na dane wejściowe w przepływie pracy](../../../docs/framework/windows-workflow-foundation/waiting-for-input-in-a-workflow.md)  
 Tematy, które informacje dotyczące używania zakładek i działania obsługi wiadomości.  
  
 [Wyjątki, transakcje i kompensacji](../../../docs/framework/windows-workflow-foundation/exceptions-transactions-and-compensation.md)  
 Tematy dotyczące sposobu używania programów obsługi wyjątków, transakcje i kompensacji do obsługi błędów czasu wykonywania.  
  
 [Hosting przepływów pracy](../../../docs/framework/windows-workflow-foundation/hosting-workflows.md)  
 Tematach opisano szczegóły dotyczące pisania przepływu pracy aplikacji hosta.  
  
 [Aktualizacja dynamiczna](../../../docs/framework/windows-workflow-foundation/dynamic-update.md)  
 Opisuje sposób używać dynamicznej aktualizacji do aktualizacji definicji przepływu pracy wystąpienia utrwalonego przepływu pracy.  
  
 [Usługi przepływu pracy](../../../docs/framework/wcf/feature-details/workflow-services.md)  
 Tematach opisano model programowania, który obsługuje usługi zapisu deklaratywnie.  
  
 [Trwałość przepływu pracy](../../../docs/framework/windows-workflow-foundation/workflow-persistence.md)  
 Tematach opisano opcje automatycznie lub ręcznie przechowywanie danych przepływu pracy i zwalnianie przepływy pracy z pamięci.  
  
 [Wskazówki dotyczące migracji](../../../docs/framework/windows-workflow-foundation/migration-guidance.md)  
 Tematach opisano sposób migracji przepływy pracy z poprzednich wersji [!INCLUDE[wf](../../../includes/wf-md.md)].  
  
 [Przepływ pracy, kontrola i śledzenie](../../../docs/framework/windows-workflow-foundation/workflow-tracking-and-tracing.md)  
 Tematach opisano przepływ pracy, kontrola i śledzenie oraz jak te funkcje są używane do monitorowania aplikacji przepływu pracy.  
  
 [Zabezpieczenia przepływu pracy](../../../docs/framework/windows-workflow-foundation/workflow-security.md)  
 Omówiono w celu poprawy bezpieczeństwa przepływu pracy przy użyciu języka SQL i [!INCLUDE[indigo1](../../../includes/indigo1-md.md)].  
  
 [Windows Workflow Foundation 4 wydajności](../../../docs/framework/windows-workflow-foundation/performance.md)  
 W tym artykule omówiono [!INCLUDE[wf2](../../../includes/wf2-md.md)] 4 wydajności i porównuje go do poprzedniej wersji programu WF.
