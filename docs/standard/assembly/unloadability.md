...
- Regularne odwołania przechowywane z zewnątrz kolekcjonowanych `AssemblyLoadContext` przechowywanych w gnieździe stosu lub w rejestrze procesora (metody lokalne, jawnie utworzone przez kod użytkownika lub niejawnie przez kompilator just-in-Time (JIT), zmienna statyczna lub silne (Przypinanie) dojście GC, a także przechodnie wskazywanie:
