...
### <a name="runtime-version-information-in-the-application-configuration-file"></a>Informacje o wersji środowiska uruchomieniowego w pliku konfiguracyjnym aplikacji  
 Oprócz informacji w nagłówku pliku PE można wdrożyć aplikację z plikiem konfiguracji aplikacji, który zawiera informacje o wersji środowiska uruchomieniowego. Plik konfiguracji aplikacji jest plikiem opartym na formacie XML, który jest tworzony przez dewelopera aplikacji i dostarczany z aplikacją. [Element requiredRuntime > w sekcji > startowej, jeśli jest obecny w tym pliku, określa wersje środowiska uruchomieniowego i wersje składnika obsługiwane przez aplikację. \<](../../../docs/framework/configure-apps/file-schema/startup/requiredruntime-element.md) [ \<](../../../docs/framework/configure-apps/file-schema/startup/startup-element.md) Możesz również użyć tego pliku do testowania, aby przetestować zgodność aplikacji z różnymi wersjami środowiska uruchomieniowego.  
 ...
 Aby uzyskać więcej informacji na temat plików konfiguracji aplikacji, zobacz [Konfigurowanie aplikacji](../../../docs/framework/configure-apps/index.md).  
...
1. Środowisko uruchomieniowe bada [ \<element supportedRuntime > elementu](../../../docs/framework/configure-apps/file-schema/startup/supportedruntime-element.md) w pliku konfiguracyjnym aplikacji. Jeśli co najmniej jedna obsługiwana wersja środowiska uruchomieniowego określona w  **\<elemencie supportedRuntime >** jest obecna, środowisko uruchomieniowe ładuje wersję środowiska uruchomieniowego określoną przez pierwszy  **\<element supportedRuntime >** . Jeśli ta wersja jest niedostępna, środowisko uruchomieniowe sprawdzi następny  **\<element supportedRuntime >** i podejmie próbę załadowania określonej wersji środowiska uruchomieniowego. Jeśli ta wersja środowiska uruchomieniowego nie jest dostępna, kolejne  **\<supportedRuntime elementy >** są badane. Jeśli żadna z obsługiwanych wersji środowiska uruchomieniowego nie jest dostępna, środowisko uruchomieniowe nie może załadować wersji środowiska uruchomieniowego i wyświetla komunikat dla użytkownika (zobacz krok 3).  
...
## <a name="partially-qualified-assembly-names-and-side-by-side-execution"></a>Częściowo kwalifikowane nazwy zestawów a wykonywanie równoczesne  
 ...
 Aby wyeliminować częściowo kwalifikowane odwołania do zestawów w kodzie, można użyć [ \<elementu qualifyAssembly >](../../../docs/framework/configure-apps/file-schema/runtime/qualifyassembly-element.md) w pliku konfiguracyjnym aplikacji, aby w pełni kwalifikować częściowo kwalifikowane odwołania do zestawów, które występują w kodzie. Użyj elementu  **\<qualifyAssembly >** , aby określić tylko pola, które nie zostały ustawione w odwołaniu częściowym. Tożsamość zestawu wymieniona w atrybucie **FullName** musi zawierać wszystkie informacje niezbędne do pełnej kwalifikacji nazwy zestawu: Nazwa zestawu, klucz publiczny, kultura i wersja.  
...
```  
 ...
 Za każdym razem, gdy następuje `myAssembly`odwołanie do instrukcji ładowania zestawu, te ustawienia pliku konfiguracji powodują, że środowisko `myAssembly` uruchomieniowe automatycznie przetłumaczy częściowo kwalifikowane odwołanie na w pełni kwalifikowane odwołanie. Na przykład zestaw. Load ("Moja Assembly") staną się zestawem. Load ("Moja Assembly, Version = 1.0.0.0, publicKeyToken =..., Culture = neutral").  
...
|Tytuł|Opis|  
|-----------|-----------------|  
|[Instrukcje: Włączanie i wyłączanie automatycznego przekierowywania powiązań](../../../docs/framework/configure-apps/how-to-enable-and-disable-automatic-binding-redirection.md)|Opis sposobu tworzenia powiązania aplikacji z określoną wersją zestawu.|  
