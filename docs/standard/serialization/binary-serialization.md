...
Serializacja binarna pozwala modyfikować prywatne elementy członkowskie wewnątrz obiektu i w związku z tym zmieniać jego stan. Z tego względu zalecane są inne struktury serializacji, takie jak <xref:System.Text.Json?displayProperty=fullName>, które działają na publicznej powierzchni interfejsu API.
...
> | <xref:System.Data.DataTable?displayProperty=nameWithType> | Ustawienie `RemotingFormat` na `SerializationFormat.Binary`może być wymieniane tylko z platformą .NET Core 2,1 i nowszymi wersjami. |
