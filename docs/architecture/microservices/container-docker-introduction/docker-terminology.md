...
**Obraz z obsługą wielodostępności**: W przypadku wielu architektur jest funkcją, która upraszcza wybór odpowiedniego obrazu, zgodnie z platformą, w której działa system Docker, np. gdy pliku dockerfile żąda obrazu podstawowego **z MCR.Microsoft.com/dotnet/Core/SDK:2.2** z rejestru faktycznie otrzymuje **2,2-SDK-nanoserver-1709**, **2,2-SDK-nanoserver-1803**, **2,2-SDK-nanoserver-1809** lub **2,2-SDK-rozciąga**, w zależności od systemu operacyjnego i wersji, w przypadku których uruchomiono platformę Docker.
...
**Azure Container Registry**: Zasób publiczny do pracy z obrazami platformy Docker i jej składnikami na platformie Azure. Zapewnia to rejestr bliski wdrożenia na platformie Azure i zapewnia kontrolę nad dostępem, dzięki czemu można korzystać z Azure Active Directory grup i uprawnień.
...
>[Poprzedni](docker-defined.md)Następny
