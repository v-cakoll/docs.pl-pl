...
```  
 ...
 Każde różne wystąpienie `AppClass<T>` tego typu wymaga oddzielnej dyrektywy, jeśli jest tworzona <xref:System.Type.MakeGenericType%2A?displayProperty=nameWithType> przy użyciu metody i nie jest używana statycznie.  
  
## <a name="methodinfomakegenericmethod-method"></a>MethodInfo. MakeGenericMethod — Metoda  
 Dana Klasa `Class1` z `GetMethod<T>(T t)` metodągenerycznąmożebyćwywoływanapoprzezodbicieprzyużyciukoduwnastępującysposób:`GetMethod`  
...
- Dynamiczne metadane dla metody, która ma zostać wywołana, tak aby delegat wywołania odbicia nie został usunięty przez łańcuch narzędzi .NET Native. Jeśli brakuje metadanych dynamicznych dla metody, następujący wyjątek jest zgłaszany, gdy <xref:System.Reflection.MethodInfo.MakeGenericMethod%2A?displayProperty=nameWithType> wywoływana jest metoda:  
...
```  
 ...
 Dyrektywa jest wymagana dla każdego innego wystąpienia metody, która jest wywoływana dynamicznie, `Arguments` i jest aktualizowana w celu odzwierciedlenia każdego innego argumentu tworzenia wystąpienia. `MethodInstantiation`  
  
## <a name="arraycreateinstance-and-typemaketypearray-methods"></a>Array. CreateInstance i Type. MakeTypeArray, metody  
 Poniższy przykład wywołuje <xref:System.Type.MakeArrayType%2A?displayProperty=nameWithType> metody i <xref:System.Array.CreateInstance%2A?displayProperty=nameWithType> dla typu, `Class1`.  
