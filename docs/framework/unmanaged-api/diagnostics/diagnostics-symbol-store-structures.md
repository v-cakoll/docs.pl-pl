---
title: Struktury magazynu symboli diagnostycznych
ms.date: 03/30/2017
helpviewer_keywords:
- unmanaged structures [.NET Framework], diagnostics symbol store
- unmanaged structures [.NET Framework], debugging
- diagnostics symbol store structures [.NET Framework]
- structures [.NET Framework debugging]
ms.assetid: 0d423791-d40d-45c6-a82b-1a165768d737
author: mairaw
ms.author: mairaw
ms.openlocfilehash: 2e7bd0fabcd4838990ae8704de4f83e9677dbc08
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
---
# <a name="diagnostics-symbol-store-structures"></a>Struktury magazynu symboli diagnostycznych
W tym temacie opisano niezarządzane struktury, których używa magazynu symboli diagnostycznych interfejsu API.  
  
## <a name="in-this-section"></a>W tej sekcji  
 [CALL_ID, struktura](../../../../docs/framework/unmanaged-api/diagnostics/call-id-structure.md)  
 Zapewnia informacje o debugera o funkcję, która jest wywoływana.  
  
 [SYMLINEDELTA, struktura](../../../../docs/framework/unmanaged-api/diagnostics/symlinedelta-structure.md)  
 Zawiera informacje metod, które zostały przeniesione w wyniku zmiany do obsługi symboli.  
  
 [USER_THREAD, struktura](../../../../docs/framework/unmanaged-api/diagnostics/user-thread-structure.md)  
 Zapewnia informacje o debugerze o wątku.  
  
## <a name="related-sections"></a>Sekcje pokrewne  
 [Interfejsy magazynu symboli diagnostycznych](../../../../docs/framework/unmanaged-api/diagnostics/diagnostics-symbol-store-interfaces.md)  
  
 [Wyliczenia magazynu symboli diagnostycznych](../../../../docs/framework/unmanaged-api/diagnostics/diagnostics-symbol-store-enumerations.md)  
  
 [Debugowanie](../../../../docs/framework/unmanaged-api/debugging/index.md)
