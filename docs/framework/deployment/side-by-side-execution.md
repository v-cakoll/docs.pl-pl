...
Oprócz informacji w nagłówku pliku PE można wdrożyć aplikację z plikiem konfiguracji aplikacji, który zawiera informacje o wersji środowiska uruchomieniowego. Plik konfiguracji aplikacji jest plikiem opartym na formacie XML, który jest tworzony przez dewelopera aplikacji i dostarczany z aplikacją. [\<requiredRuntime > elementu](../configure-apps/file-schema/startup/requiredruntime-element.md) [\<> uruchamiania](../configure-apps/file-schema/startup/startup-element.md), jeśli znajduje się w tym pliku, określa wersje środowiska uruchomieniowego i wersje składnika obsługiwane przez aplikację. Możesz również użyć tego pliku do testowania, aby przetestować zgodność aplikacji z różnymi wersjami środowiska uruchomieniowego.  
...
1. Środowisko uruchomieniowe bada element [\<supportedRuntime >](../configure-apps/file-schema/startup/supportedruntime-element.md) elementu w pliku konfiguracyjnym aplikacji. Jeśli istnieje co najmniej jedna obsługiwana wersja środowiska uruchomieniowego określona w elemencie **\<supportedRuntime >** , środowisko uruchomieniowe ładuje wersję środowiska uruchomieniowego określoną przez pierwszy **\<elementu > supportedRuntime** . Jeśli ta wersja jest niedostępna, środowisko uruchomieniowe sprawdzi następny **\<element > supportedRuntime** i podejmie próbę załadowania określonej wersji środowiska uruchomieniowego. Jeśli ta wersja środowiska uruchomieniowego nie jest dostępna, kolejne **\<elementy > supportedRuntime** . Jeśli żadna z obsługiwanych wersji środowiska uruchomieniowego nie jest dostępna, środowisko uruchomieniowe nie może załadować wersji środowiska uruchomieniowego i wyświetla komunikat dla użytkownika (zobacz krok 3).  
...
Aby ograniczyć częściowo kwalifikowane odwołania do zestawów w kodzie, można użyć elementu [\<qualifyAssembly >](../configure-apps/file-schema/runtime/qualifyassembly-element.md) w pliku konfiguracyjnym aplikacji, aby w pełni kwalifikować częściowo kwalifikowane odwołania do zestawów występujące w kodzie. Użyj elementu **\<qualifyAssembly >** , aby określić tylko pola, które nie zostały ustawione w odwołaniu częściowym. Tożsamość zestawu wymieniona w atrybucie **FullName** musi zawierać wszystkie informacje niezbędne do pełnej kwalifikacji nazwy zestawu: Nazwa zestawu, klucz publiczny, kultura i wersja.  
  
 Poniższy przykład przedstawia wpis pliku konfiguracji aplikacji, aby w pełni kwalifikować zestaw o nazwie `myAssembly`.  
...
```  
 ...
 Za każdym razem, gdy instrukcja ładowania zestawu odwołuje się `myAssembly`, te ustawienia pliku konfiguracji powodują, że środowisko uruchomieniowe automatycznie przetłumaczy częściowo kwalifikowaną `myAssembly` odwołanie na w pełni kwalifikowane odwołanie. Na przykład zestaw. Load ("Moja Assembly") staną się zestawem. Load ("Moja Assembly, Version = 1.0.0.0, publicKeyToken =..., Culture = neutral").  
