...
Buforowanie umożliwia przechowywanie danych w pamięci w celu szybkiego dostępu. Po ponownym uzyskaniu dostępu do danych aplikacje mogą pobrać dane z pamięci podręcznej, zamiast pobierać je z oryginalnego źródła. Może to poprawić wydajność i skalowalność. Ponadto buforowanie sprawia, że dane są dostępne, gdy źródło danych jest tymczasowo niedostępne.
 ...
 .NET Framework zawiera klasy, które umożliwiają korzystanie z buforowania w aplikacjach .NET Framework. Klasy te znajdują się w przestrzeni nazw <xref:System.Runtime.Caching>.

> [!NOTE]
> Przestrzeń nazw <xref:System.Runtime.Caching> jest nowa w .NET Framework 4. Ta przestrzeń nazw sprawia, że buforowanie jest dostępne dla wszystkich aplikacji .NET Framework. W poprzednich wersjach .NET Framework buforowanie było dostępne tylko w przestrzeni nazw <xref:System.Web> i dlatego wymaga zależności od klas ASP.NET.

 W tym instruktażu pokazano, jak używać funkcji buforowania dostępnej w .NET Framework w ramach aplikacji [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)]. W tym przewodniku zawartość pliku tekstowego jest buforowana.
...
## <a name="adding-a-button-to-the-wpf-window"></a>Dodawanie przycisku do okna WPF
 Następnie dodasz kontrolkę Button i utworzysz procedurę obsługi zdarzeń dla zdarzenia `Click` przycisku. Później dodasz kod do tego, gdy klikniesz przycisk, zawartość pliku tekstowego jest buforowana i wyświetlana.
...
2. Z **przybornika**w obszarze **typowe formanty WPF**przeciągnij kontrolkę `Button` do okna `MainWindow`.

3. W oknie **Właściwości** ustaw właściwość `Content` formantu `Button`, aby **uzyskać pamięć podręczną**.
...
- Utwórz wystąpienie klasy pamięci podręcznej — to znaczy, że utworzysz wystąpienie nowego obiektu <xref:System.Runtime.Caching.MemoryCache>.

- Określ, że pamięć podręczna używa obiektu <xref:System.Runtime.Caching.HostFileChangeMonitor> do monitorowania zmian w pliku tekstowym.
...
2. W górnej części pliku (przed deklaracją klasy) Dodaj następujące instrukcje `Imports` (Visual Basic) lub `using` (C#):
...
3. W programie obsługi zdarzeń Dodaj następujący kod, aby utworzyć wystąpienie obiektu pamięci podręcznej:
    ...
    ```
     ...
     Klasa <xref:System.Runtime.Caching.ObjectCache> jest wbudowaną klasą, która udostępnia pamięć podręczną obiektów w pamięci.
...
6. W bloku `if/then` Dodaj następujący kod, aby utworzyć nowy obiekt <xref:System.Runtime.Caching.CacheItemPolicy>, który określa, że wpis pamięci podręcznej wygasa po 10 sekundach.
...
7. Wewnątrz bloku `if/then` i po kodzie dodanym w poprzednim kroku Dodaj następujący kod, aby utworzyć kolekcję dla ścieżek plików, które chcesz monitorować, a następnie dodaj ścieżkę do pliku tekstowego do kolekcji:
    ...
    > [!NOTE]
    > Jeśli plik tekstowy, którego chcesz użyć, nie jest `c:\cache\cacheText.txt`, określ ścieżkę, w której ma zostać użyty plik tekstowy.

8. Po kodzie dodanym w poprzednim kroku Dodaj następujący kod, aby dodać nowy obiekt <xref:System.Runtime.Caching.HostFileChangeMonitor> do kolekcji monitorów zmian dla wpisu pamięci podręcznej:
    ...
    ```
     ...
     Obiekt <xref:System.Runtime.Caching.HostFileChangeMonitor> służy do monitorowania ścieżki pliku tekstowego i powiadamiania pamięci podręcznej w przypadku wystąpienia zmian. W tym przykładzie wpis pamięci podręcznej utraci ważność, jeśli zawartość pliku ulegnie zmianie.
...
10. Po kodzie, który został dodany w poprzednim kroku, Dodaj następujący kod, aby wstawić zawartość pliku do obiektu pamięci podręcznej jako wystąpienie <xref:System.Runtime.Caching.CacheItem>:
    ...
    ```
     ...
     Należy określić informacje dotyczące sposobu wykluczenia wpisu pamięci podręcznej przez przekazanie obiektu <xref:System.Runtime.Caching.CacheItemPolicy> utworzonego wcześniej jako parametr.

11. Po bloku `if/then` Dodaj następujący kod, aby wyświetlić zawartość pliku w pamięci podręcznej w oknie komunikatu:
