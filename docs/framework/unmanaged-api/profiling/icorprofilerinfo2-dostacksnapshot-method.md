...
## <a name="synchronous-stack-walk"></a>Przeszukiwanie stosu synchronicznego  
 ...
 Wywołanie synchroniczne należy wykonać, gdy w odpowiedzi na środowisko CLR wywołuje jedną z metod [ICorProfilerCallback](icorprofilercallback-interface.md) (lub [ICorProfilerCallback2](icorprofilercallback2-interface.md)) profilera, wywołując `DoStackSnapshot`, aby przeprowadzić stos bieżącego wątku. Jest to przydatne, gdy chcesz zobaczyć, jak wygląda stos przy użyciu powiadomienia, takiego jak [ICorProfilerCallback:: ObjectAllocated —](icorprofilercallback-objectallocated-method.md). Po prostu wywołaj `DoStackSnapshot` z poziomu metody `ICorProfilerCallback`, przekazując wartość null w parametrach `context` i `thread`.  
