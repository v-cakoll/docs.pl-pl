...
- `partial`Modyfikator może występować tylko bezpośrednio przed słowami kluczowymi `class` , `struct` lub `interface` .
...
Aby uzyskać więcej informacji, zobacz [typy częściowe](~/_csharplang/spec/classes.md#partial-types) w [specyfikacji języka C#](/dotnet/csharp/language-reference/language-specification/introduction). Specyfikacja języka jest ostatecznym źródłem informacji o składni i użyciu języka C#.
