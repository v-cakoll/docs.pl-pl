...
## <a name="exposing-com-components-to-c"></a>Udostępnianie składników COM\#
 Można wykorzystać składnik COM z C# projektu. Ogólne kroki są następujące:  
...
2. Wygeneruj bibliotekę typów modelu COM i zarejestruj ją pod kątem użycia COM.  
     ...
     Możesz zmodyfikować właściwości projektu C# wizualnego, aby automatycznie zarejestrować C# zestaw na potrzeby współdziałania z modelem com. Program Visual Studio korzysta z [Regasm. exe (Narzędzie rejestracji zestawów)](../../../framework/tools/regasm-exe-assembly-registration-tool.md), używając przełącznika wiersza polecenia `/tlb`, który pobiera zarządzany zestaw jako dane wejściowe, aby wygenerować bibliotekę typów. Ta biblioteka typów opisuje typy `public` w zestawie i dodaje wpisy rejestru, aby klienci modelu COM mogli tworzyć klasy zarządzane.  
