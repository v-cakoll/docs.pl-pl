...
Każdy typ przeznaczony specjalnie do manipulowania grupą obiektów, które mają pewną wspólną charakterystykę, może być traktowany jako kolekcja. Jest prawie zawsze odpowiednie dla takich typów, aby zaimplementować <xref:System.Collections.IEnumerable> lub <xref:System.Collections.Generic.IEnumerable%601>, dlatego w tej sekcji rozważamy tylko typy implementujące jeden lub oba te interfejsy, które mają być kolekcjami.
 ...
 ❌ nie używać słabo wpisanych kolekcji w publicznych interfejsach API.
 ...
 ❌ nie należy używać <xref:System.Collections.ArrayList> ani <xref:System.Collections.Generic.List%601> w publicznych interfejsach API.
 ...
 ❌ nie należy używać `Hashtable` ani `Dictionary<TKey,TValue>` w publicznych interfejsach API.
 ...
 ❌ nie używać <xref:System.Collections.Generic.IEnumerator%601>, <xref:System.Collections.IEnumerator>ani innego typu, który implementuje jeden z tych interfejsów, z wyjątkiem typu zwracanego metody `GetEnumerator`.

 Typy zwracające moduły wyliczające z metod innych niż `GetEnumerator` nie mogą być używane z instrukcją `foreach`.

 ❌ nie należy implementować obu `IEnumerator<T>` i `IEnumerable<T>` dla tego samego typu. To samo dotyczy interfejsów nieogólnych `IEnumerator` i `IEnumerable`.

## <a name="collection-parameters"></a>Parametry kolekcji
 ✔️ używać typu najmniej wyspecjalizowanego możliwego jako typ parametru. Większość elementów członkowskich przyjmujących kolekcje jako parametry używają interfejsu `IEnumerable<T>`.

 ❌ unikać używania <xref:System.Collections.Generic.ICollection%601> lub <xref:System.Collections.ICollection> jako parametru tylko w celu uzyskania dostępu do właściwości `Count`.
...
## <a name="collection-properties-and-return-values"></a>Właściwości kolekcji i wartości zwracane
 ❌ nie zapewniają właściwości kolekcji settable.
 ...
 ✔️ używać `Collection<T>` lub podklasy `Collection<T>` dla właściwości lub wartości zwracanych reprezentujących kolekcje do odczytu i zapisu.
 ...
 ✔️ używać <xref:System.Collections.ObjectModel.ReadOnlyCollection%601>, podklasy `ReadOnlyCollection<T>`lub w rzadkich przypadkach `IEnumerable<T>` dla właściwości lub wartości zwracanych reprezentujących kolekcje tylko do odczytu.
 ...
 ❌ nie zwracają wartości null z właściwości kolekcji ani z metod zwracających kolekcje. W zamian Zwróć pustą kolekcję lub pustą tablicę.
...
### <a name="snapshots-versus-live-collections"></a>Migawki a kolekcje na żywo
 ...
 ❌ nie zwracają kolekcji migawek z właściwości. Właściwości powinny zwracać kolekcje dynamiczne.
 ...
 ✔️ używać kolekcji migawek lub `IEnumerable<T>` na żywo (lub jej podtypu) do reprezentowania kolekcji, które są nietrwałe (tj. mogą ulec zmianie bez jawnego modyfikowania kolekcji).
...
## <a name="implementing-custom-collections"></a>Implementowanie kolekcji niestandardowych
 ✔️ ROZWAŻYĆ dziedziczenie z `Collection<T>`, `ReadOnlyCollection<T>`lub `KeyedCollection<TKey,TItem>` podczas projektowania nowych kolekcji.

 ✔️ Implementowanie `IEnumerable<T>` podczas projektowania nowych kolekcji. Rozważ zaimplementowanie `ICollection<T>` lub nawet `IList<T>`, gdzie ma to sens.
 ...
 ✔️ ROZWAŻYĆ wdrożenie interfejsów kolekcji niegenerycznej (`IList` i `ICollection`), jeśli kolekcja często będzie przenoszona do interfejsów API przyjmujących te interfejsy jako dane wejściowe.

 ❌ UNIKAj implementowania interfejsów kolekcji na typach z złożonymi interfejsami API, które nie są związane z koncepcją kolekcji.

 ❌ nie dziedziczyć z nieogólnych kolekcji podstawowych, takich jak `CollectionBase`. Zamiast tego użyj `Collection<T>`, `ReadOnlyCollection<T>`i `KeyedCollection<TKey,TItem>`.
...
### <a name="naming-custom-collections"></a>Nazewnictwo kolekcji niestandardowych
 ...
 ✔️ Użyj sufiksu "Kolekcja" w nazwach typów implementujących `IEnumerable` (lub ich elementy podrzędne) i reprezentujących listę elementów.

 ✔️ używać odpowiedniej nazwy struktury danych dla niestandardowych struktur danych.

 ❌ unikać używania dowolnych sufiksów oznaczających konkretną implementację, taką jak "LinkedList" lub "Hashtable", w nazwach abstrakcji kolekcji.

 ✔️ Rozważ umieszczenie prefiksu nazw kolekcji z nazwą typu elementu. Na przykład, kolekcja przechowująca elementy typu `Address` (implementujący `IEnumerable<Address>`) powinna być nazywana `AddressCollection`. Jeśli typ elementu to interfejs, prefiks "I" typu elementu można pominąć. W ten sposób Kolekcja elementów <xref:System.IDisposable> może być wywoływana `DisposableCollection`.
