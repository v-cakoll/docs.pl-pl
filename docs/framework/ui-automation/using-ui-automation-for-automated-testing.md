...
#### <a name="automationid"></a>AutomationID  
 Jednoznacznie identyfikuje element automatyzacji na podstawie jego elementów równorzędnych. <xref:System.Windows.Automation.AutomationElement.AutomationIdProperty>nie jest zlokalizowany, w przeciwieństwie do właściwości <xref:System.Windows.Automation.AutomationElement.NameProperty> , takiej jak zwykle jest lokalizowany, jeśli produkt zostanie wysłany w wielu językach. Zobacz [Używanie właściwości AutomationID](use-the-automationid-property.md).  
