...
|Atrybut|Opis|  
|---------------|-----------------|  
|Element MessageVersion|Określa wersję SOAP komunikatów wysyłanych za pomocą powiązania. Dla tej właściwości można ustawić tylko wartość wersji komunikatu <xref:System.ServiceModel.Channels.MessageVersion.None%2A>. Koder komunikatów strumienia bajtów nie obsługuje żadnych innych wersji komunikatów.<br /><br /> Ten atrybut jest typu <xref:System.ServiceModel.Channels.MessageVersion>.|  
