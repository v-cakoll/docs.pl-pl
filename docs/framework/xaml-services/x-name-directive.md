...
## <a name="wpf-usage-notes"></a>Uwagi dotyczące użycia WPF  
 ...
 Dla aplikacji WPF, która używa [!INCLUDE[TLA#tla_visualb](../../../includes/tlasharptla-visualb-md.md)] elementów docelowych i obejmuje pliki XAML z `Page` Akcja kompilacji, właściwości oddzielne odwołania jest tworzony podczas kompilacji, który dodaje `WithEvents` — słowo kluczowe do wszystkich elementów, które mają `x:Name`, do obsługuje `Handles` składnię delegatów obsługi zdarzeń. Ta właściwość zawsze jest publiczny. Aby uzyskać więcej informacji, zobacz [Visual Basic i obsługi zdarzeń WPF](../../../docs/framework/wpf/advanced/visual-basic-and-wpf-event-handling.md).  
