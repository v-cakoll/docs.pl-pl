...
Klasa <xref:System.Transactions.TransactionScope> zapewnia prosty sposób, aby oznaczyć blok kodu jako udział w transakcji, bez konieczności interakcji z samą transakcją. Zakres transakcji można wybrać i automatycznie zarządzać otoczenia transakcji. Ze względu na łatwość użycia i wydajność, zaleca <xref:System.Transactions.TransactionScope> się, aby używać klasy podczas tworzenia aplikacji transakcji.  
  
 Ponadto nie trzeba rejestrować zasobów jawnie z transakcji. Wszelkie <xref:System.Transactions> Menedżera zasobów (takich jak SQL Server 2005) można wykryć istnienie otoczenia transakcji utworzone przez zakres i automatycznie zarejestrować.  
  
## <a name="creating-a-transaction-scope"></a>Tworzenie zakresu transakcji  
 W poniższym przykładzie przedstawiono proste użycie <xref:System.Transactions.TransactionScope> klasy.  
  
 [!code-csharp[TransactionScope#1](../../../../samples/snippets/csharp/VS_Snippets_Remoting/TransactionScope/cs/ScopeWithSQL.cs#1)]
 [!code-vb[TransactionScope#1](../../../../samples/snippets/visualbasic/VS_Snippets_Remoting/TransactionScope/vb/ScopeWithSQL.vb#1)]  
  
 Zakres transakcji jest uruchamiany po <xref:System.Transactions.TransactionScope> utworzeniu nowego obiektu.  Jak pokazano w przykładzie kodu, zaleca się utworzenie `using` zakresów z instrukcją. Instrukcja `using` jest dostępna zarówno w języku C# i `try`Visual Basic i działa jak ... `finally` aby zapewnić prawidłowe usunięcie zakresu.  
  
 Podczas tworzenia instancji <xref:System.Transactions.TransactionScope>, Menedżer transakcji określa, która transakcja wziąć udział w. Po określeniu zakresu zawsze uczestniczy w danej transakcji. Decyzja opiera się na dwa czynniki: Określa, czy transakcja otoczenia jest obecny i ma wartość `TransactionScopeOption` parametr w konstruktorze. Transakcja otoczenia jest transakcji, w którym wykonuje kodu. Odwołanie do transakcji otoczenia można uzyskać przez wywołanie metody statyczne klasy <xref:System.Transactions.Transaction.Current%2A?displayProperty=nameWithType> właściwości <xref:System.Transactions.Transaction> klasy. Aby uzyskać więcej informacji na temat sposobu używania tego parametru, zobacz [zarządzanie przepływem transakcji przy użyciu transactionscopeoption](#ManageTxFlow) sekcji tego tematu.  
  
## <a name="completing-a-transaction-scope"></a>Kończenie zakresu transakcji  
 Po zakończeniu całej pracy, którą chce wykonać w transakcji, należy <xref:System.Transactions.TransactionScope.Complete%2A?displayProperty=nameWIthType> wywołać metodę tylko raz, aby poinformować menedżera transakcji, że jest dopuszczalne, aby zatwierdzić transakcję. Jest to bardzo dobra praktyka, <xref:System.Transactions.TransactionScope.Complete%2A> aby umieścić wywołanie jako ostatnie oświadczenie w `using` bloku.  
  
 Nie wywołanie tej metody przerywa transakcję, ponieważ menedżer transakcji interpretuje to jako błąd systemu lub równoważne wyjątek zgłoszony w zakresie transakcji. Jednak wywołanie tej metody nie gwarantuje, że transakcji będzie zatwierdzone. Jest tylko sposób informowania menedżera transakcji Twój status. Po wywołaniu <xref:System.Transactions.TransactionScope.Complete%2A> metody jest już dostępne otoczenia transakcji przy użyciu <xref:System.Transactions.Transaction.Current%2A> właściwości i próby podjęły spowodują wyjątek.  
  
 Jeśli <xref:System.Transactions.TransactionScope> obiekt utworzył transakcję początkowo, rzeczywista praca zatwierdzania transakcji przez menedżera transakcji występuje `using` po ostatnim wierszu kodu w bloku. Jeśli nie utworzył transakcji, zatwierdzanie występuje zawsze, gdy <xref:System.Transactions.CommittableTransaction.Commit%2A> jest wywoływana przez właściciela <xref:System.Transactions.CommittableTransaction> obiektu. W tym momencie menedżer transakcji wywołuje menedżerów zasobów i informuje ich do <xref:System.Transactions.TransactionScope.Complete%2A> zatwierdzenia lub wycofania, w zależności od tego, czy metoda została wywołana <xref:System.Transactions.TransactionScope> na obiekt.  
  
 Instrukcja `using` zapewnia, <xref:System.Transactions.TransactionScope.Dispose%2A> że metoda <xref:System.Transactions.TransactionScope> obiektu jest wywoływana, nawet jeśli wystąpi wyjątek. <xref:System.Transactions.TransactionScope.Dispose%2A> Metody oznacza koniec zakresu transakcji. Wyjątki, które mogą występować po wywołaniu tej metody nie może mieć wpływ na transakcji. Ta metoda również przywraca otoczenia transakcji jej poprzedniego stanu.  
...
## <a name="managing-transaction-flow-using-transactionscopeoption"></a><a name="ManageTxFlow"></a>Zarządzanie przepływem transakcji przy użyciu transactionscopeoption  
 Zakres transakcji może być zagnieżdżony <xref:System.Transactions.TransactionScope> przez wywołanie metody, która używa z poziomu `RootMethod` metody, która używa własnego zakresu, jak w przypadku metody w poniższym przykładzie,  
...
- Nie w ogóle brać udział w transakcji. Z tego względu nie ma żadnej otoczenia transakcji.  
 ...
 Jeśli zakres jest utworzone za pomocą elementów <xref:System.Transactions.TransactionScopeOption.Suppress>, nigdy nie bierze udział w transakcji, niezależnie od tego, czy transakcja otoczenia jest obecny. Zakres wystąpienia z tej wartości `null` zawsze mają jako jego transakcji otoczenia.  
  
 Powyższych opcji przedstawiono w poniższej tabeli.  
  
|TransactionScopeOption|Otoczenia transakcji|Zakres uczestniczy|  
|----------------------------|-------------------------|-----------------------------|  
|Wymagany|Nie|Nowa transakcja (będzie główny)|  
|Wymagane nowe|Nie|Nowa transakcja (będzie główny)|  
|Pomiń|Nie|Nie transakcji|  
|Wymagany|Tak|Otoczenia transakcji|  
...
|Pomiń|Tak|Nie transakcji|  
 ...
 W poniższym <xref:System.Transactions.TransactionScope> przykładzie pokazano obiekt, który tworzy trzy obiekty <xref:System.Transactions.TransactionScopeOption> zakresu zagnieżdżonego, każdy tworzy wystąpienia z inną wartością.  
...
```  
 ...
 W przykładzie pokazano blok kodu bez transakcji otoczenia tworząc nowy zakres (`scope1`) z . <xref:System.Transactions.TransactionScopeOption.Required> Zakres `scope1` jest zakresem głównego, ponieważ tworzy nową transakcję (transakcji A) i sprawia, że transakcja A otoczenia transakcji. `Scope1`następnie tworzy trzy więcej obiektów, każdy z inną <xref:System.Transactions.TransactionScopeOption> wartość. Na przykład `scope2` jest <xref:System.Transactions.TransactionScopeOption.Required>tworzony z , a ponieważ istnieje transakcja otoczenia, `scope1`łączy się z pierwszą transakcją utworzoną przez . Należy pamiętać, że `scope3` zakres główny nowej transakcji, a `scope4` ma ma otoczenia transakcji.  
...
### <a name="non-transactional-code-inside-a-transaction-scope"></a>Kod nietransakcyjny wewnątrz zakresu transakcji
 ...
 <xref:System.Transactions.TransactionScopeOption.Suppress>jest przydatne, gdy chcesz zachować operacji wykonywanych przez sekcję kodu, a nie chcesz przerwać otoczenia transakcji, jeśli operacje kończą się niepowodzeniem. Na przykład, gdy chcesz wykonać rejestrowanie lub inspekcji operacji lub gdy chcesz opublikować zdarzenia do subskrybentów, niezależnie od tego, czy transakcja otoczenia zatwierdza lub przerywa. Ta wartość umożliwia sekcję kodu nietransakcyjnego wewnątrz zakresu transakcji, jak pokazano w poniższym przykładzie.  
...
## <a name="interop-with-com"></a>Usługę międzyoperacyjną z modelu COM +  
 Podczas tworzenia nowego <xref:System.Transactions.TransactionScope> wystąpienie, można użyć <xref:System.Transactions.EnterpriseServicesInteropOption> wyliczenia w jednym z konstruktorów do określenia sposobu interakcji z modelu COM +. Aby uzyskać więcej informacji na ten temat, zobacz [Interoperacyjność z usługami dla przedsiębiorstw i transakcjami COM+.](interoperability-with-enterprise-services-and-com-transactions.md)  
