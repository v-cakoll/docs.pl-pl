...
| Aplikacje wdrożone maszyny wirtualne na platformie Azure | Aplikacje monolityczne lub N-warstwowych wdrożone maszyny wirtualne i wystąpienia kontenera platformy Azure (ACI) oraz usługi Azure App Service przy użyciu kontenerów lub AKS (usługi Azure Kubernetes Service) | Konteneryzowane mikrousługi na platformie Azure Kubernetes Service (AKS) i/lub mikrousług bezserwerowe, oparte na usłudze Azure Functions. |
