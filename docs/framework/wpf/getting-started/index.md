---
title: Wprowadzenie (WPF)
ms.date: 01/26/2018
helpviewer_keywords:
- getting started [WPF]
- introduction [WPF]
- WPF [WPF], getting started
ms.assetid: 04f91da8-708c-46c7-8172-f1695ec847cd
ms.openlocfilehash: f4c4c4a19c1919a27c15c623bcb30a119a560c75
ms.sourcegitcommit: ba5c189bf44d44204a3e8838e59ec378a62d82f3
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 09/12/2018
ms.locfileid: "44709876"
---
# <a name="getting-started-wpf"></a>Wprowadzenie (WPF)
Windows Presentation Foundation (WPF) to struktura interfejsu użytkownika, która tworzy aplikacje klienta pulpitu. Platforma programistyczna WPF obsługuje szeroką gamę funkcje tworzenia aplikacji, w tym modelu aplikacji, zasobów, formanty, grafiki, układ, powiązań danych, dokumentów i zabezpieczeń. Jest podzbiorem .NET Framework, więc jeśli aplikacje zostały wcześniej utworzony za pomocą programu .NET Framework przy użyciu platformy ASP.NET lub formularzy Windows, doświadczenie w programowaniu należy się zapoznać. WPF używa Extensible Application Markup Language (XAML), aby zapewnić deklaratywny model programowania aplikacji. Ta sekcja zawiera tematy, które wprowadzają i pomóc Ci rozpocząć pracę przy użyciu platformy WPF.  
  
## <a name="where-should-i-start"></a>Gdzie mam zacząć?  
  
|||  
|-|-|  
|Chcę, aby przejść bezpośrednio...|[Przewodnik: moja pierwsza aplikacja klasyczna WPF](../../../../docs/framework/wpf/getting-started/walkthrough-my-first-wpf-desktop-application.md)|  
|Jak projektowanie interfejsu użytkownika aplikacji?|[Projektowanie XAML w programie Visual Studio](/visualstudio/designers/designing-xaml-in-visual-studio)|  
|Jesteś nowym użytkownikiem platformy .NET?|[Omówienie programu .NET Framework](https://msdn.microsoft.com/library/zw4w595w\(v=vs.140\).aspx)<br /><br /> [Podstawy aplikacji .NET framework](../../../../docs/standard/application-essentials.md)<br /><br /> [Wprowadzenie do języków Visual C# i Visual Basic](/visualstudio/ide/getting-started-with-visual-csharp-and-visual-basic)|  
|Więcej informacji o WPF...|[Wprowadzenie do platformy WPF w programie Visual Studio](../../../../docs/framework/wpf/getting-started/introduction-to-wpf-in-vs.md)<br /><br /> [Przegląd XAML (WPF)](../../../../docs/framework/wpf/advanced/xaml-overview-wpf.md)<br /><br /> [Kontrolki](../../../../docs/framework/wpf/controls/index.md)<br /><br /> [Powiązanie danych — omówienie](../../../../docs/framework/wpf/data/data-binding-overview.md)|  
|Jesteś deweloperem formularze Windows?|[Kontrolki formularzy Windows Forms i równoważne kontrolki WPF](../../../../docs/framework/wpf/advanced/windows-forms-controls-and-equivalent-wpf-controls.md)<br /><br /> [Współdziałanie WPF i Windows Forms](../../../../docs/framework/wpf/advanced/wpf-and-windows-forms-interoperation.md)|  
  
## <a name="see-also"></a>Zobacz też  
 [Biblioteka klas](../../../../docs/framework/wpf/class-library-wpf.md)  
 [Projektowanie aplikacji](../../../../docs/framework/wpf/app-development/index.md)  
 [Centrum deweloperów .NET framework](https://www.microsoft.com/net)
