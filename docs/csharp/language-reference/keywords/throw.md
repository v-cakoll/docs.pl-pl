...
- [operator łączenie null](../operators/null-coalescing-operator.md). W poniższym przykładzie `throw` wyrażenie jest używany z operatorem łączenie null zostać zgłoszony wyjątek, jeśli ciąg jest przypisany do `Name` jest właściwość `null`.
