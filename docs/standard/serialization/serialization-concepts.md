...
---
# <a name="serialization-concepts"></a>Koncepcje serializacji
...
Obiekty są prawidłowe tylko w domenie aplikacji, w której zostały utworzone. Każda próba przekazania obiektu jako parametru lub zwrócenia go w wyniku zakończy się niepowodzeniem, chyba że obiekt pochodzi od `MarshalByRefObject` lub jest oznaczony jako `Serializable`. Jeśli obiekt jest oznaczony jako `Serializable`, obiekt zostanie automatycznie Zserializowany, transportowany z jednej domeny aplikacji do drugiego, a następnie deserializowany, aby utworzyć dokładną kopię obiektu w drugiej domenie aplikacji. Ten proces jest zwykle określany jako marshal przez wartość.

Gdy obiekt pochodzi z `MarshalByRefObject`, odwołanie do obiektu jest przesyłane z jednej domeny aplikacji do innej, a nie do samego obiektu. Można także oznaczyć obiekt, który pochodzi od `MarshalByRefObject` jako `Serializable`. Gdy ten obiekt jest używany z usługami zdalnymi, program formatujący odpowiedzialny za serializacji, który został wstępnie skonfigurowany z selektorem surogatu (`SurrogateSelector`), przejmuje kontrolę nad procesem serializacji i zastępuje wszystkie obiekty pochodne od `MarshalByRefObject` z serwerem proxy. Bez `SurrogateSelector`, architektura serializacji jest zgodna ze standardowymi regułami serializacji opisanymi w [krokach procesu serializacji](steps-in-the-serialization-process.md).  
