...
## <a name="obtain-the-service-contract-bindings-and-addresses"></a>Uzyskiwanie kontraktu, powiązań i adresów usługi  
 W programie WCF, usługi i klienci modelują kontrakty przy użyciu atrybutów zarządzanych, interfejsów i metod. Aby nawiązać połączenie z usługą w aplikacji klienckiej, należy uzyskać informacje o typie dla kontraktu usługi. Zazwyczaj można to zrobić za pomocą narzędzia do obsługi [metadanych ServiceModel (Svcutil. exe)](../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md), które pobiera metadane z usługi, konwertuje je na plik zarządzanego kodu źródłowego w wybranym języku i tworzy plik konfiguracji aplikacji klienckiej. za pomocą programu można skonfigurować obiekt klienta WCF. Na przykład jeśli zamierzasz utworzyć obiekt klienta WCF w celu wywołania a `MyCalculatorService`i wiesz, że metadane dla tej usługi są publikowane w `http://computerName/MyCalculatorService/Service.svc?wsdl`programie, Poniższy przykład kodu pokazuje, jak używać programu Svcutil `ClientCode.vb` . exe do uzyskania pliku, który zawiera kontrakt usługi w kodzie zarządzanym.  
...
```  
 ...
 Przykład tego procesu można znaleźć w temacie [How to: Utwórz klienta](../../../docs/framework/wcf/how-to-create-a-wcf-client.md). Aby uzyskać więcej informacji na temat umów, zobacz [kontrakt](../../../docs/framework/wcf/feature-details/contracts.md).  
...
## <a name="create-a-wcf-client-object"></a>Tworzenie obiektu klienta WCF  
 ...
 Za pomocą obiektów kanału klienta WCF można także łączyć się z usługami i korzystać z nich. Aby uzyskać szczegółowe informacje, zobacz [Architektura klienta WCF](../../../docs/framework/wcf/feature-details/client-architecture.md).  
...
> Jeśli używasz programu Visual Studio do utworzenia klienta WCF, obiekty są ładowane automatycznie do przeglądarki obiektów po dodaniu do projektu odwołania do usługi.  
 ...
 Jeśli nie używasz programu Visual Studio, sprawdź wygenerowany kod kontraktu, aby znaleźć typ, który rozszerza <xref:System.ServiceModel.ClientBase%601> i interfejs `ISampleService`kontraktu usługi. W tym przypadku ten typ wygląda podobnie do następującego kodu:  
...
## <a name="configuring-and-securing-clients"></a>Konfigurowanie i Zabezpieczanie klientów  
 ...
 Na przykład wymagania dotyczące zabezpieczeń dla kontraktów usług są deklarowane w interfejsie kontraktu usługi, a plik konfiguracji programu Svcutil. exe został utworzony w celu zapewnienia obsługi wymagań dotyczących zabezpieczeń usługi. Jednak w niektórych przypadkach może być wymagana większa Konfiguracja zabezpieczeń, na przykład skonfigurowanie poświadczeń klienta. Aby uzyskać pełne informacje na temat konfiguracji zabezpieczeń dla klientów programu WCF, zobacz [Zabezpieczanie klientów](../../../docs/framework/wcf/securing-clients.md).  
...
## <a name="calling-services-asynchronously"></a>Asynchroniczne wywoływanie usług  
 Sposób wywoływania operacji jest całkowicie do dewelopera klienta. Wynika to z faktu, że komunikaty wchodzące w skład operacji można zamapować na metody synchroniczne lub asynchroniczne, gdy wyrażasz w kodzie zarządzanym. W związku z tym, jeśli chcesz skompilować klienta, który wywołuje operacje asynchronicznie, możesz użyć programu Svcutil. exe do generowania kodu klienta asynchronicznego `/async` przy użyciu opcji. Aby uzyskać więcej informacji, zobacz [jak: Asynchroniczne](../../../docs/framework/wcf/feature-details/how-to-call-wcf-service-operations-asynchronously.md)wywoływanie operacji usługi.  
  
## <a name="calling-services-using-wcf-client-channels"></a>Wywoływanie usług przy użyciu kanałów klienta WCF  
 Typ klienta WCF pozwala <xref:System.ServiceModel.ClientBase%601>zwiększyć, który sam pochodzi <xref:System.ServiceModel.IClientChannel?displayProperty=nameWithType> z interfejsu, aby uwidocznić podstawowy system kanałów. Możesz wywoływać usługi przy użyciu kontraktu usługi docelowej z <xref:System.ServiceModel.ChannelFactory%601?displayProperty=nameWithType> klasą. Aby uzyskać szczegółowe informacje, zobacz [Architektura klienta WCF](../../../docs/framework/wcf/feature-details/client-architecture.md).  
