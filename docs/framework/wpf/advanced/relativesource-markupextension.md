...
Określa właściwości <xref:System.Windows.Data.RelativeSource> źródła powiązania, które ma być używane w [rozszerzeniu Binding Markup](binding-markup-extension.md)lub podczas ustawiania <xref:System.Windows.Data.Binding.RelativeSource%2A> właściwości <xref:System.Windows.Data.Binding> elementu ustanowionego w języku XAML.
...
|||
|-|-|
|`modeEnumValue`|Jeden z poniższych programów:<br /><br /> -Token ciągu `Self` ; odnosi się do typu <xref:System.Windows.Data.RelativeSource> , który został utworzony z <xref:System.Windows.Data.RelativeSource.Mode%2A> właściwością ustawioną na <xref:System.Windows.Data.RelativeSourceMode.Self> .<br />-Token ciągu `TemplatedParent` ; odnosi się do typu <xref:System.Windows.Data.RelativeSource> , który został utworzony z <xref:System.Windows.Data.RelativeSource.Mode%2A> właściwością ustawioną na <xref:System.Windows.Data.RelativeSourceMode.TemplatedParent> .<br />-Token ciągu `PreviousData` ; odnosi się do typu <xref:System.Windows.Data.RelativeSource> , który został utworzony z <xref:System.Windows.Data.RelativeSource.Mode%2A> właściwością ustawioną na <xref:System.Windows.Data.RelativeSourceMode.PreviousData> .<br />-Zobacz poniżej, aby uzyskać informacje o `FindAncestor` trybie.|
|`FindAncestor`|Token ciągu `FindAncestor` . Użycie tego tokenu spowoduje przejście do trybu, w którym `RelativeSource` określa typ elementu nadrzędnego i opcjonalnie poziomu nadrzędnego. Odnosi się to <xref:System.Windows.Data.RelativeSource> tak, jakby został utworzony z <xref:System.Windows.Data.RelativeSource.Mode%2A> właściwością ustawioną na <xref:System.Windows.Data.RelativeSourceMode.FindAncestor> .|
...
W [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] implementacji procesora XAML obsługa tego rozszerzenia znacznika jest definiowana przez <xref:System.Windows.Data.RelativeSource> klasę.
