...
|`statements`|Opcjonalny. Jedna lub więcej instrukcji między `With` i `End With`, które mogą odwoływać się do elementów członkowskich obiektu, który jest produkowany w wyniku oceny `objectExpression`.|
|`End With`|Wymagany. Kończy definicję bloku `With`.|
...
@No__t_0 jest oceniane jednokrotnie przy wpisie do bloku. Nie można ponownie przypisać `objectExpression` z poziomu bloku `With`.
