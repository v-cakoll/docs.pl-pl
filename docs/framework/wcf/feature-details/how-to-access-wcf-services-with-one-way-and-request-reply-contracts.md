...
---
# <a name="how-to-access-wcf-services-with-one-way-and-request-reply-contracts"></a><span data-ttu-id="77f05-102">Instrukcje: Uzyskiwanie dostępu do usług WCF za pomocą kontraktów jednokierunkowych i kontraktów „żądanie-odpowiedź”</span><span class="sxs-lookup"><span data-stu-id="77f05-102">How to: Access WCF Services with One-Way and Request-Reply Contracts</span></span>
<span data-ttu-id="77f05-103">W poniższych procedurach opisano, jak uzyskać dostęp do usługi Windows Communication Foundation (WCF) definiuje kontraktu jednokierunkowego i kontraktu "żądanie-odpowiedź", które nie korzystają z paradygmacie komunikacji dupleksowej.</span><span class="sxs-lookup"><span data-stu-id="77f05-103">The following procedures describe how to access a Windows Communication Foundation (WCF) service that defines a one-way contract and a request-reply contract and that does not use the duplex communication pattern.</span></span>  
  
### <a name="to-define-the-service"></a><span data-ttu-id="77f05-104">Aby zdefiniować usługi</span><span class="sxs-lookup"><span data-stu-id="77f05-104">To define the service</span></span>  
  
1. <span data-ttu-id="77f05-105">Zadeklaruj kontraktu usługi.</span><span class="sxs-lookup"><span data-stu-id="77f05-105">Declare the service contract.</span></span> <span data-ttu-id="77f05-106">Operacje, które mają być jednokierunkowe musi mieć `IsOneWay` równa `true` w ramach <xref:System.ServiceModel.OperationContractAttribute>.</span><span class="sxs-lookup"><span data-stu-id="77f05-106">The operations that are to be one-way must have `IsOneWay` set to `true` within the <xref:System.ServiceModel.OperationContractAttribute>.</span></span> <span data-ttu-id="77f05-107">Poniższy kod deklaruje `IOneWayCalculator` kontraktu, który zawiera operacje jednokierunkowe `Add`, `Subtract`, `Multiply`, i `Divide`.</span><span class="sxs-lookup"><span data-stu-id="77f05-107">The following code declares the `IOneWayCalculator` contract that has one-way operations for `Add`, `Subtract`, `Multiply`, and `Divide`.</span></span> <span data-ttu-id="77f05-108">Umożliwia on również definiowanie operacji odpowiedzi żądania o nazwie `SayHello`.</span><span class="sxs-lookup"><span data-stu-id="77f05-108">It also defines a request response operation called `SayHello`.</span></span>  
...
2. <span data-ttu-id="77f05-109">Implementowanie kontraktu usługi.</span><span class="sxs-lookup"><span data-stu-id="77f05-109">Implement the service contract.</span></span> <span data-ttu-id="77f05-110">Poniższy kod implementuje `IOnewayCalculator` interfejsu.</span><span class="sxs-lookup"><span data-stu-id="77f05-110">The following code implements the `IOnewayCalculator` interface.</span></span>  
...
3. <span data-ttu-id="77f05-111">Hostowanie usług w aplikacji konsoli.</span><span class="sxs-lookup"><span data-stu-id="77f05-111">Host the service in a console application.</span></span> <span data-ttu-id="77f05-112">Poniższy kod pokazuje, jak hostować usługę.</span><span class="sxs-lookup"><span data-stu-id="77f05-112">The following code shows how to host the service.</span></span>  
...
1. <span data-ttu-id="77f05-114">Uruchom [narzędzia narzędzie metadanych elementu ServiceModel (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) przy użyciu adresu punktu końcowego metadanych programu exchange do utworzenia klasy klienta dla usługi przy użyciu następującego polecenia: `Svcutil http://localhost:8000/Service` [Narzędzia narzędzie metadanych elementu ServiceModel (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) generuje zestaw interfejsów i klas, jak pokazano w poniższym przykładowym kodzie.</span><span class="sxs-lookup"><span data-stu-id="77f05-114">Run the [ServiceModel Metadata Utility Tool (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) using the metadata exchange endpoint address to create the client class for the service using the following command line: `Svcutil http://localhost:8000/Service` The [ServiceModel Metadata Utility Tool (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) generates a set of interfaces and classes, as shown in the following sample code.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="77f05-115">Należy zauważyć w `IOneWayCalculator` interfejs, który ma operacje usługi jednokierunkowej <xref:System.ServiceModel.OperationContractAttribute.IsOneWay%2A> ustawioną wartość atrybutu `true` i operacji usługi "żądanie-odpowiedź" ma atrybut ustawiony na wartość domyślną `false`.</span><span class="sxs-lookup"><span data-stu-id="77f05-115">Notice in the `IOneWayCalculator` interface that the one-way service operations have the <xref:System.ServiceModel.OperationContractAttribute.IsOneWay%2A> attribute set to `true` and the request-reply service operation has the attribute set to the default value, `false`.</span></span> <span data-ttu-id="77f05-116">Należy również zauważyć `OneWayCalculatorClient` klasy.</span><span class="sxs-lookup"><span data-stu-id="77f05-116">Also notice the `OneWayCalculatorClient` class.</span></span> <span data-ttu-id="77f05-117">Jest to klasa, która zostanie wykorzystany do wywołania usługi.</span><span class="sxs-lookup"><span data-stu-id="77f05-117">This is the class that you will use to call the service.</span></span>  
  
2. <span data-ttu-id="77f05-118">Utworzenie obiektu klienta.</span><span class="sxs-lookup"><span data-stu-id="77f05-118">Create the client object.</span></span>  
...
3. <span data-ttu-id="77f05-119">Wywoływanie operacji usługi.</span><span class="sxs-lookup"><span data-stu-id="77f05-119">Call service operations.</span></span>  
...
4. <span data-ttu-id="77f05-120">Zamknij klienta, aby zamknąć połączenia i wyczyścić zasoby.</span><span class="sxs-lookup"><span data-stu-id="77f05-120">Close the client to close connections and clean up resources.</span></span>  
...
## <a name="example"></a><span data-ttu-id="77f05-121">Przykład</span><span class="sxs-lookup"><span data-stu-id="77f05-121">Example</span></span>  
 <span data-ttu-id="77f05-122">Poniżej znajduje się pełna lista kod używany w tym temacie.</span><span class="sxs-lookup"><span data-stu-id="77f05-122">The following is a complete listing of the code used  in this topic.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="77f05-123">Zobacz także</span><span class="sxs-lookup"><span data-stu-id="77f05-123">See also</span></span>

- [<span data-ttu-id="77f05-124">Usługi jednokierunkowe</span><span class="sxs-lookup"><span data-stu-id="77f05-124">One-Way Services</span></span>](../../../../docs/framework/wcf/feature-details/one-way-services.md)
