...
|<xref:System.Text.RegularExpressions.RegexOptions.Multiline>|`m`|Użyj trybu wielowierszowego, gdzie `^` i `$` dopasowuje początek i koniec każdego wiersza (zamiast początku i końca ciągu wejściowego). Aby uzyskać więcej informacji, zobacz [tryb wielowierszowy](#Multiline).|
...
|<xref:System.Text.RegularExpressions.RegexOptions.ExplicitCapture>|`n`|Nie przechwytuje nienazwanych grup. Jedyne prawidłowe przechwycenia to jawnie nazwane lub numerowane grupy formularza `(?<`*nazwa*`>` *Podwyrażenie*`)`. Aby uzyskać więcej informacji, zobacz [tylko jawne przechwycenia](#Explicit).|
...
- Stosując opcje wbudowane w określonej konstrukcji grupowania we wzorcu wyrażenia regularnego z składnią `(?imnsx-imnsx:`*podwyrażeniem*`)`. Nie pisz przed zestawem opcji ustawia włączony; znak minus przed zestawem opcji powoduje wyłączenie zestawu. (`?` to stała część składni konstrukcji języka, która jest wymagana, czy opcje są włączone, czy wyłączone). Ta opcja ma zastosowanie tylko do tej grupy. Aby uzyskać więcej informacji, zobacz [grupowanie konstrukcji](../../../docs/standard/base-types/grouping-constructs-in-regular-expressions.md).
...
Jeśli opcje są określone w tekście, znak minusa (`-`) przed opcją lub zestaw opcji powoduje wyłączenie tych opcji. Na przykład, konstrukcja wbudowana `(?ix-ms)` włącza opcje <xref:System.Text.RegularExpressions.RegexOptions.IgnoreCase?displayProperty=nameWithType> i <xref:System.Text.RegularExpressions.RegexOptions.IgnorePatternWhitespace?displayProperty=nameWithType> i wyłącza opcje <xref:System.Text.RegularExpressions.RegexOptions.Multiline?displayProperty=nameWithType> i <xref:System.Text.RegularExpressions.RegexOptions.Singleline?displayProperty=nameWithType>. Wszystkie opcje wyrażenia regularnego są domyślnie wyłączone.

> [!NOTE]
> Jeśli opcje wyrażenia regularnego określone w parametrze `options` konstruktora lub wywołania metody powodują konflikt z opcjami określonymi wewnętrznie we wzorcu wyrażenia regularnego, używane są opcje wbudowane.
...
Następujące pięć opcji wyrażenia regularnego można ustawić za pomocą parametru `options`, ale nie można ustawić go w tekście:
...
Aby przetestować obecność dowolnej opcji, z wyjątkiem <xref:System.Text.RegularExpressions.RegexOptions.None?displayProperty=nameWithType>, wykonaj operację i z wartością właściwości <xref:System.Text.RegularExpressions.Regex.Options%2A?displayProperty=nameWithType> i wartości <xref:System.Text.RegularExpressions.RegexOptions>, w której interesują Cię zainteresowania. Następnie sprawdź, czy wynik jest równy wartości <xref:System.Text.RegularExpressions.RegexOptions>. Poniższy przykład sprawdza, czy ustawiono opcję <xref:System.Text.RegularExpressions.RegexOptions.IgnoreCase?displayProperty=nameWithType>.
...
- Element języka `.` dopasowuje każdy znak z wyjątkiem `\n`.
...
Opcja <xref:System.Text.RegularExpressions.RegexOptions.IgnoreCase> lub opcja wbudowana `i` zapewniają dopasowanie bez uwzględniania wielkości liter. Domyślnie używane są konwencje wielkości liter bieżącej kultury.

W poniższym przykładzie zdefiniowano wzorzec wyrażenia regularnego, `\bthe\w*\b`, który dopasowuje wszystkie wyrazy rozpoczynające się od "a". Ponieważ pierwsze wywołanie metody <xref:System.Text.RegularExpressions.Regex.Match%2A> używa domyślnego porównania uwzględniającego wielkość liter, dane wyjściowe wskazują, że ciąg "" ", który rozpoczyna zdanie, nie jest dopasowany. Jest dopasowywany w przypadku wywołania metody <xref:System.Text.RegularExpressions.Regex.Match%2A> z opcjami ustawionymi na <xref:System.Text.RegularExpressions.RegexOptions.IgnoreCase>.
...
Opcja <xref:System.Text.RegularExpressions.RegexOptions.Multiline?displayProperty=nameWithType> lub opcja wbudowana `m` umożliwia aparatowi wyrażeń regularnych obsługę ciągu wejściowego, który składa się z wielu wierszy. Zmienia interpretację elementów języka `^` i `$`, tak aby pasowały do początku i końca wiersza, zamiast początku i końca ciągu wejściowego.

Domyślnie `$` pasuje tylko do końca ciągu wejściowego. W przypadku określenia opcji <xref:System.Text.RegularExpressions.RegexOptions.Multiline?displayProperty=nameWithType> pasuje do znaku nowego wiersza (`\n`) lub końca ciągu wejściowego. Nie jest jednak zgodna z kombinacją znaku powrotu karetki/wysuwu wiersza. Aby pomyślnie dopasować je, użyj podwyrażenia `\r?$` zamiast tylko `$`.

W poniższym przykładzie wykorzystamy nazwy i wyniki dla Pucharek i dodaje je do kolekcji <xref:System.Collections.Generic.SortedList%602>, która sortuje je w kolejności malejącej. Metoda <xref:System.Text.RegularExpressions.Regex.Matches%2A> jest wywoływana dwukrotnie. W pierwszym wywołaniu metody wyrażenie regularne jest `^(\w+)\s(\d+)$` i nie są ustawione żadne opcje. Jak pokazuje dane wyjściowe, ponieważ aparat wyrażeń regularnych nie może dopasować wzorca wejściowego do początku i końca ciągu wejściowego, nie znaleziono żadnych dopasowań. W drugim wywołaniu metody wyrażenie regularne jest zmieniane na `^(\w+)\s(\d+)\r?$`, a opcje są ustawione na <xref:System.Text.RegularExpressions.RegexOptions.Multiline?displayProperty=nameWithType>. Jak pokazuje dane wyjściowe, nazwy i oceny zostaną pomyślnie dopasowane, a wyniki są wyświetlane w kolejności malejącej.
...
Opcja <xref:System.Text.RegularExpressions.RegexOptions.Singleline?displayProperty=nameWithType> lub opcja wbudowana `s` powoduje, że aparat wyrażeń regularnych traktuje ciąg wejściowy tak, jakby zawiera pojedynczy wiersz. Robi to poprzez zmianę zachowania elementu języka (`.`), tak aby pasował do każdego znaku, a nie do dopasowania każdego znaku z wyjątkiem znaku nowego wiersza `\n` lub \u000A.
...
Można również użyć elementu wbudowanego `(?n)`, aby pominąć Automatyczne przechwytywanie. Poniższy przykład modyfikuje poprzedni wzorzec wyrażenia regularnego, aby użyć wbudowanego elementu `(?n)` zamiast opcji <xref:System.Text.RegularExpressions.RegexOptions.ExplicitCapture?displayProperty=nameWithType>.
...
Skompilowane wyrażenia regularne można używać w wywołaniach zarówno w wyrażeniach statycznych, jak i wystąpieniach regularnych. W statycznych wyrażeniach regularnych opcja <xref:System.Text.RegularExpressions.RegexOptions.Compiled?displayProperty=nameWithType> jest przenoszona do parametru `options` metody dopasowania do wzorca wyrażenia regularnego. W wyrażeniach regularnych wystąpienia jest ona przenoszona do parametru `options` konstruktora klasy <xref:System.Text.RegularExpressions.Regex>. W obu przypadkach powstaje zwiększona wydajność.

Jednak ten wzrost wydajności występuje tylko w następujących warunkach:

- Obiekt <xref:System.Text.RegularExpressions.Regex> reprezentujący określone wyrażenie regularne jest używany w wielu wywołaniach metod dopasowania do wzorca wyrażenia regularnego.

- Obiekt <xref:System.Text.RegularExpressions.Regex> nie może znajdować się poza zakresem, więc można go ponownie użyć.
...
> [!NOTE]
> Opcja <xref:System.Text.RegularExpressions.RegexOptions.Compiled?displayProperty=nameWithType> nie jest powiązana z metodą <xref:System.Text.RegularExpressions.Regex.CompileToAssembly%2A?displayProperty=nameWithType>, która tworzy zestaw specjalnego przeznaczenia zawierający wstępnie zdefiniowane skompilowane wyrażenia regularne.
...
- Niezmieniony biały znak we wzorcu wyrażenia regularnego jest ignorowany. Aby była częścią wzorca wyrażenia regularnego, znaki odstępu muszą mieć wartość ucieczki (na przykład `\s` lub "`\`").
...
- Biały znak nie jest dozwolony w obrębie kwantyfikatora, takiego jak `{`*n*`}`, `{`*n*`,}` i `{`*n*`,`*m*0. Na przykład wzorzec wyrażenia regularnego `\d{1, 3}` nie jest zgodny z żadną sekwencją cyfr od jednej do trzech cyfr, ponieważ zawiera znak odstępu.

- Biały znak jest niedozwolony w sekwencji znaków, która wprowadza element języka. Na przykład:

  - Element języka `(?:`*Podwyrażenie*`)` reprezentuje grupę nieprzechwytującą, a `(?:` część elementu nie może mieć osadzonych spacji. Wzorzec `(? :`*subexpression*`)` zgłasza <xref:System.ArgumentException> w czasie wykonywania, ponieważ aparat wyrażeń regularnych nie może przeanalizować wzorca, a wzorzec `( ?:`*Podwyrażenie*`)` nie dopasuje do *wyrażenia*podrzędnego.

  - Element języka `\p{`*nazwa*`}`, reprezentujący kategorię Unicode lub blok nazwany, nie może zawierać osadzonych miejsc w części `\p{` elementu. Jeśli zostanie uwzględniony biały znak, element zgłasza <xref:System.ArgumentException> w czasie wykonywania.
...
Ten wzorzec jest podobny do wzorca zdefiniowanego w sekcji [tylko przechwycenia jawne](#Explicit) , z tą różnicą, że używa opcji <xref:System.Text.RegularExpressions.RegexOptions.IgnorePatternWhitespace?displayProperty=nameWithType> do ignorowania białego znaku w deseniu.

[!code-csharp[Conceptual.Regex.Language.Options#12](../../../samples/snippets/csharp/VS_Snippets_CLR/conceptual.regex.language.options/cs/whitespace1.cs#12)]
[!code-vb[Conceptual.Regex.Language.Options#12](../../../samples/snippets/visualbasic/VS_Snippets_CLR/conceptual.regex.language.options/vb/whitespace1.vb#12)]

W poniższym przykładzie zastosowano opcję wbudowaną `(?x)`, aby zignorować biały znak w deseniu.
...
> [!NOTE]
> Tryb wzorca od prawej do lewej jest dostępny tylko przez dostarczenie wartości <xref:System.Text.RegularExpressions.RegexOptions.RightToLeft?displayProperty=nameWithType> do parametru `options` konstruktora klasy <xref:System.Text.RegularExpressions.Regex> lub statycznej metody dopasowania do wzorca. Nie jest ona dostępna jako opcja wbudowana.
...
Domyślnie aparat wyrażeń regularnych używa zachowania kanonicznego, gdy dopasowuje wzorzec wyrażenia regularnego do tekstu wejściowego. Można jednak poinstruować aparat wyrażeń regularnych, aby używał zachowania zgodnego ze standardem ECMAScript, określając opcję <xref:System.Text.RegularExpressions.RegexOptions.ECMAScript?displayProperty=nameWithType>.
...
- Rozpoznawanie niejasności między ósemkowymi znakami ucieczki i odwołaniami wstecznymi. Poniższa tabela podsumowuje różnice w liczbie ósemkowej i interpretacji odwołania wstecznego przez kanoniczne i ECMAScript wyrażenia regularne.
  ...
  |------------------------|------------------------|-------------------------|
  |`\0`, po których następują cyfry ósemkowe (od 0 do 2)|Interpretuj jako ósemkowy. Na przykład `\044` jest zawsze interpretowana jako wartość ósemkowa i oznacza "$".|Takie samo zachowanie.|
  |`\` następuje cyfra od 1 do 9, a po niej nie ma dodatkowych cyfr dziesiętnych,|Interpretuj jako odwołanie wsteczne. Na przykład `\9` zawsze oznacza odwołanie wsteczne 9, nawet jeśli dziewiąta grupa przechwytywania nie istnieje. Jeśli grupa przechwytywania nie istnieje, Analizator wyrażenia regularnego zgłasza <xref:System.ArgumentException>.|Jeśli istnieje pojedyncza cyfra dziesiętna grupa przechwytywania, odwołuje się do tej cyfry. W przeciwnym razie interpretuje wartość jako literał.|
