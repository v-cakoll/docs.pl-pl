...
Wszystkie aplikacje obsługujące środowisko uruchomieniowe języka wspólnego (CLR) muszą uruchamiać, lub *aktywować*, środowisko CLR w celu uruchomienia kodu zarządzanego. Zazwyczaj aplikacja .NET Framework jest uruchamiana w wersji środowiska CLR, w którym została skompilowana, ale można zmienić to zachowanie dla aplikacji klasycznych za pomocą pliku konfiguracji aplikacji (czasami określanego jako plik app.config). Przy użyciu pliku konfiguracji aplikacji nie można jednak zmienić domyślnego zachowania aktywacji dla aplikacji do Sklepu Windows lub dla systemu Windows Phone. W tym artykule wyjaśniono, jak umożliwić uruchamianie aplikacji klasycznej w innej wersji .NET Framework i przedstawiono przykład sposobu, w jaki jest przeznaczony dla wersji 4 lub nowszej.

 Wersja programu .NET Framework, w której działa aplikacja, jest określana w następującej kolejności:

- Plik konfiguracji.

     Jeśli plik konfiguracyjny aplikacji zawiera [\<supportedRuntime>](../configure-apps/file-schema/startup/supportedruntime-element.md) wpisy określające co najmniej jedną wersję .NET Framework, a jedna z tych wersji jest obecna na komputerze użytkownika, aplikacja jest uruchamiana w tej wersji. Plik konfiguracji odczytuje [\<supportedRuntime>](../configure-apps/file-schema/startup/supportedruntime-element.md) wpisy w kolejności, w jakiej są wyświetlane, i używa pierwszej wersji .NET Framework wymienionej na komputerze użytkownika. (Użyj [ \<requiredRuntime> elementu](../configure-apps/file-schema/startup/requiredruntime-element.md) dla wersji 1,0).
...
2. Dodaj [\<supportedRuntime>](../configure-apps/file-schema/startup/supportedruntime-element.md) element w następujący sposób do pliku konfiguracji aplikacji:
    ...
    ```
     ...
     gdzie *\<version>* określa wersję środowiska CLR, która jest wyrównana do .NET Framework wersji obsługiwanej przez aplikację. Użyj następujących ciągów:
    ...
    - .NET Framework 4 i nowsze wersje: "v 4.0"
     ...
     [\<supportedRuntime>](../configure-apps/file-schema/startup/supportedruntime-element.md)Aby określić obsługę wielu wersji .NET Framework, można dodać wiele elementów, które są wymienione w kolejności preferencji.
