...
Aby uzyskać dalsze informacje dotyczące wdrażania Kubernetes na podstawie plików `kubectl` i oryginalnych `.yaml`, zobacz wpis post on [eShopOnContainers up in AKS (Azure Kubernetes Service)](https://github.com/dotnet-architecture/eShopOnContainers/wiki/10.-Setting-the-solution-up-in-AKS-(Azure-Kubernetes-Service)).
