...
Ścieżki, które zaczynają się ciągiem podstawienia `|DataDirectory|` są traktowane tak samo jak ścieżki względne. W przypadku ustawienia ścieżki są tworzone względem wartości właściwości domena aplikacji usługi DataDirectory.
...
| Wartość           | Opis                                                                                        |
| --------------- | -------------------------------------------------------------------------------------------------- |
| ReadWriteCreate | Otwiera bazę danych do odczytu i zapisu, a następnie tworzy ją, jeśli nie istnieje. Jest to domyślne ustawienie. |
...
| Wartość   | Opis                                                                                    |
| ------- | ---------------------------------------------------------------------------------------------- |
| Domyślny | Używa domyślnego trybu podstawowej biblioteki programu SQLite. Jest to domyślne ustawienie.                   |
| Private | Każde połączenie używa prywatnej pamięci podręcznej.                                                          |
| Współużytkowane  | Połączenia korzystają z pamięci podręcznej. Ten tryb pozwala zmienić zachowanie blokowania transakcji i tabel. |

### <a name="password"></a>Hasło

Klucz szyfrowania. Gdy jest określony, `PRAGMA key` jest wysyłana natychmiast po otwarciu połączenia.
...
| ciągiem | Nie wysyła `PRAGMA foreign_keys`. Jest to domyślne ustawienie. |
...
| Fałsz | Nie wysyła `PRAGMA recursive_triggers`. Jest to domyślne ustawienie.              |

## <a name="connection-string-builder"></a>Konstruktor parametrów połączenia

<xref:Microsoft.Data.Sqlite.SqliteConnectionStringBuilder> można użyć jako jednoznacznie określonego sposobu tworzenia parametrów połączenia. Można go również użyć, aby zapobiec atakom polegającym na iniekcji parametrów połączenia.
