...
## <a name="simple-windows-forms-applications"></a>Proste aplikacje formularzy systemu Windows  
 Co najmniej aplikacja Windows Forms składa się z następujących elementów:  
  
- Co najmniej jedna klasa, <xref:System.Windows.Forms.Form?displayProperty=nameWithType>która pochodzi od .  
  
- Metoda, `Main` która wywołuje `static` `shared` (w języku <xref:System.Windows.Forms.Application.Run%2A> Visual Basic) metoda i przekazuje wystąpienie <xref:System.Windows.Forms.Form> do niego. Metoda <xref:System.Windows.Forms.Application.Run%2A> przetwarza komunikaty z systemu operacyjnego do aplikacji.  
...
## <a name="using-controls-in-a-windows-forms-application"></a>Używanie formantów w aplikacji Windows Forms  
 Poniższy przykład kodu przedstawia prostą aplikację, która ilustruje, jak aplikacje Windows Forms używają formantów i obsługują zdarzenia. Przykład składa się z trzech przycisków w formularzu; każdy przycisk zmienia kolor tła po kliknięciu.  
