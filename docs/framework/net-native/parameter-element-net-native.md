...
|---------------|--------------------|-----------------|  
|`Name`|Ogólne|Atrybut wymagany. Nazwa parametru. Na przykład dla sygnatury `String.CompareTo(Object value)`metody wartość `Name` atrybutu to "value".|  
...
|Wartość|Opis|  
|-----------|-----------------|  
|*parameter_name*|Nazwa parametru metody, do którego zastosowano zasady. Na przykład dla sygnatury `String.CompareTo(Object value)`metody wartość `Name` atrybutu to "value".|  
...
## <a name="remarks"></a>Uwagi  
 Element jest elementem podrzędnym [ \<metody >](method-element-net-native.md) element i służy do zastosowania zasad do określonego parametru metody. `<Parameter>` Konkretny parametr metody jest określony według nazwy, a nie według typu. Co najmniej jeden atrybut reprezentujący typ zasad, taki jak `Activate` lub `Dynamic`, musi być obecny.  
