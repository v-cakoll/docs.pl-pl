...
Jeśli nie można załadować Projektant formularzy systemu Windows ze względu na błąd w kodzie, w składniku innej firmy lub w innym miejscu zobaczysz stronę błędu zamiast projektanta. Ta strona błędu niekoniecznie oznacza usterkę w projektancie. Usterka może znajdować się gdzieś na stronie powiązanej z kodem, która jest nazywana \<nazwą >. Designer.cs. Błędy pojawiają się w zwijanych, żółtych słupkach z linkiem umożliwiającym przechodzenie do lokalizacji błędu na stronie kodowej.
...
### <a name="name-already-exists-in-project-name"></a>"\<Name >" już istnieje w "\<nazwie projektu >"

Komunikat o błędzie: ""\<nazwa > "istnieje już w"\<nazwie projektu > ". Wprowadź unikatową nazwę ".
...
### <a name="an-exception-occurred-while-trying-to-create-an-instance-of-object-name"></a>Wystąpił wyjątek podczas próby utworzenia wystąpienia "\<obiektu nazwa >"

Komunikat o błędzie: "Wystąpił wyjątek podczas próby utworzenia wystąpienia"\<obiektu nazwa > ". Wyjątek miał wartość "\<ciągu wyjątków\>".
...
### <a name="another-editor-has-document-name-open-in-an-incompatible-mode"></a>Inny edytor ma "\<nazwie dokumentu >" otwarty w niezgodnym trybie

Komunikat o błędzie: "inny edytor ma\<nazwie dokumentu >" otwarty w niezgodnym trybie. Zamknij Edytor i spróbuj wykonać tę operację ponownie. "
...
Ten komunikat jest podobny do "inny edytor ma\<nazwie dokumentu >" otwarty w niezgodnym trybie ", ale program Visual Studio nie może określić nazwy pliku. Aby naprawić ten błąd, Zamknij Edytor, który otworzył plik, i spróbuj ponownie.

### <a name="array-rank-rank-in-array-is-too-high"></a>Ranga tablicy "\<rangi w tablicy >" jest zbyt wysoka
...
### <a name="assembly-assembly-name-could-not-be-opened"></a>Nie można otworzyć zestawu "\<Assembly Name >"

Komunikat o błędzie: nie można otworzyć "Assembly"\<nazwa zestawu > ". Sprawdź, czy plik nadal istnieje. "
...
### <a name="cannot-name-the-object-name-because-it-is-already-named-name"></a>Nie można nanosić nazwy obiektu "\<Name >", ponieważ ma już nazwę "\<nazwę >"
...
### <a name="category-toolbox-tab-name-does-not-have-a-tool-for-class-class-name"></a>Nazwa karty przybornika\<kategorii ">" nie ma narzędzia klasy "\<nazwa klasy >"
...
### <a name="code-generation-for-property-property-name-failed"></a>Generowanie kodu dla właściwości "\<właściwości >" nie powiodło się
...
### <a name="component-component-name-did-not-call-containeradd-in-its-constructor"></a>Składnik "\<składnik Name >" nie wywołał kontenera. Add () w jego konstruktorze
...
Aby naprawić ten błąd, skontaktuj się z dostawcą składnika. Lub, jeśli jest to składnik, który został utworzony, wywołaj metodę `IContainer.Add` w konstruktorze składnika.
...
### <a name="could-not-access-the-variable-variable-name-because-it-has-not-been-initialized-yet"></a>Nie można uzyskać dostępu do zmiennej "\<zmiennej Name >", ponieważ nie została jeszcze zainicjowana
...
### <a name="could-not-find-type-type-name"></a>Nie można znaleźć typu "\<typ nazwy >"

Komunikat o błędzie: "nie można odnaleźć typu"\<type name > ". Upewnij się, że zestaw zawierający ten typ jest przywoływany. Jeśli ten typ jest częścią projektu deweloperskiego, upewnij się, że projekt został pomyślnie skompilowany.
...
### <a name="could-not-load-type-type-name"></a>Nie można załadować typu "\<typ nazwy >"

Komunikat o błędzie: "nie można załadować typu\<nazwy typu >". Upewnij się, że zestaw zawierający ten typ jest dodawany do odwołań projektu.
...
### <a name="duplicate-declaration-of-member-member-name"></a>Zduplikowana Deklaracja elementu członkowskiego "\<nazwa elementu członkowskiego >"

Ten błąd występuje, ponieważ zmienna członkowska została zadeklarowana dwukrotnie (na przykład dwie kontrolki o nazwie `Button1` są zadeklarowane w kodzie). Nazwy muszą być unikatowe w różnych formularzach dziedziczonych. Ponadto nazwy nie mogą się różnić tylko wielkością liter.

### <a name="error-reading-resources-from-the-resource-file-for-the-culture-culture-name"></a>Błąd podczas odczytu zasobów z pliku zasobów dla kultury "\<nazwy kultury >"
...
Komunikat o błędzie: "nie można przeanalizować metody"\<nazwy metody > ". Analizator składni zgłosił następujący błąd: "\<ciągu błędu >". Zapoznaj się z Lista zadań, aby uzyskać potencjalne błędy ".
...
### <a name="the-assembly-assembly-name-could-not-be-found"></a>Nie można znaleźć zestawu "\<Assembly Name >"

Komunikat o błędzie: "nie można odnaleźć zestawu\<nazwie zestawu >". Upewnij się, że zestaw jest przywoływany. Jeśli zestaw jest częścią bieżącego projektu programistycznego, upewnij się, że projekt został skompilowany.

Ten błąd jest podobny do "typ\<nazwy typu >", ale ten błąd zwykle występuje z powodu atrybutu metadanych. Aby naprawić ten błąd, sprawdź, czy istnieją odwołania do wszystkich zestawów używanych przez atrybuty.
...
### <a name="the-base-class-class-name-cannot-be-designed"></a>Nie można zaprojektować klasy podstawowej "\<klasy >"
...
### <a name="the-base-class-class-name-could-not-be-loaded"></a>Nie można załadować klasy bazowej "\<nazwy klasy >"
...
### <a name="the-class-class-name-cannot-be-designed-in-this-version-of-visual-studio"></a>Nie można zaprojektować klasy "\<klasy >" w tej wersji programu Visual Studio
...
### <a name="the-component-cannot-be-added-because-it-contains-a-circular-reference-to-reference-name"></a>Nie można dodać składnika, ponieważ zawiera odwołanie cykliczne do "\<nazwy odwołania >"
...
### <a name="the-designer-for-base-class-class-name-is-not-installed"></a>Nie zainstalowano projektanta dla klasy podstawowej "\<klasy >"
...
### <a name="the-designer-must-create-an-instance-of-type-type-name-but-it-cant-because-the-type-is-declared-as-abstract"></a>Projektant musi utworzyć wystąpienie typu "\<typu Name >", ale nie można tego zrobić, ponieważ typ jest zadeklarowany jako abstrakcyjny
...
### <a name="the-language-parser-class-class-name-is-not-implemented-properly"></a>Klasa parsera języka "\<nazwa klasy >" nie została zaimplementowana prawidłowo

Komunikat o błędzie: "Klasa parsera języka"\<nazwa klasy > "nie została zaimplementowana prawidłowo. Skontaktuj się z dostawcą, aby uzyskać zaktualizowany moduł parsera ".
...
### <a name="the-object-object-name-does-not-implement-the-icomponent-interface"></a>Obiekt "\<obiektu Name >" nie implementuje interfejsu IComponent

Program Visual Studio próbował utworzyć składnik, ale utworzony obiekt nie implementuje interfejsu <xref:System.ComponentModel.IComponent>. Aby rozwiązać problem, skontaktuj się z dostawcą składnika.

### <a name="the-object-object-name-returned-null-for-the-property-property-name-but-this-is-not-allowed"></a>Obiekt "\<obiektu Name >" zwrócił wartość null dla właściwości "\<nazwy właściwości >", ale jest to niedozwolone
...
### <a name="the-service-service-name-is-required-but-could-not-be-located"></a>Wymagana jest usługa "\<Service Name >", ale nie można jej zlokalizować

Komunikat o błędzie: "usługa\<nazwa usługi >" jest wymagana, ale nie można jej zlokalizować. Może wystąpić problem z instalacją programu Visual Studio ".
...
### <a name="the-service-instance-must-derive-from-or-implement-interface-name"></a>Wystąpienie usługi musi dziedziczyć po lub zaimplementować "\<nazwy interfejsu >"
...
### <a name="the-toolbox-item-for-component-name-could-not-be-retrieved-from-the-toolbox"></a>Nie można pobrać elementu przybornika dla nazwy składnika "\<>" z przybornika

Komunikat o błędzie: "nie można pobrać elementu przybornika dla nazwy składnika\<>" z przybornika. Upewnij się, że zestaw zawierający element przybornika jest prawidłowo zainstalowany. Element przybornika zgłosił następujący błąd: \<ciągu błędu > ".
...
### <a name="the-toolbox-item-for-toolbox-item-name-could-not-be-retrieved-from-the-toolbox"></a>Nie można pobrać elementu przybornika dla nazwy elementu przybornika\<> "z przybornika
...
### <a name="the-type-type-name-could-not-be-found"></a>Nie można znaleźć typu "\<typu >"

Komunikat o błędzie: "nie można odnaleźć typu"\<typu > ". Upewnij się, że zestaw zawierający typ jest przywoływany. Jeśli zestaw jest częścią bieżącego projektu programistycznego, upewnij się, że projekt został skompilowany.
...
### <a name="the-variable-variable-name-is-either-undeclared-or-was-never-assigned"></a>Zmienna "\<zmiennej Name >" jest niezadeklarowana lub nigdy nie została przypisana
...
### <a name="there-is-already-a-command-handler-for-the-menu-command-menu-command-name"></a>Istnieje już procedura obsługi poleceń dla polecenia menu "\<nazwę polecenia menu >"
...
### <a name="there-is-already-a-component-named-component-name"></a>Istnieje już składnik o nazwie "\<składnik Name >"

Komunikat o błędzie: "istnieje już składnik o nazwie"\<składnik Name > ". Składniki muszą mieć unikatowe nazwy, a nazwy nie mogą być rozróżniane wielkości liter. Nazwa nie może również powodować konfliktu z nazwą żadnego składnika w dziedziczonej klasie. "
...
### <a name="type-type-name-does-not-have-a-constructor-with-parameters-of-types-parameter-type-names"></a>Typ "\<type name\>" nie ma konstruktora z parametrami typów "\<nazw typów parametrów >"
...
### <a name="unable-to-add-reference-reference-name-to-the-current-application"></a>Nie można dodać odwołania "\<nazwy odwołania >" do bieżącej aplikacji

Komunikat o błędzie: "nie można dodać odwołania"\<nazwę odwołania > "do bieżącej aplikacji. Sprawdź, czy nie ma już odwołania do innej wersji elementu "\<Reference Name >".
...
### <a name="unable-to-find-page-named-options-dialog-box-tab-name"></a>Nie można znaleźć strony o nazwie "\<opcje okna dialogowego Nazwa karty >"
...
### <a name="unable-to-find-property-property-name-on-page-options-dialog-box-tab-name"></a>Nie można znaleźć właściwości "\<nazwy właściwości >" na stronie "\<opcji w oknie dialogowym Opcje >"
...
### <a name="visual-studio-cannot-save-or-load-instances-of-the-type-type-name"></a>Program Visual Studio nie może zapisać ani załadować wystąpień typu "\<typu >"

Jest to problem związany ze składnikiem innej firmy. Skontaktuj się z dostawcą składnika.

### <a name="visual-studio-is-unable-to-open-document-name-in-design-view"></a>Program Visual Studio nie może otworzyć "\<nazwie dokumentu >" w widok Projekt

Komunikat o błędzie: "program Visual Studio nie może otworzyć"\<nazwie dokumentu > "w widok Projekt. Nie zainstalowano parsera dla typu pliku ".
...
### <a name="visual-studio-was-unable-to-find-a-designer-for-classes-of-type-type-name"></a>Program Visual Studio nie może odnaleźć projektanta dla klas typu "\<typu nazwy >"
