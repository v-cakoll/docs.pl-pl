...
> [!NOTE]
> Ponieważ zamrożony Freezable nie może być animowany, system animacji automatycznie utworzy modyfikowalne klony <xref:System.Windows.Freezable> zablokowanych obiektów podczas próby animowania ich <xref:System.Windows.Media.Animation.Storyboard>przy użyciu. Aby wyeliminować narzuty wydajności wynikające z klonowania, pozostaw obiekt niezamrożony, jeśli zamierzasz go animować. Aby uzyskać więcej informacji na temat animacji przy użyciu scenorysów, zobacz [Omówienie scenorysów](../graphics-multimedia/storyboards-overview.md).
