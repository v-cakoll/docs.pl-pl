---
title: 5007 — DCResolverResolve
ms.date: 03/30/2017
ms.assetid: 156d92a9-1de5-4ba8-a7a3-e8bf487c5523
ms.openlocfilehash: ed49fd308e5316f18dd0d7c9c89d2df758670a23
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
ms.locfileid: "33469203"
---
# <a name="5007---dcresolverresolve"></a>5007 — DCResolverResolve
## <a name="properties"></a>Właściwości  
  
|||  
|-|-|  
|ID|5007|  
|Słowa kluczowe|Serializacja|  
|Poziom|Pełny|  
|Kanał|Microsoft-Windows aplikacji debugowania serwera — aplikacje|  
  
## <a name="description"></a>Opis  
 To zdarzenie jest emitowany po uruchomieniu program rozpoznawania obiektów DataContract rozpoznawania.  
  
## <a name="message"></a>Komunikat  
 Program rozpoznawania obiektów DataContract rozpoznawanie %1 rozpoczęcia.  
  
## <a name="details"></a>Szczegóły
