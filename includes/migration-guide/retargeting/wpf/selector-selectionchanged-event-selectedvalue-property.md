...
---
### <a name="selector-selectionchanged-event-and-selectedvalue-property"></a>Zdarzenie SelectionChanged selektora i właściwość SelectedValue

|   |   |
|---|---|
|Szczegóły|Począwszy od .NET Framework 4.7.1 <xref:System.Windows.Controls.Primitives.Selector> zawsze aktualizuje wartość jego <xref:System.Windows.Controls.Primitives.Selector.SelectedValue%2A> właściwości przed podniesieniem <xref:System.Windows.Controls.Primitives.Selector.SelectionChanged> zdarzenia, gdy jego wybór ulegnie zmianie. Dzięki temu SelectedValue właściwość zgodne z<xref:System.Windows.Controls.Primitives.Selector.SelectedItem%2A> innymi <xref:System.Windows.Controls.Primitives.Selector.SelectedIndex%2A>właściwościami zaznaczenia ( i ), które są aktualizowane przed podniesieniem zdarzenia.<p/>W .NET Framework 4.7 i wcześniejszych wersjach aktualizacja do SelectedValue wystąpiła przed zdarzeniem w większości przypadków, ale <xref:System.Windows.Controls.Primitives.Selector.SelectedValue%2A> stało się to po zdarzeniu, jeśli zmiana wyboru została spowodowana przez zmianę właściwości.|
