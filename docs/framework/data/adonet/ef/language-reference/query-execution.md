...
### <a name="literals-and-parameters"></a>Literały i parametry  
 ...
 Parametry metody są również oceniane na kliencie. Parametr `orderID` , który został przesłany `MethodParameterExample` do metody, poniżej, jest przykładem.  
...
### <a name="casting-literals-on-the-client"></a>Rzutowanie literałów na kliencie  
 Rzutowanie `null` z na typ CLR jest wykonywane na kliencie:  
 ...
 Na kliencie jest wykonywane rzutowanie na typ, <xref:System.Decimal>na przykład na wartość null:  
...
- Wywołania funkcji lub metod, które znajdują się w zapytaniu LINQ to Entities, są mapowane na funkcje kanoniczne w Entity Framework, które następnie są tłumaczone na Transact-SQL i wykonywane w bazie danych SQL Server. Istnieją przypadki, gdy zachowanie tych mapowanych funkcji może różnić się od implementacji w bibliotekach klas bazowych. <xref:System.String.Contains%2A>Na przykład wywoływanie metod, <xref:System.String.StartsWith%2A>, i <xref:System.String.EndsWith%2A> z pustym ciągiem jako parametr `true` zwróci wartość, gdy zostanie wykonane w środowisku CLR, ale `false` zwróci się po wykonaniu w SQL Server. <xref:System.String.EndsWith%2A> Metoda może również zwracać różne wyniki, ponieważ SQL Server traktuje dwa ciągi, aby były równe, jeśli różnią się tylko znakami końcowymi, podczas gdy środowisko CLR uważa, że nie jest równe. Ilustruje to Poniższy przykład:  
