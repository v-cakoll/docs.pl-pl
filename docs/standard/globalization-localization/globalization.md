...
W tym artykule omówiono niektóre z najważniejszych problemów, które należy wziąć pod uwagę i najlepsze rozwiązania, które można zastosować podczas obsługi ciągów, dat i wartości czasu oraz wartości liczbowych w aplikacji zglobalizowanej.
...
Domyślnie .NET używa ciągów Unicode. Ciąg Unicode zawiera zero, co najmniej jeden <xref:System.Char> obiektów, z których każdy reprezentuje jednostkę kodu UTF-16. Brak reprezentację w formacie Unicode prawie każdy znak w każdym zestawie znaków stosowanym na świecie.
...
.NET używa tabel do zależnego kultury sortowania danych ciągów. Zawartość tych tabel, które zawierają dane dotyczące wag sortowania i normalizacji ciągów, jest określana przez wersję standard Unicode zaimplementowany przy użyciu określonej wersji programu .NET. Poniższa tabela zawiera listę wersji standardu Unicode implementowanych przez określone wersje programu .NET Framework i .NET Core. Należy zauważyć, że ta lista obsługiwanych wersji standardu Unicode dotyczy znaków porównywanie i sortowanie. nie ma zastosowania do klasyfikacji znaków Unicode w według kategorii. Aby uzyskać więcej informacji, zobacz sekcję "Ciągów i Unicode Standard" w <xref:System.String> artykułu.
...
|.NET framework 4.5, a później Windows 7|Unicode 5.0|
|.NET framework 4.5 i później w systemie Windows 8 i nowszych systemów operacyjnych|Unicode 6.3.0|
|.NET core (wszystkie wersje)|Zależy od wersji standardu Unicode obsługiwanych przez system operacyjny.|

Uruchamianie przy użyciu programu .NET Framework 4.5 i we wszystkich wersjach programu .NET Core, porównywanie i sortowanie ciągów zależy od systemu operacyjnego. .NET Framework 4.5 i nowszym, systemem Windows 7 pobiera dane z własnych tabel, które implementują standard Unicode 5.0. .NET Framework 4.5 i nowsze uruchomiony w systemie Windows 8 i nowszych pobiera dane z tabel systemu operacyjnego, które implementują Unicode 6.3. Na platformie .NET Core obsługiwana wersja Unicode zależy od zasadniczego systemu operacyjnego. Jeśli serializujesz posortowane dane wrażliwe na ustawienia kulturowe, można użyć <xref:System.Globalization.SortVersion> klasę, aby określić, gdy potrzebuje serializowane dane mają być sortowane, tak aby były zgodne z .NET i porządek sortowania systemu operacyjnego. Aby uzyskać przykład, zobacz <xref:System.Globalization.SortVersion> temat poświęcony klasie.
...
## <a name="handle-dates-and-times"></a>Obsługa daty i godziny
...
### <a name="persist-dates-and-times"></a>Utrwalanie daty i godziny
...
### <a name="serialization-and-time-zone-awareness"></a>Rozpoznawanie serializacji i strefy czasowej
...
### <a name="perform-date-and-time-arithmetic"></a>Wykonaj arytmetyka daty i godziny
...
### <a name="use-culture-sensitive-names-for-date-elements"></a>Użyj nazw wrażliwość na ustawienia kulturowe do elementów daty
...
> [!NOTE]
> Podczas analizowania i operacjach formatowania, .NET rozpoznaje tylko podstawowe znaki łacińskie od 0 do 9 (U + 0030 do U + 0039) jako cyfry.

### <a name="display-numeric-values"></a>Wyświetlanie wartości numerycznych
...
## <a name="work-with-culture-specific-settings"></a>Korzystanie z ustawień specyficznych dla kultury

Na platformie .NET <xref:System.Globalization.CultureInfo> klasa reprezentuje określoną kulturę lub region. Niektóre właściwości zwracają obiekty, które zawierają szczegółowe informacje dotyczące pewnych aspektów kultury:
...
- Pojedyncze wartości właściwości mogą się różnić w różnych wersjach aplikacji .NET lub systemu operacyjnego.

- .NET obsługuje kultury zamienne. Pozwala na zdefiniowanie nowej niestandardowej kultury, która stanowi uzupełnienie istniejących standardowych kultur lub całkowitą zamianę istniejącej standardowej kultury.

- W systemach Windows, użytkownik może dostosować ustawienia właściwe dla kultury za pomocą **Region i język** aplikacji w Panelu sterowania. Podczas tworzenia wystąpienia <xref:System.Globalization.CultureInfo> obiektu, można określić, czy odzwierciedla te modyfikacje użytkownika, wywołując <xref:System.Globalization.CultureInfo.%23ctor%28System.String%2CSystem.Boolean%29?displayProperty=nameWithType> konstruktora. Zazwyczaj dla aplikacji użytkownika końcowego należy przestrzegać preferencji użytkownika tak, aby użytkownik zobaczy z danymi w formacie, który użytkownik oczekuje.
