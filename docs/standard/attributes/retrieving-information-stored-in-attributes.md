...
## <a name="retrieving-a-single-instance-of-an-attribute"></a>Pobieranie pojedynczego wystąpienia atrybutu  
 W poniższym przykładzie `DeveloperAttribute` (opisane w poprzedniej sekcji) jest stosowany do klasy `MainApp` na poziomie klasy. Metoda `GetAttribute` używa metody **GetCustomAttribute** do pobierania wartości przechowywanych w `DeveloperAttribute` na poziomie klasy przed wyświetleniem ich w konsoli programu.  
...
## <a name="retrieving-multiple-instances-of-an-attribute-applied-to-the-same-scope"></a>Pobieranie wielu wystąpień atrybutu zastosowanych do tego samego zakresu  
 ...
 Jeśli nie zostaną znalezione żadne atrybuty, ten kod zgłosi użytkownika. W przeciwnym razie wyświetlane są informacje zawarte w obu wystąpieniach `DeveloperAttribute`.  
