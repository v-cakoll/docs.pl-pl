...
<xref:Microsoft.VisualBasic.PowerPacks.Printing.PrintForm> Składnika programu Visual Basic umożliwia drukowanie obraz formularza systemu Windows w czasie wykonywania. Jego zachowanie zastępuje te `PrintForm` metody we wcześniejszych wersjach programu Visual Basic.  
