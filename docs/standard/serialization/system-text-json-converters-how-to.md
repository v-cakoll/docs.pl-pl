...
W tym artykule pokazano, jak utworzyć niestandardowe konwertery dla klas serializacji JSON, które są dostępne <xref:[!OP.NO-LOC(System.Text.Json)]> w przestrzeni nazw. Wprowadzenie do `[!OP.NO-LOC(System.Text.Json)]`programu można znaleźć [w temacie jak serializować i deserializować kod JSON w programie .NET](system-text-json-how-to.md).

*Konwerter* jest klasą, która konwertuje obiekt lub wartość do i z formatu JSON. `[!OP.NO-LOC(System.Text.Json)]` Przestrzeń nazw ma wbudowane konwertery dla większości typów pierwotnych, które są mapowane na elementy pierwotne języka JavaScript. Możesz pisać niestandardowe konwertery:
...
Możesz również napisać niestandardowe konwertery, aby dostosować lub `[!OP.NO-LOC(System.Text.Json)]` zwiększyć funkcjonalność, która nie jest uwzględniona w bieżącej wersji. Poniższe scenariusze zostały omówione w dalszej części tego artykułu:
...
Istnieją dwa wzorce do tworzenia niestandardowego konwertera: wzorzec podstawowy i wzorzec fabryki. Wzorzec fabryki jest przeznaczony dla konwerterów, które `Enum` obsługują typ lub otwierają typy ogólne. Wzorzec podstawowy dotyczy typów ogólnych nieogólnych i zamkniętych.  Na przykład konwertery dla następujących typów wymagają wzorca fabryki:
...
Poniższy kod przedstawia niestandardowy konwerter, który współpracuje z `Dictionary<Enum,TValue>`. Kod jest zgodny z wzorcem fabryki, ponieważ pierwszy parametr typu ogólnego `Enum` to, a drugi jest otwarty. `CanConvert` Metoda `true` zwraca tylko dla `Dictionary` z dwoma parametrami ogólnymi, z których pierwszy jest `Enum` typem. Wewnętrzny konwerter pobiera istniejący konwerter do obsługi dowolnego typu w czasie wykonywania dla `TValue`.
...
* Utwórz klasę, która pochodzi od <xref:[!OP.NO-LOC(System.Text.Json)].Serialization.JsonConverter%601> elementu `T` WHERE, który ma być serializowany i deserializowany.
* Zastąp `Read` metodę w celu deserializacji przychodzącego pliku JSON i przekonwertuj go na typ `T`. Użyj <xref:[!OP.NO-LOC(System.Text.Json)].Utf8JsonReader> , która jest przenoszona do metody odczytu JSON.
* Zastąp `Write` metodę, aby serializować obiekt przychodzący `T`typu. Użyj <xref:[!OP.NO-LOC(System.Text.Json)].Utf8JsonWriter> , która jest przenoszona do metody, aby zapisać kod JSON.
...
Możesz odwołać się do [wbudowanego kodu źródłowego konwerterów](https://github.com/dotnet/runtime/tree/81bf79fd9aa75305e55abe2f7e9ef3f60624a3a1/src/libraries/[!OP.NO-LOC(System.Text.Json)]/src/[!OP.NO-LOC(System/Text/Json)]/Serialization/Converters/) jako implementacji odwołań do pisania konwerterów niestandardowych.
...
* Utwórz klasę, która pochodzi od <xref:[!OP.NO-LOC(System.Text.Json)].Serialization.JsonConverterFactory>.
* Zastąp `CanConvert` metodę, aby zwracał wartość true, gdy typ do przekonwertowania jest taki, który może obsłużyć konwerter. Na przykład, jeśli konwerter jest przeznaczony dla `List<T>` , może `List<int>`obsłużyć `List<string>`tylko, `List<DateTime>`i.
...
`Enum` Typ jest podobny do otwartego typu ogólnego: konwerter dla `Enum` programu musi utworzyć konwerter dla określonego `Enum` (`WeekdaysEnum`na przykład) w tle.

## <a name="error-handling"></a>Obsługa błędów

Jeśli musisz zgłosić wyjątek w kodzie obsługi błędu, rozważ przegenerowanie <xref:[!OP.NO-LOC(System.Text.Json)].JsonException> bez komunikatu. Ten typ wyjątku powoduje automatyczne utworzenie komunikatu zawierającego ścieżkę do części JSON, która spowodowała błąd. Na przykład instrukcja `throw new JsonException();` generuje komunikat o błędzie, jak w poniższym przykładzie:
...
Jeśli zostanie wyświetlony komunikat (na przykład `throw new JsonException("Error occurred")`, wyjątek nadal zapewnia ścieżkę we <xref:[!OP.NO-LOC(System.Text.Json)].JsonException.Path> właściwości.

## <a name="register-a-custom-converter"></a>Rejestrowanie niestandardowego konwertera

*Zarejestruj* konwerter niestandardowy, aby zastosować metody `Serialize` i `Deserialize` . Wybierz jedną z następujących metod:

* Dodaj wystąpienie klasy Converter do <xref:[!OP.NO-LOC(System.Text.Json)].JsonSerializerOptions.Converters?displayProperty=nameWithType> kolekcji.
* Zastosuj atrybut [[JsonConverter]](xref:[!OP.NO-LOC(System.Text.Json)].Serialization.JsonConverterAttribute) do właściwości, które wymagają konwertera niestandardowego.
* Zastosuj atrybut [[JsonConverter]](xref:[!OP.NO-LOC(System.Text.Json)].Serialization.JsonConverterAttribute) do klasy lub struktury, która reprezentuje niestandardowy typ wartości.

## <a name="registration-sample---converters-collection"></a>Przykład rejestracji — kolekcja konwerterów

Oto przykład, który sprawia <xref:System.ComponentModel.DateTimeOffsetConverter> , że domyślne właściwości typu: <xref:System.DateTimeOffset>
...
Tutaj kod, który tworzy strukturę i stosuje do niego `[JsonConverter]` atrybut:
...
Jeśli wiele konwerterów niestandardowych dla typu są zarejestrowane w `Converters` kolekcji, używany jest pierwszy konwerter zwracający wartość true `CanConvert` dla.
...
Konwerter może serializować i deserializować `TemperatureRanges` właściwości następującej klasy, która używa następujących `Enum`elementów:
...
[Folder testów jednostkowych](https://github.com/dotnet/runtime/blob/81bf79fd9aa75305e55abe2f7e9ef3f60624a3a1/src/libraries/System.Text.Json/tests/Serialization/) w kodzie `System.Text.Json.Serialization` źródłowym zawiera inne przykłady niestandardowego konwertera, takie jak:
