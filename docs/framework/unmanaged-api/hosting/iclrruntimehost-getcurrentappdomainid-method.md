...
Pobiera identyfikator liczbowy z <xref:System.AppDomain> obecnie jest wykonywana.  
...
## <a name="parameters"></a>Parametry  
 `pdwAppDomainId`  
 [out] Identyfikator numeryczny z <xref:System.AppDomain> obecnie jest wykonywana.  
  
## <a name="return-value"></a>Wartość zwracana  
  
|HRESULT|Opis|  
|-------------|-----------------|  
|S_OK|`GetCurrentAppDomainId` pomyślnie zwrócił.|  
...
## <a name="remarks"></a>Uwagi  
 `pdwAppDomainId` Parametr jest ustawiony na wartość <xref:System.AppDomain.Id%2A> właściwość <xref:System.AppDomain> w jest wykonywanie bieżącego wątku.  
