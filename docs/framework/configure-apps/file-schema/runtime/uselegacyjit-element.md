...
Nazwa elementu `useLegacyJit` uwzględnia wielkość liter.
...
- Ustawianie zmiennej środowiskowej
  ...
  Ustaw `COMPLUS_useLegacyJit` zmienną środowiskową na wartość `0` (Użyj nowego kompilatora 64-bitowego JIT) lub `1` (Użyj starszego, 64-bitowego kompilatora JIT):
