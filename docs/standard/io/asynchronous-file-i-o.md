...
W następnym przykładzie przedstawiono plik związany z kodem i plik XAML, który służy do otwierania pliku jako <xref:System.IO.Stream> w aplikacji ze sklepu Windows 8. x i odczytywania jego zawartości przy użyciu instancji klasy <xref:System.IO.StreamReader>. Metoda asynchroniczna umożliwia otwarcie pliku jako strumienia i odczytanie jego zawartości.
