...
>
> Ponadto `/r` i `/ct` dla typów odwołujących są przeznaczone do generowania kontraktów danych. Te przełączniki nie działają w przypadku używania elementu XmlSerializer.
...
|/out: plik\<>|Określa nazwę pliku dla wygenerowanego kodu.<br /><br /> Wartość domyślna: pochodna z nazwy definicji WSDL, nazwy usługi WSDL lub docelowej przestrzeni nazw jednego z schematów.<br /><br /> Krótka forma: `/o`|
|/Reference:\<ścieżka pliku >|Odwołuje się do typów w określonym zestawie. Podczas generowania klientów należy użyć tej opcji, aby określić zestawy, które mogą zawierać typy reprezentujące importowane metadane.<br /><br /> Nie można określić kontraktów komunikatów i typów <xref:System.Xml.Serialization.XmlSerializer> przy użyciu tego przełącznika.<br /><br /> Jeśli <xref:System.DateTimeOffset> odwołanie, ten typ jest używany zamiast generowania nowego typu. Jeśli aplikacja jest zapisywana przy użyciu .NET Framework 3,5, SvcUtil. exe odwołuje się automatycznie <xref:System.DateTimeOffset>.<br /><br /> Krótka forma: `/r`|
...
|/serializer:Auto|Automatycznie wybierz serializator. Spowoduje to próbę użycia serializatora kontraktu danych i użycie elementu XmlSerializer w razie niepowodzenia.<br /><br /> Krótka forma: `/ser`|
...
> [!NOTE]
> Gdy powiązanie usługi jest jednym z powiązań dostarczonych przez system (zobacz [powiązania dostarczone przez system](system-provided-bindings.md)), a właściwość <xref:System.ServiceModel.ServiceContractAttribute.ProtectionLevel%2A> jest ustawiona na `None` lub `Sign`, Svcutil generuje plik konfiguracji przy użyciu [\<niestandardowegobinding >](../configure-apps/file-schema/wcf/custombinding.md) zamiast oczekiwanego elementu określonego przez system. Na przykład jeśli usługa używa elementu `<wsHttpBinding>` z `ProtectionLevel` ustawioną na `Sign`, wygenerowana konfiguracja będzie `<customBinding>` w sekcji powiązania zamiast `<wsHttpBinding>`. Aby uzyskać więcej informacji na temat poziomu ochrony, zobacz [Opis poziomu ochrony](understanding-protection-level.md).
...
Domyślnie Svcutil. exe używa powiązań zdefiniowanych w klasie <xref:System.ServiceModel.Description.MetadataExchangeBindings>, aby wykonywać żądania MEX. Aby skonfigurować powiązanie używane dla WS-Metadata Exchange, należy zdefiniować punkt końcowy klienta w konfiguracji, który używa kontraktu kontraktem IMetadataExchange. Można go zdefiniować w pliku konfiguracji programu Svcutil. exe lub w innym pliku konfiguracyjnym określonym przy użyciu opcji `/svcutilConfig`.
...
|Opcja|Opis|
|------------|-----------------|
|/Reference:\<ścieżka pliku >|Dodaje określony zestaw do zestawu zestawów używanych do rozpoznawania odwołań do typu.<br /><br /> Krótka forma: `/r`|
|/excludeType: > typ\<|Określa w pełni kwalifikowaną lub kwalifikowaną dla zestawu nazwę typu, który ma zostać wykluczony z eksportu lub walidacji.<br /><br /> Krótka forma: `/et`|
|/out: plik\<>|Określa nazwę pliku dla wygenerowanego kodu. Ta opcja jest ignorowana, gdy wiele zestawów jest przenoszona jako dane wejściowe do narzędzia.<br /><br /> Wartość domyślna: pochodna na podstawie nazwy zestawu.<br /><br /> Krótka forma: `/o`|
|/UseSerializerForFaults|Określa, że <xref:System.Xml.Serialization.XmlSerializer>a powinna być używana do odczytu i zapisu błędów zamiast domyślnej <xref:System.Runtime.Serialization.DataContractSerializer>.|
...
Należy używać odpowiedniej listy Access Control (ACL) do ochrony folderu instalacyjnego programu Svcutil. exe, Svcutil. config i plików, które są wskazywane przez `/svcutilConfig`. Może to uniemożliwić rejestrowanie i uruchamianie złośliwych rozszerzeń.
