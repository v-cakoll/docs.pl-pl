...
Aby obejść problem <xref:System.Double> wartości sformatowane przy użyciu standardowego ciągu formatu liczbowego "R", który nie został pomyślnie zaokrąglony, w przypadku skompilowania za pomocą przełączników `/platform:x64` lub `/platform:anycpu` i uruchamiania w systemach 64-bitowych. można sformatować <xref:System.Double> wartości przy użyciu "G17" ciąg standardowego formatu liczbowego. Poniższy przykład używa ciągu formatu "R" z wartością <xref:System.Double>, która nie jest pomyślna, a także używa ciągu formatu "G17", aby pomyślnie zaokrąglić oryginalną wartość:
