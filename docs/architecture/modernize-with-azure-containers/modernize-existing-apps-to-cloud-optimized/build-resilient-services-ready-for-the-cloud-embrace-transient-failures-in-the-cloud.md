...
Rozważmy na przykład monolityczną aplikację .NET, która uzyskuje dostęp do bazy danych w Azure SQL Database. Jeśli baza danych Azure SQL lub jakakolwiek inna usługa nie odpowiada przez krótki czas (baza danych Azure SQL Database może zostać przeniesiona do innego węzła lub serwera i nie będzie odpowiadać przez kilka sekund), gdy użytkownik spróbuje wykonać dowolne działanie, aplikacja może ulec awarii i w tym samym czasie pokazywać wyjątek.
