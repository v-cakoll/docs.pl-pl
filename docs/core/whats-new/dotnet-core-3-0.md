...
Mimo że program .NET Core 3,0 obsługuje **.NET Standard 2,1**, domyślny szablon `dotnet new classlib` generuje projekt, który nadal jest celem **.NET Standard 2,0**. Aby docelowa **.NET Standard 2,1**, edytuj plik projektu i zmień właściwość `TargetFramework` na `netstandard2.1`:
...
Nowa Wbudowana obsługa JSON to wysoka wydajność, niska alokacja i oparta na `Span<byte>`. Do programu .NET Core 3,0 przestrzeń nazw <xref:System.Text.Json?displayProperty=nameWithType> dodano trzy nowe główne typy związane z JSON. Te typy nie obsługują *jeszcze* zwykłych serializacji i deserializacji obiektu CLR (POCO).
...
<xref:System.Text.Json.JsonSerializer?displayProperty=nameWithType> jest zbudowany w oparciu o <xref:System.Text.Json.Utf8JsonReader> i <xref:System.Text.Json.Utf8JsonWriter>, aby zapewnić szybką i niewielką pamięć podczas pracy z dokumentami i fragmentami JSON.
