...
---
# <a name="message-of-wshttpbinding"></a>\<> komunikatu > \<WSHttpBinding
Definiuje ustawienia zabezpieczeń [ \<](wshttpbinding.md)na poziomie wiadomości WSHttpBinding >.  
...
|clientCredentialType|Opcjonalny. Określa typ poświadczeń, które mają być używane podczas uwierzytelniania klienta przy użyciu trybu `Message` zabezpieczeń lub. `TransportWithMessageCredentials` Zobacz poniższe wartości wyliczenia. Wartość domyślna to `Windows`.<br /><br /> Ten atrybut jest typu <xref:System.ServiceModel.MessageCredentialType>.|  
