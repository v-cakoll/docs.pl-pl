...
```  
 ...
 Poniższy przykład jest taki sam jak powyżej, z wyjątkiem tego, że drzewo XML jest załadowane do <xref:System.Xml.Linq.XDocument>, a nie <xref:System.Xml.Linq.XElement>.  
...
```  
 ...
 Zwróć uwagę, że to samo zapytanie zwróciło jeden węzeł `Root` zamiast trzech węzłów podrzędnych.  
...
```  
 ...
 To zapytanie jest teraz wykonywane w taki sam sposób, jak zapytanie w drzewie w <xref:System.Xml.Linq.XElement>. Przykład generuje następujące dane wyjściowe:  
