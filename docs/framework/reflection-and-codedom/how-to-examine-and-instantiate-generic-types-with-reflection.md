...
1. Pobierz wystąpienie <xref:System.Type> reprezentujące typ ogólny. W poniższym kodzie, typ jest uzyskiwany przy użyciu operatora języka `typeof` C# (`GetType` w Visual Basic, `typeid` w Visual C++). Zapoznaj <xref:System.Type> się z tematem klasy, aby poznać <xref:System.Type> inne sposoby pobierania obiektu. Należy zauważyć, że w pozostałej części tej procedury typ jest zawarty w parametrze metody `t`o nazwie.  
...
1. Pobierz <xref:System.Type> obiekt, który reprezentuje typ ogólny. Poniższy kod pobiera <xref:System.Collections.Generic.Dictionary%602> typ ogólny na dwa różne sposoby: przy użyciu przeciążenia <xref:System.Type.GetType%28System.String%29?displayProperty=nameWithType> metody z ciągiem opisującym typ oraz przez wywołanie <xref:System.Type.GetGenericTypeDefinition%2A> metody dla konstruowanego typu `Dictionary\<String, Example>` (`Dictionary(Of String, Example)` w Visual Basic). <xref:System.Type.MakeGenericType%2A> Metoda wymaga definicji typu ogólnego.  
...
## <a name="example"></a>Przykład  
 Poniższy przykład kodu definiuje `DisplayGenericType` metodę, aby przeanalizować definicje typów ogólnych i skonstruowane typy używane w kodzie i wyświetlić ich informacje. `DisplayGenericType` Metoda pokazuje <xref:System.Type.IsGenericType%2A>, jak używać <xref:System.Type.IsGenericParameter%2A> <xref:System.Type.GenericParameterPosition%2A> właściwości, i i <xref:System.Type.GetGenericArguments%2A> metody.  
 ...
 Przykład tworzy typ z <xref:System.Collections.Generic.Dictionary%602> klasy przez utworzenie tablicy argumentów typu i wywołanie <xref:System.Type.MakeGenericType%2A> metody. Program porównuje <xref:System.Type> obiekt skonstruowany przy <xref:System.Type.MakeGenericType%2A> użyciu z <xref:System.Type> obiektem uzyskanym przy użyciu `typeof` (`GetType` w Visual Basic), pokazując, że są one takie same. Podobnie program używa <xref:System.Type.GetGenericTypeDefinition%2A> metody do uzyskania definicji typu ogólnego złożonego typu i porównuje go z <xref:System.Type> obiektem reprezentującym <xref:System.Collections.Generic.Dictionary%602> klasę.  
