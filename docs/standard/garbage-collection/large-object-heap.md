...
Moduł wyrzucania elementów bezużytecznych platformy .NET (GC) dzieli obiekty na małe i duże obiekty. Gdy obiekt jest duży, niektóre jego atrybuty stają się bardziej znaczące niż wtedy, gdy obiekt jest mały. Na przykład kompaktowanie polega na &mdash; tym, że kopiowanie go w pamięci w innym miejscu na stercie &mdash; może być kosztowne. W związku z tym moduł wyrzucania elementów bezużytecznych umieszcza duże obiekty na stosie dużego obiektu (LOH). W tym artykule omówiono, co kwalifikuje obiekt jako duży obiekt, jak zbierane są duże obiekty, oraz jaki rodzaj wydajności jest nakładany na duże obiekty.

> [!IMPORTANT]
> W tym artykule omówiono stertę dużego obiektu w .NET Framework i .NET Core działającą tylko w systemach Windows. Nie obejmuje to LOH działających w ramach implementacji platformy .NET na innych platformach.

## <a name="how-an-object-ends-up-on-the-loh"></a>Jak obiekt zostaje zakończony na LOH
...
.NET Core i .NET Framework (począwszy od .NET Framework 4.5.1) zawierają Właściwość umożliwiającą <xref:System.Runtime.GCSettings.LargeObjectHeapCompactionMode?displayProperty=nameWithType> użytkownikom określenie, że LOH należy kompaktować podczas następnego pełnego blokowania GC. W przyszłości platforma .NET może zdecydować się na automatyczne kompaktowanie LOH. Oznacza to, że w przypadku przydzielenia dużych obiektów i upewnienia się, że nie są one przenoszone, należy je nadal przypiąć.

Rysunek 1 ilustruje scenariusz, w którym formularze GC generacji 1 po pierwszej generacji 0 GC, gdzie `Obj1` i `Obj3` są martwe, a następnie generacji tworzy 2 po pierwszej generacji 1 `Obj2` `Obj5` . Należy zauważyć, że te i poniższe wartości są przeznaczone tylko do celów ilustracyjnych. zawierają one bardzo mało obiektów, aby lepiej zobaczyć, co się dzieje na stercie. W rzeczywistości wiele więcej obiektów jest zwykle używanych w wykazie globalnym.

![Rysunek 1: Gen 0 GC i gen 1 GC](media/loh/loh-figure-1.jpg)\
Rysunek 1: generacja 0 i generacja 1 GC.

Rysunek 2 pokazuje, że po 2. generacji GC, który wykaże, że `Obj1` i `Obj2` są martwe, system GC tworzy ciągłe wolne miejsce poza pamięcią, która `Obj1` `Obj2` została użyta w celu zaspokojenia żądania alokacji dla `Obj4` . Miejsce po ostatnim obiekcie, `Obj3` do końca segmentu może również służyć do zaspokojenia żądań alokacji.
...
Ogólnie rzecz biorąc, wykaz globalny występuje w jednym z następujących trzech warunków:
...
- <xref:System.GC.Collect%2A?displayProperty=nameWithType>Metoda jest wywoływana.
  ...
  Jeśli wywoływana jest metoda bez parametrów <xref:System.GC.Collect?displayProperty=nameWithType> lub inne Przeciążenie jest przenoszona <xref:System.GC.MaxGeneration?displayProperty=nameWithType> jako argument, LOH jest zbierane wraz z resztą zarządzanej sterty.
...
- Elementy tablicy z typami referencyjnymi.
  ...
  Zamiast odwoływać się do danych lewego węzła jako `left.d` , odwołuje się do niego jako `binary_tr[left_index].d` . A Moduł wyrzucania elementów bezużytecznych nie musi odwoływać się do żadnych odwołań dla lewego i prawego węzła.
...
## <a name="collect-performance-data-for-the-loh"></a>Zbieranie danych wydajności dla LOH
...
![Zrzut ekranu pokazujący Dodawanie liczników wydajności.](media/large-object-heap/add-performance-counter.png)
