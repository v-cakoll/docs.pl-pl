...
Narzędzie definicji schematu XML ([narzędzie definicji schematu XML (XSD. exe)](xml-schema-definition-tool-xsd-exe.md)) jest instalowane wraz z narzędziami .NET Framework w ramach &reg; zestawu Software Development Kit (SDK) dla systemu Windows. To narzędzie jest przeznaczony głównie do dwóch celów:  
