...
---
# <a name="plinq-data-sample"></a><span data-ttu-id="0b622-102">Próbka danych PLINQ</span><span class="sxs-lookup"><span data-stu-id="0b622-102">PLINQ Data Sample</span></span>
<span data-ttu-id="0b622-103">W tym przykładzie zawiera przykładowe dane w formacie CSV, wraz z metod, które przetwarzają je do kolekcji w pamięci dla klientów, produkty, zamówień i szczegóły zamówienia.</span><span class="sxs-lookup"><span data-stu-id="0b622-103">This sample contains example data in .csv format, together with methods that transform it into in-memory collections of Customers, Products, Orders, and Order Details.</span></span> <span data-ttu-id="0b622-104">Do dalszych eksperymentów z PLINQ, Wklej przykłady kodu z niektórych innych tematów, jak kod w tym temacie i wywołać go z `Main` metody.</span><span class="sxs-lookup"><span data-stu-id="0b622-104">To further experiment with PLINQ, you can paste code examples from certain other topics into the code in this topic and invoke it from the `Main` method.</span></span> <span data-ttu-id="0b622-105">Umożliwia także te dane za pomocą zapytań PLINQ.</span><span class="sxs-lookup"><span data-stu-id="0b622-105">You can also use this data with your own PLINQ queries.</span></span>  
  
 <span data-ttu-id="0b622-106">Dane reprezentuje podzbiór bazy danych Northwind.</span><span class="sxs-lookup"><span data-stu-id="0b622-106">The data represents a subset of the Northwind database.</span></span> <span data-ttu-id="0b622-107">50 (50), rekordy klientów są uwzględnione, ale nie wszystkie pola.</span><span class="sxs-lookup"><span data-stu-id="0b622-107">Fifty (50) customer records are included, but not all fields.</span></span> <span data-ttu-id="0b622-108">Dołączono podzestaw wierszy z zamówień i odpowiadające im dane Order_Detail dla każdego klienta.</span><span class="sxs-lookup"><span data-stu-id="0b622-108">A subset of the rows from the Orders and corresponding Order_Detail data for every Customer is included.</span></span> <span data-ttu-id="0b622-109">Wszystkie produkty są uwzględniane.</span><span class="sxs-lookup"><span data-stu-id="0b622-109">All Products are included.</span></span>  
  
> [!NOTE]
>  <span data-ttu-id="0b622-110">Zestaw danych nie jest wystarczająco duży, aby wykazać, że PLINQ jest szybsza niż LINQ do obiektów dla zapytań, które zawierają tylko podstawowe `where` i `select` klauzul.</span><span class="sxs-lookup"><span data-stu-id="0b622-110">The data set is not large enough to demonstrate that PLINQ is faster than LINQ to Objects for queries that contain just basic `where` and `select` clauses.</span></span> <span data-ttu-id="0b622-111">Aby obserwować, zwiększa szybkość w przypadku małych zestawów danych, takich jak to, należy użyć zapytań, które zawierają obliczeniowo kosztowne operacje dla każdego elementu w zestawie danych.</span><span class="sxs-lookup"><span data-stu-id="0b622-111">To observe speed increases for small data sets such as this, use queries that contain computationally expensive operations on every element in the data set.</span></span>  
  
### <a name="to-set-up-this-sample"></a><span data-ttu-id="0b622-112">Aby skonfigurować w tym przykładzie</span><span class="sxs-lookup"><span data-stu-id="0b622-112">To set up this sample</span></span>  
  
1. <span data-ttu-id="0b622-113">Utwórz projekt aplikacji konsoli Visual Basic lub Visual C#.</span><span class="sxs-lookup"><span data-stu-id="0b622-113">Create a Visual Basic or Visual C# console application project.</span></span>  
  
2. <span data-ttu-id="0b622-114">Zastąp zawartość Module1.vb lub Program.cs przy użyciu kodu, który obejmuje następujące kroki.</span><span class="sxs-lookup"><span data-stu-id="0b622-114">Replace the contents of Module1.vb or Program.cs by using the code that follows these steps.</span></span>  
  
3. <span data-ttu-id="0b622-115">Na **projektu** menu, kliknij przycisk **Dodaj nowy element**.</span><span class="sxs-lookup"><span data-stu-id="0b622-115">On the **Project** menu, click **Add New Item**.</span></span> <span data-ttu-id="0b622-116">Wybierz **plik tekstowy** a następnie kliknij przycisk **OK**.</span><span class="sxs-lookup"><span data-stu-id="0b622-116">Select **Text File** and then click **OK**.</span></span> <span data-ttu-id="0b622-117">Skopiuj dane w tym temacie, a następnie wklej go w nowy plik tekstowy.</span><span class="sxs-lookup"><span data-stu-id="0b622-117">Copy the data in this topic and then paste it in the new text file.</span></span> <span data-ttu-id="0b622-118">Na **pliku** menu, kliknij przycisk **Zapisz**, nadaj plikowi Plinqdata.csv, a następnie zapisz go w folderze, który zawiera pliki kodu źródłowego.</span><span class="sxs-lookup"><span data-stu-id="0b622-118">On the **File** menu, click **Save**, name the file Plinqdata.csv, and then save it in the folder that contains your source code files.</span></span>  
  
4. <span data-ttu-id="0b622-119">Naciśnij klawisz F5, aby zweryfikować, że projekt kompiluje i działa poprawnie.</span><span class="sxs-lookup"><span data-stu-id="0b622-119">Press F5 to verify that the project builds and runs correctly.</span></span> <span data-ttu-id="0b622-120">Następujące dane wyjściowe powinna być wyświetlana w oknie konsoli.</span><span class="sxs-lookup"><span data-stu-id="0b622-120">The following output should be displayed in the console window.</span></span>  
...
## <a name="data"></a><span data-ttu-id="0b622-121">Dane</span><span class="sxs-lookup"><span data-stu-id="0b622-121">Data</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="0b622-122">Zobacz także</span><span class="sxs-lookup"><span data-stu-id="0b622-122">See also</span></span>

- [<span data-ttu-id="0b622-123">Równoległe LINQ (PLINQ)</span><span class="sxs-lookup"><span data-stu-id="0b622-123">Parallel LINQ (PLINQ)</span></span>](../../../docs/standard/parallel-programming/parallel-linq-plinq.md)
