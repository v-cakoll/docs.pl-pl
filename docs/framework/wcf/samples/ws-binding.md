---
title: "Powiązania WS"
ms.custom: 
ms.date: 03/30/2017
ms.prod: .net-framework
ms.reviewer: 
ms.suite: 
ms.technology: dotnet-clr
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 629945b2-d771-46d7-a87b-5df3b2e460da
caps.latest.revision: "7"
author: Erikre
ms.author: erikre
manager: erikre
ms.openlocfilehash: d83ff828515a2399f3dae3c263b22ec91bb1fc6b
ms.sourcegitcommit: bd1ef61f4bb794b25383d3d72e71041a5ced172e
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 10/18/2017
---
# <a name="ws-binding"></a>Powiązania WS
Ta sekcja zawiera przykłady ilustrujące powiązania usługi systemu Windows w [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)].  
  
## <a name="in-this-section"></a>W tej sekcji  
 [Przepływ transakcji WS](../../../../docs/framework/wcf/samples/ws-transaction-flow.md)  
 Pokazuje użycie transakcji koordynowane przez klienta i opcje klienta i serwera przy użyciu protokołu WS-Atomic Transaction albo OleTransactions przepływu transakcji.  
  
 [WS 2007 Federation HTTP powiązania](../../../../docs/framework/wcf/samples/ws-2007-federation-http-binding.md)  
 Pokazuje użycie <xref:System.ServiceModel.WS2007FederationHttpBinding>.  
  
 [Podwójny WS Http](../../../../docs/framework/wcf/samples/ws-dual-http.md)  
 Pokazuje, jak skonfigurować `WSDualHttpBinding` powiązania.  
  
 [Kodowanie MTOM](../../../../docs/framework/wcf/samples/mtom-encoding.md)  
 Pokazuje użycie kodowania z WSHttpBinding komunikatu mechanizmu optymalizacji transmisji wiadomości (MTOM).  
  
 [WSHttpBinding](../../../../docs/framework/wcf/samples/wshttpbinding.md)  
 Pokazuje, jak do zaimplementowania typowych usługi i typowego klienta przy użyciu [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)].  
  
 [Sesja niezawodna WS](../../../../docs/framework/wcf/samples/ws-reliable-session.md)  
 Pokazuje użycie niezawodnej sesji.  
  
 [Zabezpieczenia transportu WS](../../../../docs/framework/wcf/samples/ws-transport-security.md)  
 Pokazuje użycie zabezpieczenia transportowe protokołu SSL z <xref:System.ServiceModel.WSHttpBinding> powiązania.  
  
 [Wiązanie zabezpieczeń komunikatów](../../../../docs/framework/wcf/samples/message-security-binding.md)  
 Pokazuje korzystanie z zabezpieczeń wiadomości w [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)].  
  
 [Transport WS z poświadczeniami komunikatu](../../../../docs/framework/wcf/samples/ws-transport-with-message-credential.md)  
 W przykładzie pokazano użycie zabezpieczenia transportowe protokołu SSL w połączeniu z poświadczeniami klienta odbywa się w komunikacie.
