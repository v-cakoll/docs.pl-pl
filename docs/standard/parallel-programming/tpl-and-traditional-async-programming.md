...
## <a name="wrapping-apm-operations-in-a-task"></a>Zawijanie operacji APM w zadaniu  
 ...
 Poniższy przykład przedstawia sygnaturę przeciążenia odpowiadającą `FromAsync` <xref:System.IO.FileStream.BeginRead%2A?displayProperty=nameWithType> <xref:System.IO.FileStream.EndRead%2A?displayProperty=nameWithType> metodom i. To Przeciążenie wymaga trzech parametrów wejściowych, jak pokazano poniżej.  
...
## <a name="exposing-complex-eap-operations-as-tasks"></a>Uwidacznianie złożonych operacji EAP jako zadań  
 ...
 Należy pamiętać, że każde zadanie, które jest tworzone przez program, <xref:System.Threading.Tasks.TaskCompletionSource%601> zostanie uruchomione przez ten TaskCompletionSource, dlatego kod użytkownika nie powinien wywoływać metody Start dla tego zadania.  
