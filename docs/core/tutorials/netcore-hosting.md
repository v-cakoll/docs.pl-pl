...
* Środowisko .NET Core może być również obsługiwane za pomocą `ICLRRuntimeHost4` interfejsu w [bibliotece mscoree. h](https://github.com/dotnet/runtime/blob/master/src/coreclr/src/pal/prebuilt/inc/mscoree.h). Ten interfejs API był dłuższy niż CoreClrHost. h, dlatego prawdopodobnie widzisz na nich starsze hosty. Nadal działa i zapewnia lepszą kontrolę nad procesem hostingu niż CoreClrHost. W większości scenariuszy, chociaż CoreClrHost. h jest preferowany teraz z powodu łatwiejszych interfejsów API.
...
Poniższe kroki szczegółowo opisują sposób użycia `nethost` `hostfxr` bibliotek i do uruchomienia środowiska uruchomieniowego platformy .NET Core w aplikacji natywnej i wywołania do zarządzanej metody statycznej. [Przykład](https://github.com/dotnet/samples/tree/master/core/hosting/HostWithHostFxr) używa `nethost` nagłówka i biblioteki zainstalowanej z zestawem SDK .NET i kopii [`coreclr_delegates.h`](https://github.com/dotnet/core-setup/blob/master/src/corehost/cli/coreclr_delegates.h) plików i w [`hostfxr.h`](https://github.com/dotnet/core-setup/blob/master/src/corehost/cli/hostfxr.h) repozytorium [dotnet/Core-Setup](https://github.com/dotnet/core-setup) .
...
Jak wspomniano wcześniej, CoreClrHost. h jest teraz preferowaną metodą hostowania środowiska uruchomieniowego platformy .NET Core. `ICLRRuntimeHost4`Można nadal używać interfejsu, ale jeśli interfejsy CoreClrHost. h nie są wystarczające (jeśli są potrzebne niestandardowe flagi uruchamiania, na przykład, lub jeśli element AppDomainManager jest wymagany w domenie domyślnej). Te instrukcje przeprowadzą Cię przez Hostowanie programu .NET Core przy użyciu biblioteki Mscoree. h.
...
`ICLRRuntimeHost4`Interfejs hostingu platformy .NET Core jest zdefiniowany w [bibliotece mscoree. IDL](https://github.com/dotnet/runtime/blob/master/src/coreclr/src/inc/MSCOREE.IDL). Wersja nagłówka tego pliku (mscoree. h), do którego należy odwołanie, jest tworzona za pośrednictwem MIDL podczas kompilowania [środowiska uruchomieniowego .NET Core](https://github.com/dotnet/runtime/) . Jeśli nie chcesz kompilować środowiska uruchomieniowego programu .NET Core, biblioteka mscoree. h jest również dostępna jako [wstępnie skompilowany nagłówek](https://github.com/dotnet/runtime/blob/master/src/coreclr/src/pal/prebuilt/inc/) w repozytorium dotnet/Runtime.
...
`ICLRRuntimeHost4`Interfejs hostingu jest pobierany przez wywołanie `GetProcAddress` (lub `dlsym` w systemie Linux/macOS) w systemie `GetCLRRuntimeHost` , a następnie wywołanie tej funkcji.
