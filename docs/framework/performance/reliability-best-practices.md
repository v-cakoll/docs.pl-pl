...
- Nigdy nie przerywaj współużytkowanego stanu domeny, co umożliwia bezproblemowe odtwarzanie <xref:System.AppDomain>.

Chociaż jest to teoretycznie możliwe, aby napisać kod zarządzany do obsługi <xref:System.Threading.ThreadAbortException>, <xref:System.StackOverflowException> i <xref:System.OutOfMemoryException> wyjątków, oczekujemy, że deweloperzy będą pisać takie niezawodne kod w całej aplikacji.  Z tego powodu wyjątki poza pasmem powodują przerwanie wykonywania wątku. a jeśli wątek przerywał edytowanie stanu udostępnionego, który można określić, czy wątek utrzymuje blokadę, a następnie <xref:System.AppDomain> zostanie zwolniony.  Gdy metoda, która edytuje stan udostępniony, zostanie zakończona, stan będzie uszkodzony, ponieważ nie jest możliwe zapisanie niezawodnego kodu zapasowego dla aktualizacji stanu udostępnionego.
...
Nie można zależeć od finalizatorów ani destruktorów ani bloków `try/finally` dla kodu zapasowego. Mogą być one przerywane lub niewywoływane.

Wyjątki asynchroniczne można zgłaszać w nieoczekiwanych lokalizacjach, prawdopodobnie każda instrukcja komputera: <xref:System.Threading.ThreadAbortException>, <xref:System.StackOverflowException> i <xref:System.OutOfMemoryException>.
...
W przypadku ograniczenia zamkniętej blokady lub zasobu, SQL Server przerywa wątek lub zrywa <xref:System.AppDomain>.  W takim przypadku należy uruchomić tylko kod zapasowy w ograniczonym regionie wykonywania (CER).

### <a name="use-safehandle-to-avoid-resource-leaks"></a>Użyj elementu SafeHandle, aby uniknąć przecieków zasobów

W przypadku <xref:System.AppDomain> zwalniania nie można zależeć od bloków `finally` lub finalizatorów, dlatego ważne jest, aby cały dostęp do zasobów systemu operacyjnego był przeprowadzany za pomocą klasy <xref:System.Runtime.InteropServices.SafeHandle>, a nie <xref:System.IntPtr>, <xref:System.Runtime.InteropServices.HandleRef> lub podobnych klas. Dzięki temu środowisko CLR będzie śledzić i zamykać wykorzystywane przez Ciebie uchwyty nawet w przypadku wyrywania <xref:System.AppDomain>.  <xref:System.Runtime.InteropServices.SafeHandle> będzie używać finalizatora krytycznego, który będzie zawsze uruchamiany przez środowisko CLR.

Dojście systemu operacyjnego jest przechowywane w bezpiecznym obsłudze od momentu jego utworzenia do momentu jego zwolnienia.  Nie ma okna, w którym <xref:System.Threading.ThreadAbortException> może spowodować przeciek dojścia.  Ponadto, wywołanie platformy będzie odwoływać się do dojścia, które umożliwia zamknięcie śledzenia okresu istnienia dojścia, uniemożliwiając problem z zabezpieczeniami z sytuacją wyścigu między `Dispose` a metodą, która aktualnie korzysta z dojścia.
...
Należy pamiętać, że <xref:System.Runtime.InteropServices.SafeHandle> nie zastępuje <xref:System.IDisposable.Dispose%2A?displayProperty=nameWithType>.  Nadal istnieją potencjalne korzyści związane z rywalizacją o zasoby i wydajność w celu jawnego usuwania zasobów systemu operacyjnego.  Wystarczy pamiętać, że bloki `finally`, które jawnie usuwają zasoby, mogą nie zostać wykonane do ukończenia.

<xref:System.Runtime.InteropServices.SafeHandle> umożliwia zaimplementowanie własnej metody <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A>, która wykonuje zadania w celu zwolnienia dojścia, na przykład przekazanie stanu do procedury zwalniania obsługi systemu operacyjnego lub zwolnienie zestawu dojść w pętli.  Środowisko CLR gwarantuje, że ta metoda jest uruchamiana.  Jest odpowiedzialny autor implementacji <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A>, aby upewnić się, że dojście zostanie wydane we wszystkich okolicznościach. Niewykonanie tej czynności spowoduje przeciek uchwytu, co często skutkuje wyciekiem zasobów natywnych skojarzonych z dojściem. W związku z tym ważne jest, aby struktura <xref:System.Runtime.InteropServices.SafeHandle> klas pochodnych w taki sposób, że implementacja <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A> nie wymaga przydziału żadnych zasobów, które mogą nie być dostępne w czasie wywołania. Należy zauważyć, że dozwolone jest wywoływanie metod, które mogą kończyć się niepowodzeniem w ramach implementacji <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A>, pod warunkiem, że kod może obsłużyć takie błędy i zakończyć kontrakt w celu zwolnienia uchwytu macierzystego. Na potrzeby debugowania <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A> ma wartość zwracaną <xref:System.Boolean>, która może być ustawiona na `false` w przypadku napotkania błędu krytycznego, który uniemożliwia wydanie zasobu. Wykonanie tej czynności spowoduje aktywowanie [releaseHandleFailed](../debug-trace-profile/releasehandlefailed-mda.md) MDA, jeśli jest włączone, aby pomóc w zidentyfikowaniu problemu. Nie ma to wpływu na środowisko uruchomieniowe w żaden inny sposób; <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A> nie zostanie wywołane ponownie dla tego samego zasobu, w związku z czym dojście zostanie ujawnione.

<xref:System.Runtime.InteropServices.SafeHandle> nie jest odpowiednia w niektórych kontekstach.  Ponieważ metoda <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A> może być uruchamiana w wątku <xref:System.GC> finalizatora, wszelkie uchwyty, które są wymagane do zwolnienia w określonym wątku, nie powinny być opakowane w <xref:System.Runtime.InteropServices.SafeHandle>.

Wywoływane otoki środowiska uruchomieniowego (RCW) mogą być czyszczone przez środowisko CLR bez dodatkowego kodu.  Dla kodu, który używa wywołania platformy i traktuje obiekt COM jako `IUnknown*` lub <xref:System.IntPtr>, należy ponownie napisać kod, aby użyć otoki RCW.  <xref:System.Runtime.InteropServices.SafeHandle> może nie być odpowiednie dla tego scenariusza ze względu na możliwość wywołania niezarządzanej metody wydania z powrotem do kodu zarządzanego.
...
klauzule `finally` nie są gwarantowane w przypadku uruchamiania poza programem CERs, co wymaga, aby deweloperzy biblioteki nie korzystali z kodu w bloku `finally` w celu zwolnienia niezarządzanych zasobów.  Zalecanym rozwiązaniem jest użycie <xref:System.Runtime.InteropServices.SafeHandle>.

#### <a name="code-analysis-rule"></a>Reguła analizy kodu

Użyj <xref:System.Runtime.InteropServices.SafeHandle> do czyszczenia zasobów systemu operacyjnego zamiast `Finalize`. Nie należy używać <xref:System.IntPtr>; Użyj <xref:System.Runtime.InteropServices.SafeHandle> do hermetyzacji zasobów. Jeśli klauzula finally musi być uruchomiona, należy ją umieścić w CER.
...
Środowisko CLR musi wiedzieć, gdy kod jest w blokadzie, dzięki czemu będzie wiadomo, że <xref:System.AppDomain>, a nie tylko przerwanie wątku.  Przerywanie wątku może być niebezpieczne, ponieważ dane obsługiwane przez wątek mogą pozostać w niespójnym stanie. W związku z tym cały <xref:System.AppDomain> musi być odtwarzany.  Skutkiem niepowodzenia identyfikacji blokady mogą być zakleszczenia lub nieprawidłowe wyniki. Użyj metod <xref:System.Threading.Thread.BeginCriticalRegion%2A> i <xref:System.Threading.Thread.EndCriticalRegion%2A>, aby zidentyfikować regiony blokady.  Są one statycznymi metodami klasy <xref:System.Threading.Thread>, które mają zastosowanie tylko do bieżącego wątku, co uniemożliwia jednemu wątkowi edytowanie liczby blokad innego wątku.

<xref:System.Threading.Monitor.Enter%2A> i <xref:System.Threading.Monitor.Exit%2A> mają wbudowane powiadomienie środowiska CLR, więc ich użycie jest zalecane, a także użycie [instrukcji lock](../../csharp/language-reference/keywords/lock-statement.md), która używa tych metod.
...
Oznacz i zidentyfikuj wszystkie blokady przy użyciu <xref:System.Threading.Thread.BeginCriticalRegion%2A> i <xref:System.Threading.Thread.EndCriticalRegion%2A>. Nie należy używać <xref:System.Threading.Interlocked.CompareExchange%2A>, <xref:System.Threading.Interlocked.Increment%2A> i <xref:System.Threading.Interlocked.Decrement%2A> w pętli.  Nie wykonuj wywołania platformy dla wariantów Win32 tych metod.  Nie należy używać <xref:System.Threading.Thread.Sleep%2A> w pętli.  Nie używaj pól nietrwałych.
...
Kod czyszczący nigdy nie powinien występować w bloku `catch`; powinna znajdować się w `finally` lub w bloku `catch`. Powinno to być normalne dobre rozwiązanie. Blok `finally` jest ogólnie preferowany, ponieważ ma ten sam kod zarówno w przypadku zgłoszenia wyjątku, jak i po wystąpieniu ostatniego bloku `try`.  W przypadku zgłaszania nieoczekiwanego wyjątku, na przykład <xref:System.Threading.ThreadAbortException>, kod czyszczący nie zostanie uruchomiony.  Wszystkie niezarządzane zasoby, które można oczyścić w `finally` należy zawinąć w <xref:System.Runtime.InteropServices.SafeHandle>, aby zapobiec wyciekom.  Zwróć uwagę C# , że słowo kluczowe `using` może być efektywnie używane do usuwania obiektów, w tym dojścia.

Mimo że odtwarzanie <xref:System.AppDomain> może czyścić zasoby w wątku finalizatora, nadal ważne jest, aby w prawidłowym miejscu umieścić kod czyszczący. Należy pamiętać, że jeśli wątek odbiera wyjątek asynchroniczny bez utrzymywania blokady, środowisko CLR próbuje zakończyć wątek bez konieczności odtwarzania <xref:System.AppDomain>.  Zapewnienie, że zasoby są czyszczone wcześniej, a nie w przyszłości, dzięki zwiększeniu liczby dostępnych zasobów i lepszym zarządzaniu okresem istnienia. Jeśli nie zamykasz jawnie dojścia do pliku w pewnej ścieżce kodu błędu, poczekaj, aż <xref:System.Runtime.InteropServices.SafeHandle> finalizator aby oczyścić, przy następnym uruchomieniu kodu może się nie powieść, próbując uzyskać dostęp do tego samego pliku, jeśli finalizator nie został jeszcze uruchomiony.  Z tego powodu, aby upewnić się, że kod czyszczący istnieje i działa prawidłowo, zapewni bardziej czyste i szybkie odzyskiwanie po błędach, nawet jeśli nie jest to absolutnie konieczne.
...
Zgodnie z opisem we wprowadzeniu, może być bardzo trudne, aby napisać kod zarządzany, który monitoruje stan współużytkowany w całej domenie aplikacji w niezawodny sposób.  Współużytkowany stan procesu jest dowolnym rodzajem struktury danych współdzielonym między domenami aplikacji, w kodzie Win32, wewnątrz środowiska CLR lub w zarządzanym kodzie przy użyciu komunikacji zdalnej.  Każdy modyfikowalny stan współużytkowany jest bardzo trudny do poprawnego zapisu w kodzie zarządzanym, a wszystkie statyczne Stany udostępnione mogą być wykonywane tylko z dużą ostrożnością.  Jeśli masz współużytkowany stan dla całego procesu lub całego komputera, Znajdź jakiś sposób, aby go wyeliminować lub chronić współużytkowany stan przy użyciu ograniczonego regionu wykonywania (CER).  Należy zauważyć, że każda biblioteka ze stanem udostępnionym, która nie została zidentyfikowana i poprawiona, może spowodować wystąpienie hosta, takiego jak SQL Server, które wymaga czystego <xref:System.AppDomain> zwalniania w przypadku awarii.
...
W przeszłości <xref:System.Threading.Monitor.Enter%2A> i [instrukcja Lock](../../csharp/language-reference/keywords/lock-statement.md) została użyta do utworzenia globalnych blokad procesów.  Na przykład zdarza się to w przypadku blokowania na <xref:System.AppDomain> metod agile, takich jak wystąpienia <xref:System.Type> z zestawów nieudostępnionych, <xref:System.Threading.Thread> obiekty, ciągi InterNIC i niektóre ciągi współużytkowane przez domeny aplikacji przy użyciu komunikacji zdalnej.  Te blokady nie są już cały proces.  Aby określić obecność blokady domeny międzyaplikacji obejmującej cały proces, ustal, czy kod w ramach blokady używa dowolnego zewnętrznego, utrwalonego zasobu, takiego jak plik na dysku lub prawdopodobnie baza danych.

Należy pamiętać, że wykonanie blokady w ramach <xref:System.AppDomain> może powodować problemy, Jeśli chroniony kod używa zasobu zewnętrznego, ponieważ ten kod może działać jednocześnie w wielu domenach aplikacji.  Może to być problem podczas zapisywania do jednego pliku dziennika lub powiązania do gniazda dla całego procesu.  Te zmiany oznaczają, że nie można w prosty sposób korzystać z kodu zarządzanego, aby uzyskać blokadę globalną procesu, inną niż użycie nazwanego <xref:System.Threading.Mutex> lub <xref:System.Threading.Semaphore>.  Utwórz kod, który nie działa jednocześnie w dwóch domenach aplikacji, lub użyj klas <xref:System.Threading.Mutex> lub <xref:System.Threading.Semaphore>.  Jeśli nie można zmienić istniejącego kodu, nie należy używać Win32 o nazwie mutex do osiągnięcia tej synchronizacji, ponieważ działa w trybie Fiber, nie można zagwarantować, że ten sam wątek systemu operacyjnego uzyska i zwolni element mutex.  Musisz użyć zarządzanej klasy <xref:System.Threading.Mutex> lub nazwanej <xref:System.Threading.ManualResetEvent>, <xref:System.Threading.AutoResetEvent> lub <xref:System.Threading.Semaphore>, aby zsynchronizować blokadę kodu w sposób, w jaki środowisko CLR ma świadomość, zamiast synchronizować blokadę przy użyciu kodu niezarządzanego.

#### <a name="avoid-locktypeofmytype"></a>Unikaj blokady (typeof (MyType))

Prywatne i publiczne <xref:System.Type> obiektów w zestawach współużytkowanych mają tylko jedną kopię kodu współużytkowaną we wszystkich domenach aplikacji.  Dla zestawów współużytkowanych istnieje tylko jedno wystąpienie <xref:System.Type> na proces, co oznacza, że wiele domen aplikacji współużytkuje dokładnie te same wystąpienia <xref:System.Type>.  W przypadku wystąpienia <xref:System.Type> blokada ma wpływ na cały proces, a nie tylko <xref:System.AppDomain>.  Jeśli jeden <xref:System.AppDomain> przyjmuje blokadę dla obiektu <xref:System.Type>, ten wątek zostanie nagle przerwany, nie zwolni blokady.  Ta blokada może spowodować zakleszczenie innych domen aplikacji.
...
Znaczna ilość istniejącego kodu nie używa <xref:System.GC.KeepAlive%2A>, gdy powinien lub używa go, gdy nie jest to odpowiednie.  Po konwersji na <xref:System.Runtime.InteropServices.SafeHandle> klasy nie muszą wywoływać <xref:System.GC.KeepAlive%2A>, przy założeniu, że nie mają finalizatora, ale polegają na <xref:System.Runtime.InteropServices.SafeHandle>, aby sfinalizować uchwyty systemu operacyjnego.  Chociaż koszt wydajności zachowywania wywołania <xref:System.GC.KeepAlive%2A> może być nieistotny, postrzeganie, że wywołanie <xref:System.GC.KeepAlive%2A> jest niezbędne lub wystarczające, aby rozwiązać problem z okresem istnienia, który może już nie istnieć, utrudnia utrzymywanie kodu.  Jednak w przypadku korzystania z wywoływanych otok CLR międzyoperacyjności języka COM (RCW) <xref:System.GC.KeepAlive%2A> jest nadal wymagana przez kod.
...
@No__t-0 (HPA) stanowi użycie deklaratywnych akcji zabezpieczeń do określenia wymagań dotyczących ochrony hosta, co pozwala hostowi zapobiec nawet w pełni zaufanym kodzie wywoływanie niektórych metod, które są nieodpowiednie dla danego hosta, takich jak <xref:System.Environment.Exit%2A> lub @no__ t-2 dla SQL Server.
...
Niezarządzana pamięć może być przeciekiem, podobnie jak dojście systemu operacyjnego. Jeśli to możliwe, spróbuj użyć pamięci na stosie przy użyciu [stackalloc](../../csharp/language-reference/operators/stackalloc.md) lub przypiętego obiektu zarządzanego, takiego jak [instrukcja FIXED](../../csharp/language-reference/keywords/fixed-statement.md) lub <xref:System.Runtime.InteropServices.GCHandle> przy użyciu typu Byte []. @No__t-0 ostatecznie czyści te wartości. Jeśli jednak musisz przydzielić pamięć niezarządzaną, rozważ użycie klasy, która pochodzi od <xref:System.Runtime.InteropServices.SafeHandle>, aby otoczyć alokację pamięci.

Należy zauważyć, że występuje co najmniej jeden przypadek, w którym <xref:System.Runtime.InteropServices.SafeHandle> nie jest odpowiednia. Dla wywołań metod COM, które przydzielą lub zwalniają pamięć, typowe dla jednej biblioteki DLL do przydzielenia pamięci za pośrednictwem `CoTaskMemAlloc` następnie inna Biblioteka DLL zwalnia pamięć z `CoTaskMemFree`.  Użycie <xref:System.Runtime.InteropServices.SafeHandle> w tych miejscach byłoby nieodpowiednie, ponieważ podejmie próbę powiązania okresu istnienia niezarządzanej pamięci z okresem istnienia <xref:System.Runtime.InteropServices.SafeHandle> zamiast zezwalać drugiej bibliotece DLL na okres istnienia pamięci.
...
Rozważ zmianę wszystkich miejsc, które przechwytują wszystkie wyjątki, aby przechwycić określony typ wyjątku, który będzie oczekiwany, na przykład <xref:System.FormatException> z metod formatowania ciągu.  Zapobiega to uruchamianiu bloku catch w przypadku nieoczekiwanych wyjątków i pomaga upewnić się, że kod nie ukrywa błędów przez przechwycenie nieoczekiwanych wyjątków.  Ponieważ ogólna reguła nigdy nie obsługuje wyjątku w kodzie biblioteki (kod, który wymaga przechwytywania wyjątku może wskazywać na przykład projektu w wywoływanym kodzie).  W niektórych przypadkach może być konieczne przechwycenie wyjątku i zgłoszenie innego typu wyjątku w celu zapewnienia większej ilości danych.  W takim przypadku należy użyć zagnieżdżonych wyjątków, przechowując rzeczywistą przyczynę niepowodzenia w <xref:System.Exception.InnerException%2A> właściwości nowego wyjątku.

#### <a name="code-analysis-rule"></a>Reguła analizy kodu

Przejrzyj wszystkie bloki catch w zarządzanym kodzie, który przechwytuje wszystkie obiekty lub przechwytuje wszystkie wyjątki.  W C#programie oznacza to Oflagowanie zarówno `catch` {} i `catch(Exception)` {}.  Rozważ, aby typ wyjątku był bardzo specyficzny, lub sprawdź kod, aby upewnić się, że nie działa w sposób niewłaściwy, jeśli przechwytuje nieoczekiwany typ wyjątku.
...
Zezwól na SQL Server personifikacji. Nie należy używać `RevertToSelf`, `ImpersonateAnonymousToken`, `DdeImpersonateClient`, `ImpersonateDdeClientWindow`, `ImpersonateLoggedOnUser`, `ImpersonateNamedPipeClient`, `ImpersonateSelf`, `RpcImpersonateClient`, `RpcRevertToSelf`, `RpcRevertToSelfEx` lub 0.
...
CER to określony blok `try/finally` bezpośrednio poprzedzony wywołaniem do <xref:System.Runtime.CompilerServices.RuntimeHelpers.PrepareConstrainedRegions%2A>.

Wykonanie tej czynności powoduje, że kompilator just in Time przygotowuje cały kod w bloku finally przed uruchomieniem bloku `try`. Gwarantuje to, że kod w bloku finally jest zbudowany i będzie działać we wszystkich przypadkach. Nie jest to sytuacja, w której plik CER ma pusty blok `try`. Używanie narzędzia CER chroni przed asynchronicznymi przerwami wątku i wyjątkami braku pamięci. Zobacz <xref:System.Runtime.CompilerServices.RuntimeHelpers.ExecuteCodeWithGuaranteedCleanup%2A>, aby uzyskać formularz CER, który dodatkowo obsługuje przepełnienie stosu przez przekroczenie głębokiego kodu.
