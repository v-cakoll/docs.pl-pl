...
- Otwórz *Module1. vb* lub *Program.cs* i Dodaj `Imports` lub `using` instrukcji dla `System.ComponentModel.Composition` i `System.ComponentModel.Composition.Hosting`. Te dwie przestrzenie nazw zawierają typy MEF, które będą potrzebne do opracowania rozszerzalnej aplikacji.
...
Teraz powinno być możliwe skompilowanie i uruchomienie projektu. W Visual Basic upewnij się, że dodano słowo kluczowe `Public` do `Module1`. W oknie konsoli wpisz operację dodawania, taką jak "5 + 3", a Kalkulator zwraca wyniki. Wszystkie inne operatory mają wynik "nie znaleziono operacji!" .
