...
### <a name="deferred"></a>Odroczone  
 Wykonanie odroczone oznacza to, czy operacja nie jest wykonywana w momencie w kodzie gdzie zapytanie jest zadeklarowany. Operacja jest wykonywana tylko wtedy, gdy wyliczone zmienną zapytania, na przykład za pomocą `foreach` instrukcji. Oznacza to, że wyniki wykonywania zapytania zależy od zawartości źródła danych podczas wykonywania zapytania zamiast kiedy zdefiniowano zapytania. Jeśli do zmiennej zapytania jest wyliczyć wiele razy, wyniki mogą być inne zawsze. Prawie wszystkie standardowe operatory zapytań o typie zwracanym <xref:System.Collections.Generic.IEnumerable%601> lub <xref:System.Linq.IOrderedEnumerable%601> w sposób opóźnieniem.  
