...
|<xref:System.Windows.Automation.Provider.IValueProvider.Value%2A>|Zobacz uwagi.|Właściwość ta zwróci zawartość tekstową kontrolki edycji. Jeśli `IsPasswordProperty` ustawiono `true`, ta właściwość musi wygenerować `InvalidOperationException` żądanie.|
