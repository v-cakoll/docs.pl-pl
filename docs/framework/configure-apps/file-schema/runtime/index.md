...
|Element|Opis|
|-------------|-----------------|
|[\<Dodaj >](add-element-for-namedcaches.md)|Dodaje nazwaną pamięć podręczną do kolekcji `namedCaches` dla pamięci podręcznej.|
...
|[\<AppContextSwitchOverrides >](appcontextswitchoverrides-element.md)|Definiuje co najmniej jeden przełącznik używany przez klasę <xref:System.AppContext>, aby zapewnić mechanizm rezygnacji dla nowych funkcji.|
...
|[\<Wyczyść >](clear-element-for-namedcaches.md)|Czyści kolekcję `namedCaches` pamięci podręcznej pamięci.|
|[Baza kodu \<](codebase-element.md)|Określa, gdzie środowisko uruchomieniowe może znaleźć zestaw.|
...
|[\<namedCaches >](namedcaches-element-cache-settings.md)|Zawiera kolekcję ustawień konfiguracji dla wystąpienia `namedCache`.|
...
|[\<NetFx45_CultureAwareComparerGetHashCode_LongStrings >](netfx45-cultureawarecomparergethashcode-longstrings-element.md)|Określa, czy środowisko uruchomieniowe używa stałej ilości pamięci do obliczenia kodów skrótów dla metody <xref:System.StringComparer.GetHashCode%2A?displayProperty=nameWithType>.|
...
|[\<Usuń >](remove-element-for-namedcaches.md)|Usuwa wpis nazwanej pamięci podręcznej z kolekcji `namedCaches` dla pamięci podręcznej.|
|[> środowiska uruchomieniowego \<](runtime-element.md)|Zawiera informacje o powiązaniu zestawu i zachowaniu wyrzucania elementów bezużytecznych.|
...
|[\<system. Runtime. buforowanie >](system-runtime-caching-element-cache-settings.md)|Zawiera informacje o konfiguracji domyślnej pamięci podręcznej obiektów w pamięci.|
...
|[\<TimeSpan_LegacyFormatMode >](runtime-element.md)|Określa, czy środowisko uruchomieniowe używa starszej wersji formatowania dla wartości <xref:System.TimeSpan>.|
