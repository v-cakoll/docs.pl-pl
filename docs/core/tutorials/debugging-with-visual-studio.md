...
1. Wybierz **debugowania** > **Step Into** lub naciśnij klawisz F11. Visual Studio wyróżnia instrukcji, która obejmuje `name` przypisanie zmiennej. **Automatycznych** okna wskazuje, że `name` jest `null` (w języku C#) lub `Nothing` (w języku Visual Basic), i w oknie konsoli wyświetla ciąg "Co to jest nazwa?".
