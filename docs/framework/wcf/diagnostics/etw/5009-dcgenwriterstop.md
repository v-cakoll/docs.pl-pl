---
title: 5009 — DCGenWriterStop
ms.date: 03/30/2017
ms.assetid: 8489e542-b2ab-4c31-9150-08d4f6704150
ms.openlocfilehash: a3eaa58e9fd66c49aca3979d652e26f5762d3ed1
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
ms.locfileid: "33467926"
---
# <a name="5009---dcgenwriterstop"></a>5009 — DCGenWriterStop
## <a name="properties"></a>Właściwości  
  
|||  
|-|-|  
|ID|5009|  
|Słowa kluczowe|Serializacja|  
|Poziom|Pełny|  
|Kanał|Microsoft-Windows aplikacji debugowania serwera — aplikacje|  
  
## <a name="description"></a>Opis  
 To zdarzenie jest emitowany po generowanie obiektu DataContract — powoduje zatrzymanie składnika zapisywania.  
  
## <a name="message"></a>Komunikat  
 Generowanie obiektu DataContract zatrzymania składnika zapisywania.  
  
## <a name="details"></a>Szczegóły
