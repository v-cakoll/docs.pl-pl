...
```
 ...
 W C# 1,1, usterka kompilatora umożliwia zdefiniowanie klasy, która ma niejawne konwersje zdefiniowane przez użytkownika do obu `int` i `bool`, i aby użyć bitowego operatora `AND` (`&`) dla obiektów tego typu. W C# 2,0, ta usterka została naprawiona w celu zapewnienia zgodności kompilatora C# ze specyfikacją, w związku z czym Poniższy kod będzie teraz powodował CS0034:
