---
title: 5404 — TokenValidationFailure
ms.date: 03/30/2017
ms.assetid: 19783336-5560-4334-be13-2efd15da3e2a
ms.openlocfilehash: 72e002ef9006633aeac97d4bb8e18edd5192025b
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
ms.locfileid: "33470227"
---
# <a name="5404---tokenvalidationfailure"></a>5404 — TokenValidationFailure
## <a name="properties"></a>Właściwości  
  
|||  
|-|-|  
|ID|5404|  
|Słowa kluczowe|Zabezpieczenia|  
|Poziom|Błąd|  
|Kanał|Microsoft-Windows aplikacji debugowania serwera — aplikacje|  
  
## <a name="description"></a>Opis  
 To zdarzenie jest emitowany podczas sprawdzania poprawności obiektu SecurityToken nie powiodło się.  
  
## <a name="message"></a>Komunikat  
 Nie można zweryfikować SecurityToken (typu '%1' i identyfikatorze "%2"). %3  
  
## <a name="details"></a>Szczegóły
