...
3. Dodaj odpowiadającą metodę do klasy w kodzie aplikacji i Zastosuj <xref:System.Data.Objects.DataClasses.EdmFunctionAttribute> do metody Zwróć uwagę, że parametry <xref:System.Data.Objects.DataClasses.EdmFunctionAttribute.NamespaceName%2A> i <xref:System.Data.Objects.DataClasses.EdmFunctionAttribute.FunctionName%2A> atrybutu są nazwą przestrzeni nazw modelu koncepcyjnego i nazwą funkcji w modelu koncepcyjnym piwo. Rozpoznawanie nazw funkcji dla LINQ jest rozróżniana wielkość liter.
...
Poniższy kod dodaje funkcję `AvgStudentGrade` do przykładowej bazy danych szkoły.
...
Następnie zadeklaruj funkcję w języku definicji schematu magazynu (SSDL) pliku *. edmx* . Poniższy kod deklaruje funkcję `AvgStudentGrade` w SSDL:
