...
Ten artykuł zawiera podejście krok po kroku dotyczące lokalizowania aplikacji. Najpierw należy przygotować aplikację tak, aby tekst, który zostanie przetłumaczony, mógł zostać wyodrębniony. Po przetłumaczeniu tekstu przetłumaczy przetłumaczony tekst na nową kopię oryginalnej aplikacji.
  
## <a name="create-a-sample-application"></a>Tworzenie przykładowej aplikacji

W tym kroku przygotowasz aplikację do lokalizacji. W przykładach Windows Presentation Foundation (WPF) dostarcza się próbkę HelloApp, która będzie używana w przykładach kodu w tej dyskusji. Jeśli chcesz użyć tego przykładu, Pobierz pliki Extensible Application Markup Language (XAML) z [przykładu narzędzia LocBaml](https://github.com/microsoft/WPF-Samples/tree/master/Tools/LocBaml).  
...
3. Dodaj UID do plików XAML. Identyfikatory UID są używane do śledzenia zmian w plikach i identyfikowania elementów, które muszą być przetłumaczone. Aby dodać UID do plików, uruchom polecenie `updateuid` w pliku projektu:  

   `msbuild -t:updateuid helloapp.csproj`

   Aby sprawdzić, czy nie ma żadnych lub zduplikowanych identyfikatorów UID, uruchom polecenie `checkuid` :  
...
Po skonfigurowaniu aplikacji do generowania zestawu satelickiego dla zasobów języka neutralnego należy skompilować aplikację. Spowoduje to wygenerowanie głównego zestawu aplikacji oraz zestawu satelickiego dla zasobów języka neutralnego, który jest wymagany przez LocBaml do lokalizacji.
...
2. Nowo utworzony zestaw aplikacji głównej (HelloApp. exe) jest tworzony w następującym folderze: *C:\HelloApp\Bin\Debug*
  
3. Nowo utworzony zestaw satelicki dla zasobów języka neutralnego, HelloApp. resources. dll, został utworzony w następującym folderze: *C:\HelloApp\Bin\Debug\en-US*
  
## <a name="build-the-locbaml-tool"></a>Tworzenie narzędzia LocBaml  
  
1. Wszystkie pliki niezbędne do kompilowania LocBaml znajdują się w przykładach WPF. Pobierz pliki C# z [przykładu narzędzia LocBaml](https://github.com/microsoft/WPF-Samples/tree/master/Tools/LocBaml).  
...
4. Opcje, które można określić, gdy uruchamiasz LocBaml są następujące.
   ...
   | Opcja | Opis|
   | - | - |
   | `parse` lub `-p` | Analizuje pliki BAML, zasoby lub biblioteki DLL w celu wygenerowania pliku CSV lub txt. |
   ...
   | `asmpath`lub `-asmpath` {*FileDirectory*] | Jeśli kod XAML zawiera niestandardowe kontrolki, należy podać `asmpath` do niestandardowego zestawu kontrolek. |
   | `nologo` | Nie wyświetla logo ani informacji o prawach autorskich. |
   | `verbose` | Wyświetla informacje o trybie informacji pełnej. |
  
   > [!NOTE]
   > Jeśli potrzebujesz listy opcji, gdy korzystasz z narzędzia, wpisz `LocBaml.exe` i naciśnij klawisz **Enter**.
...
3. Po uruchomieniu LocBaml do analizowania plików dane wyjściowe składają się z siedmiu pól rozdzielonych przecinkami (pliki CSV) lub tabulatorami (pliki. txt). Poniżej przedstawiono przeanalizowany plik CSV dla HelloApp. resources. dll:
   ...
   - **Nazwa BAML**. Nazwa zasobu BAML w odniesieniu do zestawu satelickiego dla języka źródłowego.  
  
   - **Klucz zasobu**. Zlokalizowany identyfikator zasobu.  
  
   - **Kategoria**. Typ wartości. Zobacz [atrybuty lokalizacji i komentarze](localization-attributes-and-comments.md).  
  
   - **Czytelność**. Określa, czy wartość może być odczytywana przez lokalizatora. Zobacz [atrybuty lokalizacji i komentarze](localization-attributes-and-comments.md).  
  
   - **Modifiability**. Czy wartość może być modyfikowana przez lokalizatora. Zobacz [atrybuty lokalizacji i komentarze](localization-attributes-and-comments.md).  
  
   - **Komentarze**. Dodatkowy opis wartości, aby pomóc w ustaleniu, jak jest lokalizowana wartość. Zobacz [atrybuty lokalizacji i komentarze](localization-attributes-and-comments.md).  
  
   - **Wartość**. Wartość tekstowa do przetłumaczenia na pożądaną kulturę.  
   ...
   |---------------|------------------|--------------|-----------------|-------------------|--------------|-----------|
   |HelloApp. g. pl-US. Resources: window1. BAML|Stack1: System. Windows. Controls. StackPanel. $Content|Zignoruj|FALSE|FALSE||#Text1; #Text2|
   |HelloApp. g. pl-US. Resources: window1. BAML|Tekst1: System. Windows. Controls. TextBlock. $Content|Brak|TRUE|TRUE||Witaj, świecie|
   |HelloApp. g. pl-US. Resources: window1. BAML|Tekst2: System. Windows. Controls. TextBlock. $Content|Brak|TRUE|TRUE||Na świecie|
...
Zawartość zidentyfikowana przez analizowanie HelloApp. resources. dll z LocBaml została przetłumaczona i należy ją scalić z powrotem do oryginalnej aplikacji. Użyj `generate` opcji lub, `-g` Aby wygenerować nowy plik resources. dll.  
...
## <a name="tips-for-using-locbaml"></a>Wskazówki dotyczące używania LocBaml  
