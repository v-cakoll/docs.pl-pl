...
Ten temat zawiera tabelę symboli i operatorów, które są używane w F# języka.

## <a name="table-of-symbols-and-operators"></a>Tabela symboli i operatorów

W poniższej tabeli przedstawiono symbole używane w F# język zawiera łącza do tematów, które zawierają więcej informacji i krótki opis niektórych zastosowań symbolu. Symbole są uporządkowane według zestawu, kolejności znaków ASCII.
...
|`!=`|Nie dotyczy.|<ul><li>Nie używany w F#. Użyj `<>` operacji nierówności.<br /></li></ul>|
...
|`==`|Nie dotyczy.|<ul><li>Nie używany w F#. Użyj `=` operacji porównania.<br /></li></ul>|
...
W poniższej tabeli przedstawiono kolejność pierwszeństwa operatorów i innych słów kluczowych wyrażenia, w F# język, w kolejności od najniższego pierwszeństwa, aby najwyższy priorytet. Na liście również jest łączność, jeśli ma to zastosowanie.
