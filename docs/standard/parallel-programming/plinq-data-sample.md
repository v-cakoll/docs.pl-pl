...
---
# <a name="plinq-data-sample"></a><span data-ttu-id="d3908-102">Próbka danych PLINQ</span><span class="sxs-lookup"><span data-stu-id="d3908-102">PLINQ Data Sample</span></span>
<span data-ttu-id="d3908-103">Ten przykład zawiera przykładowe dane w formacie csv wraz z metodami, które przekształcają je w kolekcje w pamięci klientów, produktów, zamówień i szczegółów zamówienia.</span><span class="sxs-lookup"><span data-stu-id="d3908-103">This sample contains example data in .csv format, together with methods that transform it into in-memory collections of Customers, Products, Orders, and Order Details.</span></span> <span data-ttu-id="d3908-104">Aby dalej eksperymentować z PLINQ, można wkleić przykłady kodu z niektórych innych `Main` tematów do kodu w tym temacie i wywołać go z metody.</span><span class="sxs-lookup"><span data-stu-id="d3908-104">To further experiment with PLINQ, you can paste code examples from certain other topics into the code in this topic and invoke it from the `Main` method.</span></span> <span data-ttu-id="d3908-105">Można również użyć tych danych z własnych zapytań PLINQ.</span><span class="sxs-lookup"><span data-stu-id="d3908-105">You can also use this data with your own PLINQ queries.</span></span>  
  
 <span data-ttu-id="d3908-106">Dane reprezentują podzbiór bazy danych Northwind.</span><span class="sxs-lookup"><span data-stu-id="d3908-106">The data represents a subset of the Northwind database.</span></span> <span data-ttu-id="d3908-107">Uwzględniono pięćdziesiąt (50) rekordów klientów, ale nie wszystkie pola.</span><span class="sxs-lookup"><span data-stu-id="d3908-107">Fifty (50) customer records are included, but not all fields.</span></span> <span data-ttu-id="d3908-108">Uwzględniono podzbiór wierszy z zamówienia i odpowiadające mu dane Order_Detail dla każdego Klienta.</span><span class="sxs-lookup"><span data-stu-id="d3908-108">A subset of the rows from the Orders and corresponding Order_Detail data for every Customer is included.</span></span> <span data-ttu-id="d3908-109">Wszystkie Produkty są wliczone w cenę.</span><span class="sxs-lookup"><span data-stu-id="d3908-109">All Products are included.</span></span>  
  
> [!NOTE]
> <span data-ttu-id="d3908-110">Zestaw danych nie jest wystarczająco duży, aby wykazać, że PLINQ jest `where` `select` szybszy niż LINQ do obiektów dla zapytań, które zawierają tylko podstawowe i klauzule.</span><span class="sxs-lookup"><span data-stu-id="d3908-110">The data set is not large enough to demonstrate that PLINQ is faster than LINQ to Objects for queries that contain just basic `where` and `select` clauses.</span></span> <span data-ttu-id="d3908-111">Aby obserwować wzrost prędkości dla małych zestawów danych, takich jak ten, należy użyć kwerend, które zawierają operacje kosztowne obliczeniowo na każdym elemencie w zestawie danych.</span><span class="sxs-lookup"><span data-stu-id="d3908-111">To observe speed increases for small data sets such as this, use queries that contain computationally expensive operations on every element in the data set.</span></span>  
  
### <a name="to-set-up-this-sample"></a><span data-ttu-id="d3908-112">Aby skonfigurować ten przykład</span><span class="sxs-lookup"><span data-stu-id="d3908-112">To set up this sample</span></span>  
  
1. <span data-ttu-id="d3908-113">Utwórz projekt aplikacji konsoli Visual Basic lub Visual C#.</span><span class="sxs-lookup"><span data-stu-id="d3908-113">Create a Visual Basic or Visual C# console application project.</span></span>  
  
2. <span data-ttu-id="d3908-114">Zastąp zawartość modułu Module1.vb lub Program.cs, używając kodu, który jest następujący.</span><span class="sxs-lookup"><span data-stu-id="d3908-114">Replace the contents of Module1.vb or Program.cs by using the code that follows these steps.</span></span>  
  
3. <span data-ttu-id="d3908-115">W menu **Projekt** kliknij polecenie **Dodaj nowy element**.</span><span class="sxs-lookup"><span data-stu-id="d3908-115">On the **Project** menu, click **Add New Item**.</span></span> <span data-ttu-id="d3908-116">Zaznacz **pozycję Plik tekstowy,** a następnie kliknij przycisk **OK**.</span><span class="sxs-lookup"><span data-stu-id="d3908-116">Select **Text File** and then click **OK**.</span></span> <span data-ttu-id="d3908-117">Skopiuj dane w tym temacie, a następnie wklej je do nowego pliku tekstowego.</span><span class="sxs-lookup"><span data-stu-id="d3908-117">Copy the data in this topic and then paste it in the new text file.</span></span> <span data-ttu-id="d3908-118">W menu **Plik** kliknij polecenie **Zapisz**, nazwij plik Plinqdata.csv, a następnie zapisz go w folderze zawierającym pliki kodu źródłowego.</span><span class="sxs-lookup"><span data-stu-id="d3908-118">On the **File** menu, click **Save**, name the file Plinqdata.csv, and then save it in the folder that contains your source code files.</span></span>  
  
4. <span data-ttu-id="d3908-119">Naciśnij klawisz F5, aby sprawdzić, czy projekt jest poprawnie uruchamiany i działa.</span><span class="sxs-lookup"><span data-stu-id="d3908-119">Press F5 to verify that the project builds and runs correctly.</span></span> <span data-ttu-id="d3908-120">Następujące dane wyjściowe powinny być wyświetlane w oknie konsoli.</span><span class="sxs-lookup"><span data-stu-id="d3908-120">The following output should be displayed in the console window.</span></span>  
...
## <a name="data"></a><span data-ttu-id="d3908-121">Dane</span><span class="sxs-lookup"><span data-stu-id="d3908-121">Data</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="d3908-122">Zobacz też</span><span class="sxs-lookup"><span data-stu-id="d3908-122">See also</span></span>

- [<span data-ttu-id="d3908-123">Równoległe LINQ (PLINQ)</span><span class="sxs-lookup"><span data-stu-id="d3908-123">Parallel LINQ (PLINQ)</span></span>](../../../docs/standard/parallel-programming/parallel-linq-plinq.md)
