...
## <a name="dynamic-compilation"></a>Dynamicznej kompilacji
 Jeśli chcesz zaimportować i używać <xref:System.Transactions> w aplikacji ASP.NET, która jest dynamicznie kompilowana w programie Access, należy umieścić odwołanie do zestawu <xref:System.Transactions> w pliku konfiguracji. Odwołanie należy dodać do sekcji **kompilacja** /**zestawy** w domyślnym głównym pliku konfiguracji **Web. config** lub w pliku konfiguracyjnym konkretnej aplikacji sieci Web. Poniższy przykład ilustruje to.
