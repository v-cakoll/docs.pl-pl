...
|`Switch.System.Runtime.InteropServices.`<br/>`DoNotMarshalOutByrefSafeArrayOnInvoke`|Określa, czy parametry ByRef SafeArray na zdarzenia międzyoperacyjności COM kierować do kodu natywnego (`false`) lub czy jest wyłączona kierowanie do kodu natywnego (`true`).|.NET Framework 4.8|
...
|`Switch.System.Threading.UseNetCoreTimer`|Formanty czy <xref:System.Threading.Timer> wystąpień zalet usprawnień wydajności w środowiskach o dużej skali. Jeśli `true`, ulepszenia wydajności są włączone; jeśli `false` (wartość domyślna), są wyłączone.|.NET Framework 4.8|
