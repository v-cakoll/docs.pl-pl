...
## <a name="overloaded-methods"></a>Metody przeciążane  
 Mimo że można tworzyć przeciążonej metody z [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)], nie są obsługiwane przez COM. Gdy klasa, która zawiera przeciążonej metody jest ujawniona jako obiekt COM, nowe nazwy metody są generowane dla przeciążonej metody.  
