...
## <a name="when-to-modify-smsvchostexeconfig"></a>Kiedy należy zmodyfikować plik SMSvcHost. exe. config  
 ...
 Czasami jednak może zajść potrzeba zmiany konfiguracji domyślnej dla usługi udostępniania portów Net. TCP. Na przykład wartość domyślna dla `maxPendingAccepts` to 4 * liczba procesorów. Serwery obsługujące dużą liczbę usług korzystających z udostępniania portów mogą zwiększyć tę wartość, aby osiągnąć maksymalną przepływność. Wartość domyślna dla `maxPendingConnections` to 100. Należy rozważyć zwiększenie tej wartości także wtedy, gdy istnieje wielu współbieżnych klientów wywołujących usługę, a usługa porzuca połączenia klientów.  
