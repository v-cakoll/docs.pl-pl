---
title: Metadane
ms.date: 03/30/2017
helpviewer_keywords:
- metadata [WCF]
ms.assetid: 66a61bd0-18d3-4e7e-bf8b-177a10ac2f60
ms.openlocfilehash: 10b93f94a5a5193513e20eed4571aaf8f6c882a5
ms.sourcegitcommit: 5b6d778ebb269ee6684fb57ad69a8c28b06235b9
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 04/08/2019
ms.locfileid: "59174021"
---
# <a name="metadata"></a>Metadane
Windows Communication Foundation (WCF) zapewnia infrastrukturę do eksportowania, publikowania, pobieranie i Importowanie metadanych usługi. Usługi WCF umożliwia metadane opisują sposób interakcji z punktami końcowymi usługi, tak aby narzędzi, takich jak Svcutil.exe, może automatycznie generować kod klienta do uzyskania dostępu do usługi.  
  
## <a name="in-this-section"></a>W tej sekcji  
 [Przegląd architektury metadanych](../../../../docs/framework/wcf/feature-details/metadata-architecture-overview.md)  
 Ogólny przegląd architektury metadanych.  
  
 [Formaty metadanych](../../../../docs/framework/wcf/feature-details/metadata-formats.md)  
 W tym artykule opisano formaty metadanych.  
  
 [Eksportowanie i importowanie metadanych](../../../../docs/framework/wcf/feature-details/exporting-and-importing-metadata.md)  
 Zawiera opis sposobu eksportowania i importowania metadanych.  
  
 [Publikowanie metadanych](../../../../docs/framework/wcf/feature-details/publishing-metadata.md)  
 W tym artykule opisano, jak WCF publikuje metadane.  
  
 [Pobieranie metadanych](../../../../docs/framework/wcf/feature-details/retrieving-metadata.md)  
 W tym artykule opisano różne sposoby, aby pobrać metadane.  
  
 [Używanie metadanych](../../../../docs/framework/wcf/feature-details/using-metadata.md)  
 W tym artykule opisano różne sposoby korzystania z metadanych usługi.  
  
 [Zagadnienia dotyczące zabezpieczeń obejmujące metadane](../../../../docs/framework/wcf/feature-details/security-considerations-with-metadata.md)  
 W tym artykule opisano ważne zagadnienia dotyczące zabezpieczeń podczas pracy z metadanych usługi.  
  
## <a name="see-also"></a>Zobacz także

- [Rozszerzanie systemu metadanych](../../../../docs/framework/wcf/extending/extending-the-metadata-system.md)
