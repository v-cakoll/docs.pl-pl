...
Importuj aliasu w `Imports` instrukcja zawiera co najmniej jeden znak typu identyfikator.  
 ...
 Alias importu musi być prawidłową nazwą Visual Basic. Dozwolonych znaków nie zawierać znaki typu identyfikator (`%`, `&`, `@`, `!`, `#`, i `$`). Zobacz temat[Zadeklarowane nazwy elementów](../../visual-basic/programming-guide/language-features/declared-elements/declared-element-names.md).  
