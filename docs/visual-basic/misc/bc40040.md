...
Ogólny typ jest oznaczony jako `<CLSCompliant(True)>`, ale ograniczenie na jednym z jego parametrów typu określa typ, który jest oznaczony jako `<CLSCompliant(False)>`, nie jest oznaczony jako lub nie kwalifikuje się, ponieważ jest to typ niezgodne.  
 ...
 Dla typu było zgodne z [niezależność od języka i elementy niezależne od języka](../../../docs/standard/language-independence-and-language-independent-components.md) (ze specyfikacją CLS), należy użyć tylko typów zgodnych ze specyfikacją CLS. Dotyczy to również ograniczenia dotyczące parametrów typu ogólnego typu.  
