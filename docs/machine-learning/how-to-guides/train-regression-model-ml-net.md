...
1. Pobierz dane szkoleniowe w formie `IDataView`
2. Tworzenie potoku uczenia jako sekwencja podstawowe "operatory" (aplikacjom).
3. Wywołaj `Fit` do potoku w celu uzyskania trenowanego modelu.

W tym [przykładowy plik](https://github.com/dotnet/machinelearning/tree/master/test/data/generated_regression_dataset.csv), przewidywany etykiety (`target`) jest ostatnią kolumnę (12) i wszystkie pozostałe funkcje:
