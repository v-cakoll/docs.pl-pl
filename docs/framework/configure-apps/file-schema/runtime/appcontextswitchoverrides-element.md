...
|`Switch.System.Activities.`<br/>`UseSHA1HashForDebuggerSymbols`|Określa, czy mieszania sumy kontrolnej przepływ pracy używa algorytmu SHA1 wprowadzone jako domyślne w programie .NET Framework 4.7 (`true`), lub czy używa domyślny algorytm SHA256 wprowadzone jako domyślne w .NET Framework 4.8 (`false`).|.NET Framework 4.8|
