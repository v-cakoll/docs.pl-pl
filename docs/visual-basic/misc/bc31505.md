---
title: "'<typename>' nie można użyć jako atrybutu, ponieważ nie ma atrybutu 'System.AttributeUsageAttribute'"
ms.date: 07/20/2015
f1_keywords:
- vbc31505
- bc31505
helpviewer_keywords:
- BC31505
ms.assetid: 7dd84c9d-6711-4dab-afc6-1fe4dee78051
ms.openlocfilehash: bf1e7aab9612c5e5fa1b18b2531bbf2cc009a087
ms.sourcegitcommit: cdf67135a98a5a51913dacddb58e004a3c867802
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 08/21/2019
ms.locfileid: "69661040"
---
# <a name="typename-cannot-be-used-as-an-attribute-because-it-does-not-have-a-systemattributeusageattribute-attribute"></a>'\<typename >' nie można użyć jako atrybutu, ponieważ nie ma atrybutu 'System.AttributeUsageAttribute'
Podjęto próbę użycia atrybutu, który został zadeklarowany bez `System.AttributeUsageAttribute` elementu do definiowania użycia.  
  
 **Identyfikator błędu:** BC31505  
  
## <a name="to-correct-this-error"></a>Aby poprawić ten błąd  
  
1. Atrybuty niestandardowe muszą być klasami pochodnymi `System.Attribute` , które `AttributeUsageAttribute` mają zastosowany atrybut.  
  
## <a name="see-also"></a>Zobacz także

- <xref:System.AttributeUsageAttribute>
- [Tworzenie atrybutów niestandardowych](../programming-guide/concepts/attributes/creating-custom-attributes.md)
