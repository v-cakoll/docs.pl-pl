...
-   W odróżnieniu od języków, które umożliwiają dziedziczenie wielokrotne [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] umożliwia tylko pojedyncze dziedziczenie z klas; oznacza to, klasy pochodne mogą mieć tylko jedną klasę podstawową. Mimo że wielokrotne dziedziczenie nie jest dozwolona w klasy, klasy można zaimplementować wiele interfejsów, które można skutecznie wykonać zakończenia tego samego.  
...
## <a name="inheritance-modifiers"></a>Modyfikatory dziedziczenia  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]wprowadzono następujące instrukcje poziomie klasy i Modyfikatory do obsługi dziedziczenia:  
