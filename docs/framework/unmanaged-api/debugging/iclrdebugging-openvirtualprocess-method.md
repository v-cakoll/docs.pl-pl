...
## <a name="parameters"></a>Parametry  
 ...
 `pVersion`  
 [in. out] Wersja środowiska CLR. Na wejściu ta wartość może być `null`. Może również wskazywać na strukturę [CLR_DEBUGGING_VERSION](clr-debugging-version-structure.md) , w którym przypadku pole `wStructVersion` struktury musi być inicjowane na wartość 0 (zero).  
