...
|`group`|Wymagany. A variable with a type that's a collection type or Object. Refers to the collection over which the `statements` are to be repeated.|
|`statements`|Opcjonalny. One or more statements between `For Each` and `Next` that run on each item in `group`.|
|`Continue For`|Opcjonalny. Transfers control to the start of the `For Each` loop.|
|`Exit For`|Opcjonalny. Transfers control out of the `For Each` loop.|
|`Next`|Wymagany. Terminates the definition of the `For Each` loop.|
