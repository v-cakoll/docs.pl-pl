...
> [!NOTE]
> Nie ma możliwości pobrania właściwości XmlResolver . Pozwala to zapobiec użyciu przez użytkownika elementu XmlResolver, na którym zostały ustawione poświadczenia. Ponadto jeśli **XmlTextReader** lub Walidacja <xref:System.Xml.XmlReader> jest używana do załadowania **dokumentu** XmlDocument, a element XmlDocument ma skonfigurowany mechanizm rozwiązywania konfliktów, nie są one buforowane przez XmlDocument po  **Faza obciążenia** , ponieważ stanowi to również zagrożenie bezpieczeństwa.  
  
 Aby uzyskać więcej informacji, zobacz sekcję <xref:System.Xml.XmlResolver> uwagi na stronie referencyjnej.  
