...
## <a name="images"></a>Obrazy  
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]obrazowanie zapewnia znaczną poprawę w stosunku do możliwości przetwarzania obrazu w poprzednich wersjach systemu Windows. Funkcje obrazowania, takie jak wyświetlanie mapy bitowej lub używanie obrazu na wspólnej formancie, były obsługiwane głównie przez interfejs GDI (Microsoft Windows Graphics Device Interface) lub microsoft Windows GDI+ (API). Te interfejsy API zapewniały podstawowe funkcje obrazowania, ale brakowało funkcji, takich jak obsługa rozszerzalności kodeka i obsługa obrazu o wysokiej wierności. Interfejsy API programu WPF Imaging zostały przeprojektowane, aby przezwyciężyć niedociągnięcia interfejsów GDI i GDI+ i udostępnić nowy zestaw interfejsów API do wyświetlania i używania obrazów w aplikacjach.  
