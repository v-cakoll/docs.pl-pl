---
title: Binding1
ms.date: 03/30/2017
ms.assetid: 324fa627-d012-465a-b266-95594a09dac2
ms.openlocfilehash: eb5e3e66e21c0552ce267673c17308765a932ce4
ms.sourcegitcommit: 15109844229ade1c6449f48f3834db1b26907824
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/07/2018
---
# <a name="binding"></a>Powiązanie
Ta sekcja zawiera przykłady ilustrujące przy użyciu atrybutu powiązania elementu punktu końcowego.  
  
## <a name="in-this-section"></a>W tej sekcji  
 [Koder elementu ByteStream](../../../../docs/framework/wcf/samples/bytestream-encoder.md)  
 Przedstawia sposób tworzenia `ByteStreamHttpBinding`, <xref:System.ServiceModel.Channels.Binding> który prezentuje działanie funkcji kodera strumienia bajtów.  
  
 [Podstawowe powiązanie](../../../../docs/framework/wcf/samples/basic-binding.md)  
 Przedstawia podstawowe powiązanie w programie WCF.  
  
 [Powiązanie niestandardowe](../../../../docs/framework/wcf/samples/custom-binding.md)  
 Demonstracja niestandardowego powiązania w programie WCF.  
  
 [Powiązanie sieci](../../../../docs/framework/wcf/samples/net-binding.md)  
 Pokazuje powiązanie sieci w programie WCF.  
  
 [Powiązanie WS](../../../../docs/framework/wcf/samples/ws-binding.md)  
 Pokazuje powiązania usługi systemu Windows w programie WCF.
