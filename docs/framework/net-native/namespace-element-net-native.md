...
---
# <a name="namespace-element-net-native"></a>\<Przestrzeń nazw > element (.NET Native)
...
|Wartość|Opis|  
|-----------|-----------------|  
|*namespace_name*|Nazwa przestrzeni nazw. [ \<](library-element-net-native.md) [ \<](assembly-element-net-native.md) [ \<](application-element-net-native.md)Jeśli przestrzeń nazw > element jest elementem podrzędnym > aplikacji, biblioteki > lub elementu > zestawu, namespace_name musi być w pełni kwalifikowana \< nazwa przestrzeni nazw. Jeśli przestrzeń nazw > element jest elementem podrzędnym innej \<przestrzeni nazw > elementu, namespace_name musi być względną nazwą przestrzeni nazw. \<|  
  
## <a name="all-other-attributes"></a>Wszystkie inne atrybuty  
  
|Wartość|Opis|  
|-----------|-----------------|  
|*policy_setting*|Ustawienie, które ma zostać zastosowane do tego typu zasad dla wszystkich typów w przestrzeni nazw. Możliwe wartości to `All`, `Auto` `Excluded` ,,`Public` ,,`Required All`, i. `PublicAndInternal` `Required Public` `Required PublicAndInternal` Aby uzyskać więcej informacji, zobacz [Ustawienia zasad dyrektywy środowiska uruchomieniowego](runtime-directive-policy-settings.md).|  
...
|Element|Opis|  
|-------------|-----------------|  
|[\<> Aplikacji](application-element-net-native.md)|Służy jako kontener dla typów i składowych dla całej aplikacji, których metadane są dostępne do odbicia w czasie wykonywania. Element > [ \<](assembly-element-net-native.md) aplikacji może mieć zero, jeden lub więcej elementów > zestawu. [ \<](application-element-net-native.md)|  
...
|[\<> Biblioteki](library-element-net-native.md)|Definiuje zestaw zawierający typy i elementy członkowskie typu, których metadane są dostępne do odbicia w czasie wykonywania. Biblioteka > element może mieć zero lub jeden [ \<zestaw >](assembly-element-net-native.md) elementu. [ \<](library-element-net-native.md)|  
|`<Namespace>`|Stosuje zasady odbicia do wszystkich typów w nadrzędnej przestrzeni nazw.|  
  
## <a name="remarks"></a>Uwagi  
 Atrybuty `Activate`, `Browse`, `Dynamic`i sąopcjonalne.`Serialize` Jeśli żaden nie jest obecny, `<Namespace>` element służy tylko jako kontener dla elementów podrzędnych. Jeśli są obecne, `<Namespace>` element stosuje zasady odbicia środowiska uruchomieniowego do wszystkich typów w określonym obszarze nazw.  
  
 Gdy jest elementem podrzędnym [ \<zestawu >](assembly-element-net-native.md) elementu, `<Namespace>` element zastępuje zasady odbicia środowiska uruchomieniowego zdefiniowane przez [ \<element > zestawu](assembly-element-net-native.md) .  
