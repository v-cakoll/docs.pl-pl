...
---
# <a name="how-to-use-datacontractjsonserializer"></a>Jak używać Klasa DataContractJsonSerializer

> [!NOTE]
> Ten artykuł ma informacje o programie <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> . W przypadku większości scenariuszy obejmujących serializację i deserializacja kodu JSON zalecamy używanie interfejsów API wSystem.Text.Jsw [przestrzeni nazw](../../../standard/serialization/system-text-json-overview.md).
...
W tym artykule pokazano, jak serializować obiekty typu .NET do danych zakodowanych w formacie JSON, a następnie deserializować dane z powrotem do wystąpień typów .NET. Ten przykład używa kontraktu danych do zademonstrowania serializacji i deserializacji typu zdefiniowanego przez użytkownika `Person` i użycia <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> .
...
2. Serializacja `Person` obiektu do strumienia pamięci za pomocą <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> .
...
3. Użyj <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer.WriteObject%2A> metody, aby zapisać dane JSON w strumieniu.
...
1. Deserializacja danych zakodowanych w formacie JSON do nowego wystąpienia przy `Person` użyciu <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer.ReadObject%2A> metody <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> .
