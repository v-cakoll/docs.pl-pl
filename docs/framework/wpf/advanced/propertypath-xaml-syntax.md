...
Niektóre właściwości stylu i szablonu, takie <xref:System.Windows.Setter.Property%2A?displayProperty=nameWithType> jak Take kwalifikowana nazwa właściwości, która jest podobna do. <xref:System.Windows.PropertyPath> Nie jest to jednak wartość true <xref:System.Windows.PropertyPath>; zamiast tego jest to kwalifikowana *właściciel.* użycie formatu ciągu właściwości, które jest włączone przez procesor [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] WPF w połączeniu z konwerterem typów dla <xref:System.Windows.DependencyProperty>.
...
Należy zauważyć, że powiązanie [!INCLUDE[TLA#tla_xml](../../../../includes/tlasharptla-xml-md.md)] danych z nie <xref:System.Windows.PropertyPath>używa, ponieważ nie jest używane <xref:System.Windows.Data.Binding.Path%2A> w <xref:System.Windows.Data.Binding>. Zamiast tego należy użyć <xref:System.Windows.Data.Binding.XPath%2A> i określić prawidłową składnię XPath [!INCLUDE[TLA#tla_xmldom](../../../../includes/tlasharptla-xmldom-md.md)] do danych. <xref:System.Windows.Data.Binding.XPath%2A>jest również określony jako ciąg, ale nie jest tutaj udokumentowany. Zobacz [powiąż z danymi XML przy użyciu XmlDataProvider i zapytań XPath](../data/how-to-bind-to-xml-data-using-an-xmldataprovider-and-xpath-queries.md).
...
Funkcja *PropertyName* musi zostać rozpoznana jako nazwa właściwości, która znajduje się w bieżącej <xref:System.Windows.FrameworkElement.DataContext%2A> <xref:System.Windows.Data.Binding.Path%2A> wartości użycia. Jeśli powiązanie aktualizuje źródło, ta właściwość musi mieć wartość odczyt/zapis, a obiekt źródłowy musi być modyfikowalny.
...
`propertyName` Aby<xref:System.Windows.DependencyProperty>można było użyć jako elementu docelowego scenorysu, właściwość określona jako musi być.
...
Składnia/w tej składni jest używana do nawigowania w obrębie hierarchicznego obiektu źródła danych, a wiele kroków do hierarchii z kolejnymi/znakami są obsługiwane. Źródłowe konta przechodzenia dla bieżącego położenia wskaźnika rekordu, które są określane przez synchronizację danych z interfejsem użytkownika widoku. Aby uzyskać szczegółowe informacje na temat powiązania z obiektami hierarchicznych źródeł danych i koncepcji bieżącego wskaźnika rekordu w powiązaniu danych, zobacz [Korzystanie ze wzorca wzorzec-szczegóły z danymi hierarchicznymi lub z](../data/how-to-use-the-master-detail-pattern-with-hierarchical-data.md) [omówieniem powiązań danych](../data/data-binding-overview.md).

> [!NOTE]
> Ta składnia jest podobna [!INCLUDE[TLA2#tla_xpath](../../../../includes/tla2sharptla-xpath-md.md)]. Wyrażenie " [!INCLUDE[TLA2#tla_xpath](../../../../includes/tla2sharptla-xpath-md.md)] true" dla powiązania [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] ze źródłem danych nie <xref:System.Windows.Data.Binding.Path%2A> jest używane jako wartość i powinno być używane dla wzajemnie wykluczających <xref:System.Windows.Data.Binding.XPath%2A> się właściwości.
...
Każda składnia pokazana powyżej może zostać przeplatana. Na przykład, poniżej można utworzyć ścieżkę właściwości do koloru na określonej x, y `ColorGrid` właściwości, która zawiera <xref:System.Windows.Media.SolidColorBrush> tablicę siatki pikseli obiektów:
...
Właściwość Target animacji musi być właściwością zależności, która przyjmuje <xref:System.Windows.Freezable> albo lub typ pierwotny. Jednak właściwość target typu i ostateczna Właściwość animowana może istnieć w różnych obiektach. W przypadku animacji ścieżka właściwości jest używana do definiowania połączenia między właściwością nazwanego obiektu docelowego animacji a zamierzoną właściwością animacji docelowej, przez przechodzenie między relacjami obiektu i właściwości w wartościach właściwości.
...
Aby uzyskać więcej informacji na temat ogólnych pojęć dotyczących animacji [](../graphics-multimedia/storyboards-overview.md) , zobacz Omówienie scenorysów i Omówienie [animacji](../graphics-multimedia/animation-overview.md).
...
`propertyName2`musi być nazwą właściwości zależności, która istnieje w obiekcie, który jest wartością `propertyName`. Innymi słowy, `propertyName2` musi istnieć jako właściwość zależności w typie, który `propertyName` <xref:System.Windows.DependencyProperty.PropertyType%2A>jest.
...
Użycie kodu dla <xref:System.Windows.PropertyPath>, w tym sposób <xref:System.Windows.PropertyPath>konstruowania, jest udokumentowane w temacie referencyjnym <xref:System.Windows.PropertyPath>dla.

Ogólnie rzecz biorąc <xref:System.Windows.PropertyPath> , jest przeznaczony do korzystania z dwóch różnych konstruktorów, jednej do użycia powiązań i najprostszych zastosowań animacji oraz jeden dla złożonych zastosowań animacji. <xref:System.Windows.PropertyPath.%23ctor%28System.Object%29> Użyj podpisu do powiązań użycia, gdzie obiekt jest ciągiem. Użyj podpisu dla ścieżek animacji jednoetapowych, gdzie obiekt <xref:System.Windows.DependencyProperty>jest. <xref:System.Windows.PropertyPath.%23ctor%28System.Object%29> <xref:System.Windows.PropertyPath.%23ctor%28System.String%2CSystem.Object%5B%5D%29> Użyj podpisu dla złożonych animacji. Ten drugi Konstruktor używa ciągu tokenu dla pierwszego parametru i tablicy obiektów, które wypełniają pozycje w ciągu tokenu, aby zdefiniować relację ścieżki właściwości.
