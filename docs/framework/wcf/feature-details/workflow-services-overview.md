...
`xref:System.ServiceModel.Activities.SendReply.ValueType`
 ...
 W rezultacie wnioskowania kontraktu jest opis usługi przy użyciu tych samych struktur danych jako [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] usług i kontrakty operacji. Te informacje jest następnie używane do udostępnienia WSDL usługi przepływu pracy.  
...
## <a name="hosting-a-workflow-service"></a>Hosting usług przepływu pracy  
 Podobnie jak [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] usług przepływu pracy musi być hostowany usług. [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]Użyj usługi <xref:System.ServiceModel.ServiceHost> klasy do hostowania usług i przepływ pracy usług użyj <xref:System.ServiceModel.Activities.WorkflowServiceHost> do hostowania usług. Podobnie jak [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] usług przepływu pracy, usługi mogą być hostowane na wiele sposobów, na przykład:  
...
-   W zarządzanych usług systemu Windows.  
 ...
 Do hosta usługi przepływu pracy w usługach IIS lub WAS, umieść plik .xamlx, który zawiera definicję usługi przepływu pracy do katalogu wirtualnego. Domyślny punkt końcowy (przy użyciu <xref:System.ServiceModel.BasicHttpBinding>) jest tworzony automatycznie [!INCLUDE[crdefault](../../../../includes/crdefault-md.md)] [uproszczony konfiguracji](../../../../docs/framework/wcf/simplified-configuration.md). Można również umieścić plik Web.config w katalogu wirtualnego, aby określić własne punktów końcowych. Jeśli definicja przepływu pracy znajduje się w zestawie można umieścić plików .svc w katalogu wirtualnym i zestawu przepływu pracy w katalogu App_Code. W pliku svc należy określić fabryki hostów usług i klasy, która implementuje usługi przepływu pracy. Poniższy przykład pokazuje, jak wybierz fabryki hostów usług i określ klasy, która implementuje usługi przepływu pracy.  
