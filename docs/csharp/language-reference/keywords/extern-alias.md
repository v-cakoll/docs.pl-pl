...
>  Słowo kluczowe [extern](./extern.md) jest również używane jako modyfikator metody, deklarując metodę zapisaną w kodzie niezarządzanym.  
 ...
 Spowoduje to utworzenie aliasów `GridV1` zewnętrznych `GridV2`i. Aby użyć tych aliasów z poziomu programu, odwołując się do `extern` nich za pomocą słowa kluczowego. Na przykład:  
