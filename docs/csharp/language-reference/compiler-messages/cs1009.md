---
title: Błąd kompilatora CS1009
ms.date: 07/20/2015
f1_keywords:
- CS1009
helpviewer_keywords:
- CS1009
ms.assetid: 348f500c-0e4f-44d7-95a8-e215ac49940a
ms.openlocfilehash: 6ec398abdb4fc51fbdd2f5dd3c68950769f7f844
ms.sourcegitcommit: 986f836f72ef10876878bd6217174e41464c145a
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 08/19/2019
ms.locfileid: "69607653"
---
# <a name="compiler-error-cs1009"></a>Błąd kompilatora CS1009
Nierozpoznana sekwencja ucieczki  
  
 Nieoczekiwany znak następujący po ukośniku odwrotnym (\\) w [ciągu znaków](../keywords/string.md). Kompilator oczekuje jednego z prawidłowych znaków ucieczki. Aby uzyskać więcej informacji, zobacz [Znaki specjalne](../../../standard/base-types/character-escapes-in-regular-expressions.md).  
  
 Poniższy przykład generuje CS1009.  
  
```csharp  
// CS1009-a.cs  
class MyClass  
{  
   static void Main()  
   {  
      // The following line causes CS1009.  
      string a = "\m";     
      // Try the following line instead.  
      // string a = "\t";  
   }  
}  
```  
  
 Typową przyczyną tego błędu jest użycie ukośnika w ścieżce pliku, jak pokazano w poniższym przykładzie.  
  
```csharp  
string filename = "c:\myFolder\myFile.txt";  
```  
  
 Aby rozwiązać problem, należy wpisać "\\\\" lub poprzedzić literał ciągu znakiem @-quoted — jak w przykładzie poniżej.  
  
```csharp  
// CS1009-b.cs  
class MyClass  
{  
   static void Main()  
   {  
      // The following line causes CS1009.  
      string filename = "c:\myFolder\myFile.txt";     
      // Try one of the following lines instead.  
      // string filename = "c:\\myFolder\\myFile.txt";  
      // string filename = @"c:\myFolder\myFile.txt";  
   }  
}  
```  
  
## <a name="see-also"></a>Zobacz także

- [string](../keywords/string.md)
