...
|`statements`|Opcjonalna. Jedna lub więcej instrukcji między `For Each` i `Next`, które są uruchamiane dla każdego elementu w `group`.|
|`Continue For`|Opcjonalna. Przenosi kontrolkę do początku pętli `For Each`.|
|`Exit For`|Opcjonalna. Przenosi kontrolę z pętli `For Each`.|
