...
|Zdarzenia|[Zdarzenia](#events)|Metody, które implementują zdarzenie to będzie oznakowane `SpecialName` w oznakowane.|29|  
...
|Ogólne|[Zgodność ze specyfikacją CLS: zasady](#Rules)|Zasady CLS dotyczą tylko tych części typu, które są dostępne lub widoczne poza zdefiniowanym zestawem.|1|  
...
|Typy ogólne|[Typy ogólne i członkowie](#Generics)|Dla każdej abstrakcyjnej lub standardowej wirtualnej metody jest domyślny konkretnych implementacji (nieabstrakcyjna).|47|  
...
|Konwencje nazewnictwa|[Konwencje nazewnictwa](#naming)|Zestawy postępuje zgodnie z załącznikiem 7 z technicznego raportu 15 standardy Unicode Standard3.0 regulującego zestaw znaków do rozpoczynać i znajdować się w identyfikatorach, być dostępny http://www.unicode.org/unicode/reports/tr15/tr15-18.html. Identyfikatory powinny być w formacie kanonicznym zdefiniowanym przez formularz normalizacji Unicode C. W celach ze specyfikacją CLS dwa identyfikatory są takie same, jeśli ich mapowania na małe litery (określone przez mapowania jeden toonelowercase ustawień regionalnych niezależnych od, Unicode) są takie same. Oznacza to, że dwa identyfikatory uważane za różne w specyfikacji CLS różnią w więcej niż zwykła w ich przypadku. Jednak aby zastąpić niedziedziczoną definicję interfejsu wiersza polecenia wymaga precyzyjnego kodowania pierwotnej deklaracji.|4|  
|Przeciążenie|[Konwencje nazewnictwa](#naming)|Wszystkie nazwy wprowadzone w zakresie zgodnym ze specyfikacją CLS powinny się wyróżniać niezależnie od, z wyjątkiem sytuacji, w których nazwy są identyczne i rozpoznawane przez przeciążenie. Oznacza to podczas gdy specyfikacja CTS zezwala jeden typ używał tej samej nazwy dla metody, pola, specyfikacja CLS nie zezwala.|5|  
...
|Przeciążenie|--|Jeśli dwa lub więcej metod zgodnych ze specyfikacją CLS zadeklarowane w typie mają ten sam samą nazwę dla określonego zestawu wystąpień typu mają ten sam parametr i zwracane typy, wszystkie te metody są semantycznie równoważne w tych wystąpieniach typów.|48|  
...
|Właściwości|[Właściwości](#properties)|Metody, które implementują metody getter i setter właściwości muszą być oznaczone `SpecialName` w metadanych.|24|  
