...
- Klient, który odwołuje się tylko do wersji 1,0 identycznych bibliotek DLL, ale uzyskuje dostęp do klasy z wersji 2,0.  
 ...
 Poniższy kod tworzy pierwszą z identycznych bibliotek DLL. Aby uzyskać informacje o sposobach generowania pliku klucza, zobacz [/keyfile (C# opcje kompilatora)](../../../csharp/language-reference/compiler-options/keyfile-compiler-option.md).  
...
## <a name="example"></a>Przykład  
 Poniższy kod odwołuje się do dwóch wersji biblioteki DLL, które są zdefiniowane w poprzednim kodzie. `AssemblyA`odwołuje się do biblioteki DLL utworzonej przez CS1705a.cs (wersja 1,0). `AssemblyB`odwołuje się do biblioteki DLL utworzonej przez CS1705b.cs (wersja 2,0). W `ClassC`programie zdefiniowane są dwie metody. Pierwszy, `Return1A`, zwraca obiekt typu `Class1A`, który jest aliasem dla `Class1` z wersji 1,0 biblioteki DLL. Sekunda, `Return1B`, zwraca obiekt typu `Class1B`, który jest aliasem dla `Class1` z wersji 2,0 biblioteki DLL. Definicja `Return1A` tworzy zależność od wersji 1,0; `Return1B` definicja tworzy zależność w wersji 2,0.  
