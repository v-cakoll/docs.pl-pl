---
title: Autoryzacja niestandardowa
ms.date: 03/30/2017
ms.assetid: 4f90311a-2593-462e-9174-2f7ac78217b9
ms.openlocfilehash: f89fb3c14735417f2688890eb335e9700fb99d6f
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
ms.locfileid: "33487860"
---
# <a name="custom-authorization"></a>Autoryzacja niestandardowa
Autoryzacja polega na kontrolowanie dostępu do zasobów i zadań. Tematy w tej sekcji umożliwiają tworzenie do realizacji różnych oświadczenia lub zasad.  
  
## <a name="in-this-section"></a>W tej sekcji  
 [Instrukcje: tworzenie niestandardowych zasad autoryzacji](../../../../docs/framework/wcf/extending/how-to-create-a-custom-authorization-policy.md)  
 Opisuje sposób tworzenia niestandardowych zasad.  
  
 [Instrukcje: tworzenie menedżera autoryzacji niestandardowej dla usługi](../../../../docs/framework/wcf/extending/how-to-create-a-custom-authorization-manager-for-a-service.md)  
 Opisuje sposób tworzenia niestandardowych menedżera.  
  
 [Instrukcje: tworzenie oświadczenia niestandardowego](../../../../docs/framework/wcf/extending/how-to-create-a-custom-claim.md)  
 Opisuje sposób tworzenia niestandardowych oświadczeń.  
  
 [Instrukcje: porównywanie oświadczeń](../../../../docs/framework/wcf/extending/how-to-compare-claims.md)  
 Opisuje sposób porównywania oświadczeń.  
  
 [Instrukcje: tworzenie niestandardowej tożsamości podmiotu zabezpieczeń](../../../../docs/framework/wcf/extending/how-to-create-a-custom-principal-identity.md)  
 Przedstawiono scenariusz podczas <xref:System.ServiceModel.Description.PrincipalPermissionMode.Custom> jest używany w połączeniu z niestandardowych zasad autoryzacji i niestandardowy podmiot zabezpieczeń.  
  
## <a name="reference"></a>Tematy pomocy  
 <xref:System.IdentityModel.Claims>  
  
## <a name="related-sections"></a>Sekcje pokrewne  
 [Autoryzacja](../../../../docs/framework/wcf/feature-details/authorization-in-wcf.md)  
  
 [Rozszerzanie zabezpieczeń](../../../../docs/framework/wcf/extending/extending-security.md)
