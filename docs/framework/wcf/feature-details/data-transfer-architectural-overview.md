...
## <a name="message-properties"></a>Właściwości komunikatu  
 Wiadomość może zawierać właściwości. A *właściwość* jest dowolnym [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] obiektu, który jest skojarzony z nazwy ciągu. Właściwości są dostępne za pośrednictwem `Properties` właściwość `Message`.  
...
### <a name="3-message-contracts"></a>3. Kontrakty komunikatów  
 WCF oferuje deklaratywny model programowania do opisywania wiadomości, nazywane *kontrakt komunikatu*. Ten model jest szczegółowo opisane w [za pomocą kontraktów komunikatu](../../../../docs/framework/wcf/feature-details/using-message-contracts.md). Zasadniczo cały komunikat jest reprezentowane przez jeden [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] typu, który używa atrybutów, takich jak <xref:System.ServiceModel.MessageBodyMemberAttribute> i <xref:System.ServiceModel.MessageHeaderAttribute> do opisania części klasy kontraktu komunikatu, które powinny być mapowane do której części wiadomości.  
...
## <a name="serialization"></a>Serializacja  
 Jeśli używasz kontrakty komunikatów lub parametry, aby opisać treść wiadomości, należy użyć serializacji do konwersji między [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] typów i reprezentujący zestaw informacji XML. Serializacji jest używany w innych miejscach w WCF — na przykład <xref:System.ServiceModel.Channels.Message> ma ogólny <xref:System.ServiceModel.Channels.Message.GetBody%2A> metodę, która służy do odczytać całej treści komunikatu deserializacji obiektu.  
