...
### <a name="asynchronous-client-with-callback"></a>Asynchroniczne klienta przy użyciu wywołania zwrotnego  
 ...
 W tej sekcji przedstawiono tylko przy użyciu protokołu HTTP z `propagateActivity` = `true`. Jednak dodatkowe działania i transfery dotyczą również innych przypadkach (czyli `propagateActivity` = `false`, przy użyciu lub TCP albo potoku nazwanego).  
