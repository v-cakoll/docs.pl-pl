...
|Zabezpieczenia|Określa, jak jest zabezpieczony kanał:<br /><br /> -Brak: Nie jest zabezpieczony wiadomości protokołu SOAP, a klient nie jest uwierzytelniony.<br />-Transport: Spełnione są wymagania dotyczące zabezpieczeń w warstwie transportowej.<br />-Komunikat o błędzie: Warstwie wiadomości są spełnione wymagania zabezpieczeń.<br />-Mieszane: Ten tryb zabezpieczeń jest znany jako `TransportWithMessageCredentials`. Obsługuje on poświadczenia na poziomie komunikatu i wymagania dotyczące integralności i poufności informacji są spełnione przez warstwy transportowej.<br />-Zarówno: Zarówno poziom i mechanizm transportu zabezpieczeniami na poziomie wiadomości są używane. Ta możliwość jest unikatowy dla <xref:System.ServiceModel.NetMsmqBinding>.|  
