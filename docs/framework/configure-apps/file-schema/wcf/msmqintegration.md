...
|serializationFormat|Określa program formatujący, który służy do wykonywania serializacji obiektów, które są wysyłane jako część wiadomości usługi MSMQ. Prawidłowe wartości to:<br /><br /> -ActiveX: Element formatujący ActiveX jest używany podczas serializowania obiektów COM.<br />-Pliku binarnego: Serializuje obiekt binarny pakietów.<br />-ByteArray: Serializuje obiekt jako tablicę bajtów.<br />-Stream: Serializuje obiekt w strumieniu.<br />-Xml: Serializuje obiekt do pakietu z potwierdzeniem XML. Wartość domyślna to XML.<br /><br /> Ten atrybut jest typu <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat>.|  
