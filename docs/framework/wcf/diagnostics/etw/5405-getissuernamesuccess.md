---
title: 5405 — GetIssuerNameSuccess
ms.date: 03/30/2017
ms.assetid: b7904796-0eb2-4d45-a1bb-d13af9ab740d
ms.openlocfilehash: 3d59d5abaaaa3266264c8aa373aa13b5390a9cc5
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
ms.locfileid: "33469248"
---
# <a name="5405---getissuernamesuccess"></a>5405 — GetIssuerNameSuccess
## <a name="properties"></a>Właściwości  
  
|||  
|-|-|  
|ID|5405|  
|Słowa kluczowe|Zabezpieczenia|  
|Poziom|Pełny|  
|Kanał|Microsoft-Windows aplikacji debugowania serwera — aplikacje|  
  
## <a name="description"></a>Opis  
 To zdarzenie jest emitowany podczas pobierania nazwy wystawcy zakończyło się pomyślnie.  
  
## <a name="message"></a>Komunikat  
 Pobieranie wystawcy Nazwa: % 1 z obiektu tokenid: % 2 zakończyło się pomyślnie.  
  
## <a name="details"></a>Szczegóły
