...
Instrukcja foreach nie może działać na zmiennych typu "type", ponieważ implementuje wiele wystąpień elementu "interface", spróbuj wykonać rzutowanie do określonego wystąpienia interfejsu.  
 ...
 Typ dziedziczy z dwóch lub więcej wystąpień IEnumerator\<T >, co oznacza, że nie jest unikatowy wyliczenia typu który `foreach` można użyć. Określ typ IEnumerator\<T > lub użyj innej konstrukcji pętli.  
