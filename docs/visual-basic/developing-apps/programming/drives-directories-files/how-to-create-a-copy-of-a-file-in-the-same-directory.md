...
- Użyj metody `CopyFile`, podając plik docelowy i lokalizację. Poniższy przykład tworzy kopię `test.txt` o nazwie `test2.txt`.  
...
- Użyj metody `CopyFile`, podając docelowy plik i lokalizację, a następnie ustaw `overwrite` do `True`. Poniższy przykład tworzy kopię `test.txt` o nazwie `test2.txt` i zastępuje wszystkie istniejące pliki o tej nazwie.  
