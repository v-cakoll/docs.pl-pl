...
> Podczas tworzenia wystąpienia osadzonego typu COM w kodzie, należy utworzyć wystąpienie przy użyciu odpowiedniego interfejsu. Próba utworzenia wystąpienia osadzonego typu COM przy użyciu klasy coclass powoduje wystąpienie błędu.  
 ...
 Aby ustawić opcję `-link` w programie Visual Studio, Dodaj odwołanie do zestawu i ustaw właściwość `Embed Interop Types` na **wartość true**. Wartość domyślna właściwości `Embed Interop Types` ma **wartość false**.  
...
[!code-vb[VbLinkCompiler#4](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/vblinkcompiler/vb/utility.vb#4)]  
 ...
 W poniższym przykładzie kod klienta może wywołać metodę, która zwraca <xref:System.Collections.IList> interfejs ogólny bez błędu.  
  
 [!code-vb[VbLinkCompiler#5](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/vblinkcompiler/vb/module1.vb#5)]  
  
## <a name="example"></a>Przykład  
 Poniższy wiersz polecenia kompiluje `OfficeApp.vb` pliku źródłowego i zestawy referencyjne z `COMData1.dll` i `COMData2.dll`, aby utworzyć `OfficeApp.exe`.  
