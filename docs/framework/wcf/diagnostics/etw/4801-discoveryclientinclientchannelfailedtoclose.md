---
title: 4801 — DiscoveryClientInClientChannelFailedToClose
ms.date: 03/30/2017
ms.assetid: 9d85113b-73b9-4599-b776-30b40aaf1e7a
ms.openlocfilehash: e19d190cd774f868ccb8e9e0b324dfb3ddf6733d
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/04/2018
---
# <a name="4801---discoveryclientinclientchannelfailedtoclose"></a>4801 — DiscoveryClientInClientChannelFailedToClose
## <a name="properties"></a>Właściwości  
  
|||  
|-|-|  
|ID|4801|  
|Słowa kluczowe|Odnajdywanie|  
|Poziom|Ostrzeżenie|  
|Kanał|Microsoft-Windows aplikacji debugowania serwera — aplikacje|  
  
## <a name="description"></a>Opis  
 To zdarzenie jest emitowany, gdy obiektu DiscoveryClient utworzonego w obiekcie DiscoveryClientChannel nie powiodło się zamknąć i dlatego został przerwany.  
  
## <a name="message"></a>Komunikat  
 Obiektu DiscoveryClient utworzonego w obiekcie DiscoveryClientChannel nie można zamknąć i dlatego została przerwana.  
  
## <a name="details"></a>Szczegóły
