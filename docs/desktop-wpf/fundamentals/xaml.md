...
Gdy jest reprezentowana jako tekst, pliki XAML są plikami XML, które zwykle mają `.xaml` rozszerzenie. Pliki mogą być kodowane przy użyciu dowolnego kodowania XML, ale kodowanie jako UTF-8 jest typowe.
...
Określa dwa elementy obiektów: `<StackPanel>` (z zawartością i tag zamykany później) oraz `<Button .../>` (formularz z własnym zamykaniem z kilkoma atrybutami). Elementy obiektu `StackPanel` i `Button` Każda mapa do nazwy klasy, która jest zdefiniowana przez WPF i jest częścią zestawów WPF. Gdy określisz tag elementu obiektu, tworzysz instrukcję przetwarzania XAML, aby utworzyć nowe wystąpienie typu podstawowego. Każde wystąpienie jest tworzone przez wywołanie konstruktora bez parametrów bazowego typu podczas analizowania i ładowania XAML.
...
- Klasa musi deklarować Właściwość zawartości, a właściwość zawartości musi być typu, który można przypisać do ciągu (może to być typ <xref:System.Object> ). Na przykład każdy z nich <xref:System.Windows.Controls.ContentControl> używa <xref:System.Windows.Controls.ContentControl.Content%2A> jako jego właściwości zawartości i jest typem <xref:System.Object> i obsługuje następujące użycie na <xref:System.Windows.Controls.ContentControl> przykład <xref:System.Windows.Controls.Button> : `<Button>Hello</Button>` .
...
W tym miejscu każdy <xref:System.Windows.Controls.Button> jest elementem podrzędnym <xref:System.Windows.Controls.StackPanel> . Jest to usprawnione i intuicyjne znaczniki, które pomija dwa Tagi z dwóch różnych przyczyn.
...
W [składni XAML w zwięzłej](#xaml-syntax-in-brief) sekcji stwierdzono, że wartość atrybutu musi być możliwa do ustawienia przez ciąg. Podstawowa, natywna obsługa sposobu, w jaki ciągi są konwertowane na inne typy obiektów lub wartości pierwotne, jest oparta na <xref:System.String> typie, oprócz natywnego przetwarzania niektórych typów, takich jak <xref:System.DateTime> lub <xref:System.Uri> . Jednak wiele typów WPF lub elementów członkowskich tych typów zwiększają podstawowe zachowanie przetwarzania atrybutów ciągów w taki sposób, że wystąpienia bardziej złożonych typów obiektów można określić jako ciągi i atrybuty.

<xref:System.Windows.Thickness>Struktura jest przykładem typu, który ma włączoną konwersję typu dla użycia XAML. <xref:System.Windows.Thickness>wskazuje miary w obrębie zagnieżdżonego prostokąta i jest używana jako wartość właściwości, takich jak <xref:System.Windows.FrameworkElement.Margin%2A> . Przez umieszczenie konwertera typów <xref:System.Windows.Thickness> , wszystkie właściwości, które używają a <xref:System.Windows.Thickness> są łatwiejsze do określenia w języku XAML, ponieważ można je określić jako atrybuty. W poniższym przykładzie użyto konwersji typu i składni atrybutów, aby podać wartość dla <xref:System.Windows.FrameworkElement.Margin%2A> :

[!code-xaml[XAMLOvwSupport#MarginTCE](~/samples/snippets/csharp/VS_Snippets_Wpf/XAMLOvwSupport/CSharp/page7.xaml#margintce)]

Przykład składni poprzedniej atrybutu jest odpowiednikiem poniższego, bardziej szczegółowego przykładu składni, gdzie <xref:System.Windows.FrameworkElement.Margin%2A> zamiast tego jest ustawiana za pomocą składni elementu właściwości zawierającego <xref:System.Windows.Thickness> element obiektu. Cztery właściwości klucza <xref:System.Windows.Thickness> są ustawiane jako atrybuty dla nowego wystąpienia:
...
`xmlns`Atrybuty są tylko absolutnie niezbędne dla elementu głównego każdego pliku XAML. `xmlns`definicje będą stosowane do wszystkich elementów podrzędnych elementu głównego (to zachowanie jest ponownie spójne ze specyfikacją XML 1,0 dla `xmlns` .) `xmlns` atrybuty są również dozwolone dla innych elementów znajdujących się w katalogu głównym, a następnie stosowane do wszystkich elementów podrzędnych elementu definiującego. Jednak częste Definiowanie lub ponowna definicja przestrzeni nazw XAML może spowodować, że styl znaczników XAML jest trudny do odczytania.

Implementacja WPF procesora języka XAML obejmuje infrastrukturę, która ma świadomość zestawów WPF Core. Zestawy WPF Core są znane, aby zawierały typy obsługujące mapowania WPF do domyślnej przestrzeni nazw XAML. Ta funkcja jest włączana za pomocą konfiguracji, która jest częścią pliku kompilacji projektu oraz systemów kompilacji i projektów WPF. W związku z tym deklaruje domyślną przestrzeń nazw XAML, ponieważ jest to wartość domyślna, `xmlns` która jest niezbędna do odwoływania się do elementów języka XAML, które pochodzą z zestawów WPF.

### <a name="the-x-prefix"></a>Prefiks x:

W poprzednim przykładzie elementu głównego prefiks `x:` został użyty do mapowania przestrzeni nazw XAML `http://schemas.microsoft.com/winfx/2006/xaml` , która jest dedykowaną przestrzenią nazw XAML, która obsługuje konstrukcje języka XAML. Ten `x:` prefiks jest używany do mapowania tej przestrzeni nazw XAML w szablonach dla projektów, w przykładach i w dokumentacji w całym tym zestawie SDK. Przestrzeń nazw XAML dla języka XAML zawiera kilka konstrukcji programistycznych, które będą często używane w kodzie XAML. Poniżej znajduje się lista najpopularniejszych `x:` konstrukcji programowania prefiksów, które będą używane:
...
Istnieją dodatkowe konstrukcje programistyczne w `x:` przestrzeni nazw prefix/XAML, które nie są wspólne. Aby uzyskać szczegółowe informacje, zobacz [przestrzeń nazw XAML (x:) Funkcje językowe](../xaml-services/namespace-language-features.md).
...
Należy zauważyć, że plik związany z kodem używa przestrzeni nazw CLR `ExampleNamespace` i deklaruje `ExamplePage` jako klasę częściową w tej przestrzeni nazw. Jest to równoległe `x:Class` wartość atrybutu `ExampleNamespace` .`ExamplePage` który został podany w katalogu głównym znaczników. Kompilator znaczników WPF utworzy klasę częściową dla dowolnego skompilowanego pliku XAML, pobierając klasę z typu elementu głównego. Gdy podajesz kod, który definiuje również tę samą klasę częściową, otrzymany kod jest połączony w tej samej przestrzeni nazw i klasie skompilowanej aplikacji.
...
Jest to podobne, co przypomina użycie elementu właściwości, ale w tym przypadku jest on `ownerType` zawsze innym typem niż element obiektu, w którym jest ustawiana dołączona właściwość. `ownerType`to typ, który dostarcza metody dostępu wymagane przez procesor XAML w celu uzyskania lub ustawienia wartości właściwości dołączone.
...
Poniższy przykład ilustruje <xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType> przyłączoną właściwość. <xref:System.Windows.Controls.DockPanel>Klasa definiuje metody dostępu dla i w <xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType> związku z tym jest właścicielem dołączonej właściwości. <xref:System.Windows.Controls.DockPanel>Klasa zawiera również logikę, która iteruje elementy podrzędne i sprawdza każdy element dla ustawionej wartości <xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType> . Jeśli zostanie znaleziona wartość, ta wartość jest używana podczas układania do położenia elementów podrzędnych. Użycie <xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType> dołączonej właściwości i tej możliwości pozycjonowania jest w rzeczywistości scenariuszem z motywem dla <xref:System.Windows.Controls.DockPanel> klasy.
...
Bazowy kod XAML WPF i jego przestrzeń nazw XAML to Kolekcja typów, która odpowiada obiektom CLR, a nie tylko do elementów znaczników dla języka XAML. Jednak nie wszystkie klasy mogą być mapowane do elementów. Klasy abstrakcyjne, takie jak <xref:System.Windows.Controls.Primitives.ButtonBase> i pewne nieabstrakcyjne klasy podstawowe, są używane do dziedziczenia w modelu obiektów CLR. Klasy podstawowe, w tym abstrakcyjne, są nadal ważne dla rozwoju kodu XAML, ponieważ każdy konkretny element XAML dziedziczy elementy członkowskie z pewnej klasy podstawowej w swojej hierarchii. Często te elementy członkowskie zawierają właściwości, które można ustawić jako atrybuty dla elementu, lub zdarzenia, które mogą być obsługiwane. <xref:System.Windows.FrameworkElement>jest konkretną podstawową klasą interfejsu użytkownika WPF na poziomie platformy WPF. Podczas projektowania interfejsu użytkownika będziesz używać różnych klas Shape, panel, dekoratora i Control, które pochodzą od <xref:System.Windows.FrameworkElement> . Powiązana Klasa bazowa, <xref:System.Windows.FrameworkContentElement> obsługuje elementy zorientowane na dokumenty, które dobrze działają dla prezentacji układu przepływu, przy użyciu interfejsów API, które w sposób świadomy duplikują interfejsy API w programie <xref:System.Windows.FrameworkElement> . Kombinacja atrybutów na poziomie elementu i modelu obiektów CLR zapewnia zestaw wspólnych właściwości, które są ustawiane jako settable dla większości konkretnych elementów XAML, niezależnie od konkretnego elementu XAML i jego typu podstawowego.
