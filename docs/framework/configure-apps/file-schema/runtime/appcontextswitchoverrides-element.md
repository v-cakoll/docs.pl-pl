...
|`Switch.System.Diagnostics.`<br/>`IgnorePortablePDBsInStackTraces`|Określa, czy śladów stosu uzyskać, korzystając z plików przenośnych PDB mogą obejmować informacje o źródle plików i wierszy. `false` Aby uwzględnić informacje plików i wierszy źródła; w przeciwnym razie `true`.|.NET framework 4.7.2|
...
|`Switch.System.Drawing.Text.`<br/>`DoNotRemoveGdiFontsResourcesFromFontCollection`|Określa, czy <xref:System.Drawing.Text.PrivateFontCollection?displayProperty=nameWithType> obiektów prawidłowo są usuwane podczas dodawania do kolekcji przez <xref:System.Drawing.Text.PrivateFontCollection.AddFontFile(System.String)?displayProperty=nameWithType> metody. `true` Aby zachować starsze zachowanie; `false` do usuwania wszystkich obiektów prywatnej czcionki. |.NET framework 4.7.2|
...
|`Switch.System.IO.Compression.`<br/>`DoNotUseNativeZipLibraryForDecompression`|Określa, czy natywnego systemu operacyjnego procedury służą do dekompresji przez <xref:System.IO.Compression.DeflateStream> klasy. `false` Aby użyć natywnych interfejsów API; `true` do używania <xref:System.IO.Compression.DeflateStream> implementacji.|.NET framework 4.7.2|
...
|`Switch.System.Runtime.Serialization.`<br/>`DoNotUseTimeZoneInfo`|Formanty czy <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> obsługuje wiele dostosowań lub tylko korekty pojedynczego dla strefy czasowej. Jeśli `true`, używa <xref:System.TimeZoneInfo> typ do serializacji i deserializacji danych daty i godziny; w przeciwnym razie używa <xref:System.TimeZone> typu, który nie obsługuje wielu reguł korygowania.|.NET Framework 4.6.2|
...
|`Switch.System.Uri.`<br/>`DontEnableStrictRFC3986ReservedCharacterSets`|Określa, czy niektóre procent, kodowane które czasami zostały zdekodować teraz spójnie lewej kodowania znaków. Jeśli `true`, są one dekodowane; w przeciwnym razie `false`.|.NET framework 4.7.2|
|`Switch.System.Uri.`<br/>`DontKeepUnicodeBidiFormattingCharacters`|Określa obsługę znaki dwukierunkowy Unicode w identyfikatorów URI. `true` Aby usunąć je z identyfikatorów URI; `false` zachowanie i procent zakodować je.|.NET framework 4.7.2|
...
|`Switch.System.Windows.Forms.`<br/>`UseLegacyContextMenuStripSourceControlValue`|Określa, czy <xref:System.Windows.Forms.ContextMenuStrip.SourceControl?displayProperty=nameWithType> właściwość zwraca kontroli źródła, gdy użytkownik otwiera menu z zagnieżdżoną <xref:System.Windows.Forms.ToolStripMenuItem> formantu. `true` Aby przywrócić `null`, starsze zachowanie; `false` do zwrócenia kontroli źródła.|.NET framework 4.7.2|
|`Switch.System.Windows.Input.Stylus.`<br/>`EnablePointerSupport`|Określa, czy jest to opcjonalny `WM_POINTER`— na podstawie touch/pióro stosu jest włączona w aplikacji WPF. Aby uzyskać więcej informacji, zobacz [środki zaradcze: Touch oparte na wskaźnik i pomocy technicznej pióra](../../../migration-guide/mitigation-pointer-based-touch-and-stylus-support.md)|.NET framework 4.7|
|`Switch.System.Windows.Markup.`<br/>`DoNotUseSha256ForMarkupCompilerChecksumAlgorithm`|Określa, czy domyślny algorytm skrótu używany dla sum kontrolnych jest SHA256 (`false`) lub SHA1 (`true`).|.NET framework 4.7.2|
...
|`Switch.UseLegacyAccessibilityFeatures.2`|Formanty, czy dostępne w programie .NET Framework 4.7.2 funkcje ułatwień dostępu są włączane (`false`) lub wyłączona (`true`). Jeśli `true`, `Switch.UseLegacyAccessibilityFeatures` musi być również `true` Aby włączyć funkcje ułatwień dostępu w programie .NET Framework 4.7.1.|.NET framework 4.7.2|
