...
Określa kontekst dla <xref:System.ServiceModel.NetTcpBinding>, który wymaga podpisania poziomu ochrony. ContextExchangeMechanism dla NetTcpContextBinding to SOAPHeader.  
...
|contextProtectionLevel|Prawidłowa wartość <xref:System.Net.Security.ProtectionLevel>, która określa żądany poziom ochrony nagłówka SOAP używany do propagowania informacji kontekstowych.  Wartość domyślna to <xref:System.Net.Security.ProtectionLevel.Sign>.|  
