...
|Użyj trybu wypełnienia kolumny z wartościami o różnej ważności.|Ustaw <xref:System.Windows.Forms.DataGridView.AutoSizeColumnsMode%2A> właściwość <xref:System.Windows.Forms.DataGridViewAutoSizeColumnsMode.Fill>. Ustaw duże wartości <xref:System.Windows.Forms.DataGridViewColumn.MinimumWidth%2A> dla kolumn, które muszą zawsze wyświetlać niektóre z ich danych lub użyć opcji ustalania wielkości innej niż tryb wypełnienia dla określonych kolumn.|  
