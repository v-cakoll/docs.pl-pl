...
- Można jawnie zadeklarować typ każdego pola wewnątrz nawiasów. Poniższy przykład używa tego podejścia do dekonstruowania 3-spójnej kolekcji zwracanej przez `QueryCityData` metodę.

    [!code-csharp[Deconstruction-Explicit](../../samples/snippets/csharp/programming-guide/deconstructing-tuples/deconstruct-tuple2.cs#1)]

- Możesz użyć `var` słowa kluczowego, aby C# wnioskuje typ każdej zmiennej. Słowo kluczowe jest umieszczane `var` poza nawiasami. Poniższy przykład używa wnioskowania o typie podczas dekonstruowania 3-spójnej kolekcji zwróconej przez `QueryCityData` metodę.
...
Ponieważ można przeciążać `Deconstruct` metodę w celu odzwierciedlenia grup danych, które są powszechnie wyodrębniane z obiektu, należy zachować ostrożność, aby zdefiniować `Deconstruct` metody z podpisami, które są charakterystyczne i niejednoznaczne. Wiele `Deconstruct` metod, które mają taką samą liczbę `out` parametrów lub tę samą liczbę i typ `out` parametrów w innej kolejności może spowodować pomyłkę.
...
W poniższym przykładzie zdefiniowano dwie `Deconstruct` metody rozszerzające dla <xref:System.Reflection.PropertyInfo?displayProperty=nameWithType> klasy. Pierwszy zwraca zestaw wartości wskazujący charakterystykę właściwości, w tym jej typ, czy jest to element statyczny, czy wystąpienie, czy jest tylko do odczytu i czy jest indeksowany. Drugi wskazuje dostępność właściwości. Ponieważ dostępność metody dostępu get i set może się różnić, wartości logiczne wskazują, czy właściwość ma oddzielne metody dostępu get i Set, a jeśli tak, to czy mają one taką samą dostępność. Jeśli istnieje tylko jedna metoda dostępu lub metoda dostępu get i Set ma tę samą dostępność, `access` zmienna wskazuje dostępność właściwości jako całości. W przeciwnym razie dostępność funkcji dostępu get i Set jest wskazywana przez `getAccess` `setAccess` zmienne i.
...
- [Typy krotek](language-reference/builtin-types/value-tuples.md)
