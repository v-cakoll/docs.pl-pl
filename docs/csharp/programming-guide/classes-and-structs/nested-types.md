...
Niezależnie od tego, czy typ zewnętrzny jest klasą, czy strukturą, zagnieżdżone typy domyślne to [Private](../../language-reference/keywords/private.md); są one dostępne tylko z tego typu zawierającego. W poprzednim przykładzie Klasa `Nested` jest niedostępna dla typów zewnętrznych. 
