...
### <a name="keyboard-and-mouse-classes"></a>Klasy klawiatury i myszy
 ...
 Przykłady wejściowych interfejsów API w klasie <xref:System.Windows.Input.Mouse> są <xref:System.Windows.Input.Mouse.MiddleButton%2A>, które uzyskują stan środkowego przycisku myszy, a <xref:System.Windows.Input.Mouse.DirectlyOver%2A>, który pobiera element, który jest obecnie przełączany.
...
### <a name="stylus-input"></a>Wprowadzanie piórem
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] ma zintegrowane wsparcie dla <xref:System.Windows.Input.Stylus>.  <xref:System.Windows.Input.Stylus> to wprowadzanie piórem popularne przez komputer typu Tablet.  aplikacje [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] mogą traktować pióro jako mysz przy użyciu interfejsu API myszy, ale [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] również uwidacznia abstrakcję urządzenia pióra, która używa modelu podobnego do klawiatury i myszy.  Wszystkie interfejsy API związane z piórem zawierają słowo "Pisak".
...
## <a name="event-routing"></a>Routing zdarzeń
 <xref:System.Windows.FrameworkElement> może zawierać inne elementy jako elementy podrzędne w modelu zawartości, tworząc drzewo elementów.  W [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]element nadrzędny może uczestniczyć w danych wejściowych skierowanych do elementów podrzędnych lub innych elementów podrzędnych przez podanie zdarzeń. Jest to szczególnie przydatne w przypadku kompilowania kontrolek z mniejszych kontrolek, procesów znanych jako "skład kontrolny" lub "złożenie". Aby uzyskać więcej informacji na temat drzew elementów i sposobu, w jaki drzewa elementów odnoszą się do tras zdarzeń, zobacz [drzewa w WPF](trees-in-wpf.md).
...
### <a name="keyboard-input-event-example"></a>Przykład zdarzenia wejściowego klawiatury
 Poniższy przykład nasłuchuje naciśnięcie klawisza Strzałka w lewo.  Tworzony jest <xref:System.Windows.Controls.StackPanel>, który ma <xref:System.Windows.Controls.Button>.  Procedura obsługi zdarzeń do nasłuchiwania dla klawisza Strzałka w lewo jest dołączona do wystąpienia <xref:System.Windows.Controls.Button>.

 Pierwsza sekcja przykładu tworzy <xref:System.Windows.Controls.StackPanel> i <xref:System.Windows.Controls.Button> i dołącza procedurę obsługi zdarzeń <xref:System.Windows.UIElement.KeyDown>.
 ...
 Druga sekcja jest zapisywana w kodzie i definiuje procedurę obsługi zdarzeń.  Po naciśnięciu klawisza Strzałka w lewo, gdy <xref:System.Windows.Controls.Button> ma fokus klawiatury, program obsługi zostanie uruchomiony i zostanie zmieniony kolor <xref:System.Windows.Controls.Control.Background%2A> <xref:System.Windows.Controls.Button>.  Jeśli klawisz jest wciśnięty, ale nie jest to klawisz Strzałka w lewo, kolor <xref:System.Windows.Controls.Control.Background%2A> <xref:System.Windows.Controls.Button> zostanie zmieniony z powrotem do jego koloru początkowego.
...
### <a name="mouse-input-event-example"></a>Przykład zdarzenia wejściowego myszy
 W poniższym przykładzie kolor <xref:System.Windows.Controls.Control.Background%2A> <xref:System.Windows.Controls.Button> zostanie zmieniony, gdy wskaźnik myszy zostanie przesunięty do <xref:System.Windows.Controls.Button>.  Kolor <xref:System.Windows.Controls.Control.Background%2A> jest przywracany po opuszczeniu <xref:System.Windows.Controls.Button>przez wskaźnik myszy.

 Pierwsza sekcja przykładu tworzy <xref:System.Windows.Controls.StackPanel> i kontrolkę <xref:System.Windows.Controls.Button> i dołącza procedury obsługi zdarzeń <xref:System.Windows.UIElement.MouseEnter> i <xref:System.Windows.UIElement.MouseLeave> do <xref:System.Windows.Controls.Button>.
 ...
 Druga sekcja przykładu jest zapisywana w kodzie i definiuje procedury obsługi zdarzeń.  Gdy mysz przejdzie do <xref:System.Windows.Controls.Button>, kolor <xref:System.Windows.Controls.Control.Background%2A> <xref:System.Windows.Controls.Button> zostanie zmieniony na <xref:System.Windows.Media.Brushes.SlateGray%2A>.  Gdy mysz opuści <xref:System.Windows.Controls.Button>, kolor <xref:System.Windows.Controls.Control.Background%2A> <xref:System.Windows.Controls.Button> zostanie zmieniony z powrotem na <xref:System.Windows.Media.Brushes.AliceBlue%2A>.
...
## <a name="text-input"></a>Wprowadzanie tekstu
 ...
 W przypadku danych wejściowych z klawiatury [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] najpierw wysyła odpowiednie <xref:System.Windows.ContentElement.KeyDown>/<xref:System.Windows.ContentElement.KeyUp> zdarzeń. Jeśli te zdarzenia nie są obsługiwane, a klucz to tekst (a nie klawisz kontrolny, taki jak strzałki kierunkowe lub klawisze funkcyjne), zostanie zgłoszone zdarzenie <xref:System.Windows.ContentElement.TextInput>.  Nie zawsze jest proste mapowanie jeden do jednego między <xref:System.Windows.ContentElement.KeyDown>/<xref:System.Windows.ContentElement.KeyUp> i zdarzeniami <xref:System.Windows.ContentElement.TextInput>, ponieważ wielokrotne naciśnięcie klawiszy może generować pojedynczy znak wprowadzania tekstu, a pojedynczy naciśnięcie klawiszy może generować ciągi znaków wielobajtowych.  Jest to szczególnie prawdziwe w przypadku języków takich jak chiński, japoński i koreański, które używają edytorów Input Method Editors (IME) do generowania tysięcy możliwych znaków w odpowiadających im alfabetach.

 Gdy [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] wysyła zdarzenie <xref:System.Windows.ContentElement.KeyDown> /<xref:System.Windows.ContentElement.KeyUp>, <xref:System.Windows.Input.KeyEventArgs.Key%2A> jest ustawiona na <xref:System.Windows.Input.Key.System?displayProperty=nameWithType>, jeśli naciśnięcia klawiszy może stać się częścią zdarzenia <xref:System.Windows.ContentElement.TextInput> (na przykład naciśnięcie klawiszy ALT + S). Umożliwia to kod w programie obsługi zdarzeń <xref:System.Windows.ContentElement.KeyDown>, aby sprawdzić, czy <xref:System.Windows.Input.Key.System?displayProperty=nameWithType> i, w przypadku znalezienia, pozostawić przetwarzanie dla programu obsługi podniesionego zdarzenia <xref:System.Windows.ContentElement.TextInput>. W takich przypadkach różne właściwości argumentu <xref:System.Windows.Input.TextCompositionEventArgs> mogą służyć do określenia oryginalnych naciśnięć klawiszy. Podobnie, jeśli edytor IME jest aktywny, <xref:System.Windows.Input.Key> ma wartość <xref:System.Windows.Input.Key.ImeProcessed?displayProperty=nameWithType>i <xref:System.Windows.Input.KeyEventArgs.ImeProcessedKey%2A> nadaje oryginalne naciśnięcie klawiszy lub naciśnięcia klawiszy.

 W poniższym przykładzie zdefiniowano procedurę obsługi dla zdarzenia <xref:System.Windows.Controls.Primitives.ButtonBase.Click> i programu obsługi zdarzenia <xref:System.Windows.UIElement.KeyDown>.
 ...
 Ponieważ zdarzenia wejściowe są bąbelkowe w górę trasy zdarzeń, <xref:System.Windows.Controls.StackPanel> otrzymuje dane wejściowe, niezależnie od tego, który element ma fokus klawiatury. Formant <xref:System.Windows.Controls.TextBox> jest powiadamiany jako pierwszy, a procedura obsługi `OnTextInputKeyDown` zostanie wywołana tylko wtedy, gdy <xref:System.Windows.Controls.TextBox> nie obsłużył danych wejściowych. Jeśli <xref:System.Windows.UIElement.PreviewKeyDown> zdarzenie jest używane zamiast zdarzenia <xref:System.Windows.UIElement.KeyDown>, procedura obsługi `OnTextInputKeyDown` jest wywoływana jako pierwsza.
...
- `touch device` reprezentuje urządzenie generujące dane wejściowe dotyku, takie jak pojedynczy Finger na ekranie dotykowym.
...
- <xref:System.Windows.UIElement.LostTouchCapture>
 ...
 Podobnie jak zdarzenia klawiatury i myszy, zdarzenia touch są zdarzeniami kierowanymi. Zdarzenia rozpoczynające się od `Preview` są zdarzeniami tunelowania, a zdarzenia, które zaczynają się od `Touch` są zdarzeniami propagacji. Aby uzyskać więcej informacji na temat zdarzeń kierowanych, zobacz [Omówienie zdarzeń kierowanych](routed-events-overview.md). Podczas obsługi tych zdarzeń można pobrać pozycję danych wejściowych względem dowolnego elementu, wywołując metodę <xref:System.Windows.Input.TouchEventArgs.GetTouchPoint%2A> lub <xref:System.Windows.Input.TouchEventArgs.GetIntermediateTouchPoints%2A>.
...
1. Zdarzenie <xref:System.Windows.UIElement.TouchEnter> występuje raz, gdy użytkownik umieści palca w elemencie.
...
4. Zdarzenie <xref:System.Windows.UIElement.TouchUp> występuje jednorazowo, gdy użytkownik wydźwiga palca z elementu.
...
1. Zdarzenie <xref:System.Windows.UIElement.ManipulationStarting> występuje, gdy użytkownik umieści palca w obiekcie. To zdarzenie pozwala określić między innymi Właściwość <xref:System.Windows.Input.ManipulationStartingEventArgs.ManipulationContainer%2A>. W kolejnych zdarzeniach pozycja manipulowania będzie odnosząca się do <xref:System.Windows.Input.ManipulationStartingEventArgs.ManipulationContainer%2A>. W przypadku zdarzeń innych niż <xref:System.Windows.UIElement.ManipulationStarting>ta właściwość jest tylko do odczytu, więc zdarzenie <xref:System.Windows.UIElement.ManipulationStarting> jest jedynym czasem, w którym można ustawić tę właściwość.
...
3. Zdarzenie <xref:System.Windows.UIElement.ManipulationDelta> występuje wiele razy, gdy użytkownik przechodzi na ekran dotykowy. Właściwość <xref:System.Windows.Input.ManipulationDeltaEventArgs.DeltaManipulation%2A> klasy <xref:System.Windows.Input.ManipulationDeltaEventArgs> raportuje, czy manipulowanie jest interpretowane jako przesunięcia, rozwinięcia czy tłumaczenie. Jest to miejsce, w którym wykonuje się większość pracy manipulowania obiektem.
...
5. Zdarzenie <xref:System.Windows.UIElement.ManipulationDelta> występuje wiele razy, gdy następuje bezwładności. Należy zauważyć, że to zdarzenie występuje, gdy użytkownik przechodzi przez palców na ekranie dotykowym i gdy [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] symuluje bezwładności. Innymi słowy, <xref:System.Windows.UIElement.ManipulationDelta> występuje przed zdarzeniem <xref:System.Windows.UIElement.ManipulationInertiaStarting> i po nim. Właściwość <xref:System.Windows.Input.ManipulationDeltaEventArgs.IsInertial%2A?displayProperty=nameWithType> określa, czy zdarzenie <xref:System.Windows.UIElement.ManipulationDelta> występuje w czasie bezwładności, dlatego można sprawdzić tę właściwość i wykonać różne akcje, w zależności od jej wartości.

6. Zdarzenie <xref:System.Windows.UIElement.ManipulationCompleted> występuje podczas manipulowania i dowolnych punktów końcowych. Oznacza to, że po wystąpieniu wszystkich <xref:System.Windows.UIElement.ManipulationDelta> zdarzenia <xref:System.Windows.UIElement.ManipulationCompleted> nastąpi zasygnalizowanie, że manipulacja została zakończona.

 <xref:System.Windows.UIElement> definiuje również zdarzenie <xref:System.Windows.UIElement.ManipulationBoundaryFeedback>. To zdarzenie występuje, gdy metoda <xref:System.Windows.Input.ManipulationDeltaEventArgs.ReportBoundaryFeedback%2A> jest wywoływana w zdarzeniu <xref:System.Windows.UIElement.ManipulationDelta>. Zdarzenie <xref:System.Windows.UIElement.ManipulationBoundaryFeedback> umożliwia aplikacjom i składnikom dostarczanie wizualizacji wizualnych, gdy obiekt osiągnie granicę. Na przykład Klasa <xref:System.Windows.Window> obsługuje zdarzenie <xref:System.Windows.UIElement.ManipulationBoundaryFeedback>, aby uniemożliwić przechodzenie okna po napotkaniu jego krawędzi.
...
|<xref:System.Windows.UIElement.ManipulationInertiaStarting> i <xref:System.Windows.UIElement.ManipulationCompleted>|Mysz w dół, ruch przychodzący i wskaźnik myszy.|
 ...
 Należy pamiętać, że w przypadku wywołania <xref:System.Windows.Input.ManipulationStartingEventArgs.Cancel%2A>, gdy manipulacja jest w trybie bezwładności, metoda zwraca `false` i dane wejściowe nie powodują zdarzeń myszy.

### <a name="the-relationship-between-touch-and-manipulation-events"></a>Relacja między zdarzeniami dotykowymi i manipulowania
 <xref:System.Windows.UIElement> zawsze może odbierać zdarzenia dotknięcia. Gdy właściwość <xref:System.Windows.UIElement.IsManipulationEnabled%2A> jest ustawiona na `true`, <xref:System.Windows.UIElement> może odbierać zarówno zdarzenia dotykowe, jak i manipulowania.  Jeśli zdarzenie <xref:System.Windows.UIElement.TouchDown> nie jest obsługiwane (oznacza to, że właściwość <xref:System.Windows.RoutedEventArgs.Handled%2A> jest `false`), logika manipulowania przechwytuje element i generuje zdarzenia manipulowania. Jeśli właściwość <xref:System.Windows.RoutedEventArgs.Handled%2A> jest ustawiona na `true` w zdarzeniu <xref:System.Windows.UIElement.TouchDown>, logika manipulowania nie generuje zdarzeń manipulowania. Na poniższej ilustracji przedstawiono relacje między zdarzeniami Touch i zdarzeniami manipulowania.

 ![Relacja między zdarzeniami dotykowymi i manipulowania](./media/ndp-touchmanipulateevents.png "NDP_TouchManipulateEvents") zdarzeniami Touch i manipulowania
...
- Gdy wystąpi <xref:System.Windows.UIElement.GotTouchCapture>, logika manipulowania wywołuje metodę <xref:System.Windows.Input.Manipulation.AddManipulator%2A?displayProperty=nameWithType>, która generuje zdarzenie <xref:System.Windows.UIElement.ManipulationStarting>.

- Po wystąpieniu zdarzeń <xref:System.Windows.UIElement.TouchMove> logiki manipulowania generuje zdarzenia <xref:System.Windows.UIElement.ManipulationDelta> występujące przed zdarzeniem <xref:System.Windows.UIElement.ManipulationInertiaStarting>.

- Gdy ostatnie urządzenie dotykowe w elemencie zgłasza zdarzenie <xref:System.Windows.UIElement.TouchUp>, logika manipulacji generuje zdarzenie <xref:System.Windows.UIElement.ManipulationInertiaStarting>.

<a name="focus"></a>
## <a name="focus"></a>Fokus
 Istnieją dwa główne pojęcia, które odnoszą się do skoncentrowania się w [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]: fokus klawiatury i fokus logiczny.

### <a name="keyboard-focus"></a>Fokus klawiatury
 Fokus klawiatury odnosi się do elementu, który otrzymuje dane wejściowe z klawiatury.  Na całym pulpicie może znajdować się tylko jeden element, który ma fokus klawiatury.  W [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]element, który ma fokus klawiatury, będzie miał <xref:System.Windows.IInputElement.IsKeyboardFocused%2A> ustawiony na `true`.  Metoda static <xref:System.Windows.Input.Keyboard> <xref:System.Windows.Input.Keyboard.FocusedElement%2A> zwraca element, który aktualnie ma fokus klawiatury.

 Fokus klawiatury można uzyskać, naciskając klawisz Tab do elementu lub klikając myszą na określonych elementach, takich jak <xref:System.Windows.Controls.TextBox>.  Fokus klawiatury można również uzyskać programowo przy użyciu metody <xref:System.Windows.Input.Keyboard.Focus%2A> klasy <xref:System.Windows.Input.Keyboard>.  <xref:System.Windows.Input.Keyboard.Focus%2A> próbuje nadać określony fokus klawiatury elementu.  Element zwracany przez <xref:System.Windows.Input.Keyboard.Focus%2A> to element, który aktualnie ma fokus klawiatury.
...
### <a name="logical-focus"></a>Fokus logiczny
 ...
 Element może zostać włączony do zakresu fokusu w [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] przez ustawienie <xref:System.Windows.Input.FocusManager> dołączonej właściwości <xref:System.Windows.Input.FocusManager.IsFocusScope%2A> do `true`lub w kodzie przez ustawienie dołączonej właściwości przy użyciu metody <xref:System.Windows.Input.FocusManager.SetIsFocusScope%2A>.

 Poniższy przykład powoduje, że <xref:System.Windows.Controls.StackPanel> do zakresu fokusu przez ustawienie <xref:System.Windows.Input.FocusManager.IsFocusScope%2A> dołączonej właściwości.
 ...
 Element, który ma fokus klawiatury, będzie również miał fokus logiczny dla zakresu fokusu, do którego należy. w związku z tym ustawienie fokusu na elemencie z metodą <xref:System.Windows.Input.Keyboard.Focus%2A> na klasie <xref:System.Windows.Input.Keyboard> lub klasy elementu podstawowego podejmie próbę nawiązania fokusu na klawiaturę i logicznego fokusu.
...
## <a name="the-input-system-and-base-elements"></a>System wejściowy i elementy podstawowe
 Zdarzenia wejściowe, takie jak dołączone zdarzenia zdefiniowane przez klasy <xref:System.Windows.Input.Mouse>, <xref:System.Windows.Input.Keyboard>i <xref:System.Windows.Input.Stylus> są wywoływane przez system wejściowy i wprowadzane do określonej pozycji w modelu obiektów na podstawie trafień podczas testowania drzewa wizualnego w czasie wykonywania.

 Wszystkie zdarzenia, które <xref:System.Windows.Input.Mouse>, <xref:System.Windows.Input.Keyboard>i <xref:System.Windows.Input.Stylus> definiować jako dołączone zdarzenie, są również ponownie uwidaczniane przez klasy elementów podstawowych <xref:System.Windows.UIElement> i <xref:System.Windows.ContentElement> jako nowe zdarzenie kierowane. Zdarzenia przesyłane do elementu podstawowego są generowane przez klasy obsługujące pierwotne dołączone zdarzenie i ponownie korzystające z danych zdarzenia.
...
## <a name="whats-next"></a>Co dalej
 Teraz masz kilka technik obsługi danych wejściowych w [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)].  Należy również poprawić różne typy zdarzeń wejściowych i mechanizmów zdarzeń kierowanych używanych przez [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)].

 Dostępne są dodatkowe zasoby, które szczegółowo objaśniają [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] elementów struktury i routingu zdarzeń. Zapoznaj się z poniższymi omówieniami, aby uzyskać więcej informacji, [Omówienie poleceń](commanding-overview.md), [Przegląd koncentracji](focus-overview.md), [Przegląd elementów podstawowych](base-elements-overview.md), [drzewa w WPF](trees-in-wpf.md)i [Informacje o rutowanych zdarzeniach](routed-events-overview.md).
