...
- Jeśli określisz, że testy widoczności JIT powinny być pomijane, żądanie, które jest wykonywane, gdy metoda jest zbudowana, obejmuje <xref:System.Security.Permissions.ReflectionPermission> <xref:System.Security.Permissions.ReflectionPermissionFlag.RestrictedMemberAccess?displayProperty=nameWithType> flagę i zestaw dotacji zestawu, który zawiera niepubliczny element członkowski, do którego jest uzyskiwany dostęp.  
 ...
 Zestaw systemowy, który hostuje anonimowo obsługiwane metody dynamiczne, korzysta z <xref:System.Security.SecurityRuleSet.Level1?displayProperty=nameWithType> modelu przezroczystości, który jest modelem przezroczystości, który został użyty w .NET Framework przed .NET Framework 4.  
...
- Jeśli określisz, że testy widoczności JIT powinny być pomijane, Konstruktor wymaga <xref:System.Security.Permissions.ReflectionPermission> flagi z <xref:System.Security.Permissions.ReflectionPermissionFlag.MemberAccess?displayProperty=nameWithType> flagą.  
