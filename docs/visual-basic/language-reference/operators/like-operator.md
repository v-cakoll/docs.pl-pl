...
## <a name="parts"></a>Części  
 `result`  
 Wymagany. Any `Boolean` variable. The result is a `Boolean` value indicating whether or not the `string` satisfies the `pattern`.  
  
 `string`  
 Wymagany. Any `String` expression.  
  
 `pattern`  
 Wymagany. Any `String` expression conforming to the pattern-matching conventions described in "Remarks."  
