...
W przykładach w tym temacie pokazano, jak używać <xref:System.Linq.Enumerable.Select%2A> metod <xref:System.Linq.Enumerable.SelectMany%2A> i do wykonywania zapytań <xref:System.Data.DataSet> przy użyciu składni zapytania opartego na metodzie.  
...
### <a name="example"></a>Przykład  
 W tym przykładzie używa <xref:System.Linq.Enumerable.Select%2A> metody do `Name`prognozowania właściwości `ProductNumber`,, `ListPrice` i do sekwencji typów anonimowych.  Nazwa właściwości jest również zmieniana `Price` na typ wyników. `ListPrice`  
...
### <a name="example"></a>Przykład  
 W tym przykładzie zastosowano metodę, <xref:System.Linq.Enumerable.SelectMany%2A> aby wybrać wszystkie zamówienia, których `TotalDue` wartość jest mniejsza niż 500,00.  
