...
4. W **Eksploratorze projektów**kliknij prawym przyciskiem myszy `Class1` , a następnie kliknij polecenie **Właściwości**. Zostanie wyświetlone okno **Właściwości** klasy.

5. Zmień wartość `Name` właściwości na `MathFunctions` .

6. W **Eksploratorze projektów**kliknij prawym przyciskiem myszy `MathFunctions` , a następnie kliknij polecenie **Wyświetl kod**. Zostanie wyświetlony **Edytor kodu** .
...
8. Dodaj właściwości `Let` i `Get` procedury właściwości właściwości:
...
10. Utwórz i zarejestruj obiekt COM, klikając pozycję **Utwórz ComObject1. dll** w menu **plik** .
    ...
    > [!NOTE]
    > Chociaż można również uwidocznić klasę utworzoną za pomocą Visual Basic jako obiekt COM, nie jest to prawdziwy obiekt COM i nie można jej użyć w tym instruktażu. Aby uzyskać szczegółowe informacje, zobacz [współdziałanie com w aplikacjach .NET Framework](com-interoperability-in-net-framework-applications.md).
...
2. W `Click` procedurze obsługi zdarzeń przycisku Dodaj następujący kod, aby utworzyć wystąpienie `MathClass` i wywołać przeciążone metody:
...
- [Współdziałanie COM w aplikacjach .NET Framework](com-interoperability-in-net-framework-applications.md)
- [Inherits — Instrukcja](../../language-reference/statements/inherits-statement.md)
- [Short, typ danych](../../language-reference/data-types/short-data-type.md)
