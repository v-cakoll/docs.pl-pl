...
---
# <a name="caller-information-c"></a>Informacje o obiekcie wywołującym (C#)
...
Aby uzyskać te informacje, należy użyć atrybutów stosowanych do opcjonalnych parametrów, z których każdy ma wartość domyślną. W poniższej tabeli wymieniono atrybuty Informacje <xref:System.Runtime.CompilerServices?displayProperty=nameWithType> o obiekcie wywołującym zdefiniowane w obszarze nazw:
...
Poniższy przykład przedstawia, jak używać atrybutów informacji o obiekcie wywołującym. Przy każdym wywołaniu `TraceMessage` metody informacje o obiekcie wywołującym są zastępowane jako argumenty parametrów opcjonalnych.
...
Wartości informacji o obiekcie wywołującym są emitowane jako literały do języka pośredniego (IL, Intermediate Language) w czasie kompilacji. W przeciwieństwie do <xref:System.Exception.StackTrace%2A> wyników właściwości dla wyjątków, wyniki nie są dotknięte zaciemnianiem.
...
Atrybutu można `CallerMemberName` użyć, aby uniknąć określania `String` nazwy elementu członkowskiego jako argumentu do metody wywoływanej. Za pomocą tej techniki można uniknąć problemu, że **Rename Refaktoryzacja** nie zmienia `String` wartości. Jest to szczególnie przydatne w następujących zadaniach:
...
Na poniższej tabeli przedstawiono nazwy elementów członkowskich, `CallerMemberName` które są zwracane podczas korzystania z atrybutu.
...
- [Typowe atrybuty (C#)](./attributes/common-attributes.md)
